...
> **アプリ内UIは韓国語のみです**（ポケモン・わざ名も含む。PokéAPIの韓国語名を使い、韓国語が無い場合のみ英語にフォールバックします）。このREADMEはドキュメント用に英語・日本語版も維持します。
...
4. 経験値でパートナーを育て、ほしのかけらはショップやランクバトルの賭け金に使います。ホームで最大5匹の控え相棒を選んで冒険パーティを組むと、受け取り時にパートナーが経験値100%、選んだ控えがそれぞれ30%を一緒に受け取ります。ホームには冒険の受け取り・集中時間・卒業を数えるデイリー／ウィークリーミッションがあり、図鑑では種・タイプ・色違いの収集目標も達成できます。
...
- **バトルのルール** — タイプ相性、タイプ一致、物理・特殊能力、命中、急所、PP、わざの優先度、6種の状態異常、混乱に対応しています。天候・フィールド、リフレクター・ひかりのかべ・オーロラベールなどの場の効果、おいかぜの優先度補正、まもる系統8種、テラスタルにも対応します(れんしゅうバトルとLAN1対1で使用可能)。
- **もちもの** — バトルで使うもちもの161種をショップの「バトル」タブで購入し、ポケモンに持たせられます。タイプ強化アイテム・きのみ・ジュエル・プレートから、いのちのたま・こだわり系・とつげきチョッキまであり、それぞれ代償が異なります。何を持っているかはポケモンの詳細カードとグリッドのバッグ印で確認でき、連れているポケモンならその場で外せます。バトル画面では自分側のもちものだけが表示され、相手のものは発動したときにログで明かされます。
...
- **共闘レイド** — 同じネットワークで最大8人のトレーナーと協力してボスに挑みます。午前・午後で異なるボスが登場し、さらにティアごとにも異なるボスです — 1★は高級種、3★は希少種、5★は伝説。報酬と捕獲のチャンスは時間帯・ティアごとに1回ずつなので、1日最大6回(午前3回・午後3回)挑む価値があります。報酬を受け取った後も参加でき、失敗した挑戦は回数を消費しません。成功すると最後まで部屋に残った参加者ごとに(倒れても抜けていなければ対象)、高級25%・希少15%・伝説5%の確率でボスを捕獲し、チャットと一緒に結果が一人ずつ順番に表示されます。
...
- **ウェーブラン** — 最初のポケモンを選び、30ウェーブに挑みます。4ウェーブごとにボスが現れ（突破すると手持ちのHPが最大70%まで回復）、ウェーブを突破するたびにアイテムを1つ選びます。野生のウェーブではモンスターボール（1ランに5個、最大9個）を投げて相手を捕まえ、手持ちを6匹まで増やせます。野生ウェーブの8回に1回ほど相手が2匹ずつ現れ、そのときは自分も2匹を並べて戦うダブルバトルになります — 枠ごとに行動を選び、じしんのような範囲の広い技はフィールドの複数体を一度に攻撃します。ランは毎回新しく抽選され、1日の回数制限はなく、進行中のランはアプリを終了しても続きから再開できます。毎日の初回クリアではタマゴ1個とほしのかけら5,000個を受け取れます。

## サファリゾーンとポコピアタウン

- **サファリゾーン** — 草原・湿地・洞窟の3つのゾーンから1つを選び、方向キーでフィールドを歩いて野生のポケモンを探します。エンカウントでは、えさ(捕獲率が上がるが、にげる確率も上がる)・どろ(にげる確率が下がるが、捕獲率も下がる)・ボール・にげるから選び、ボールを投げるとポケモンに向かって飛んでいくアニメーションが再生されます。エンカウント画面では相手の性別・タイプ・とくせいを事前に確認でき、フィールドには木・水たまり・岩などの障害物があります。ボールと歩数は訪問ごとに、訪問と捕獲は1日ごとに上限があります。
- **ポコピアタウン** — 図鑑に登録した種にへんしんすると、そのタイプに合った地形(草・水・砂・岩など)を無料で押し広げられます。集中セッションを終えるたびに移住判定が行われ、タウンの環境に合ったポケモンが自分から住み着きます — 地形を多様にするほど受け入れられる住民が増えます。地形の種類数と定着率に応じて環境レベル(Lv.1〜10、空き地からポコピアまでの5段階)が上下する、専用のウィンドウです。
...
<td width="55%" valign="middle">
<h3>ポケモンと図鑑</h3>
手持ちの確認、パートナー交代、わざ・経験値の確認、発見した種の閲覧ができます。名前で検索すれば、ボックス・チーム選択・交換リストから目的のポケモンをすぐに呼び出せます。さらに、図鑑にまだ永続登録されていないポケモンや、現在の姿が違っても同じ進化系統に属する重複ポケモンだけに絞り込めます。ポケモンタブ以外の選択一覧はほとんどがレベルの高い順で表示され(チーム選択は並べ替えボタンで名前順・ふ化順にも切り替えられます)、オークションに出品するポケモンを選ぶ一覧だけが名前順です。第1〜9世代の全種に対応しています（アニメーションのない第9世代後半の14種を除く）。大切なポケモンに星を付けると、にがす・交換・出品ができなくなり、見出しの星でおきにいりだけを表示できます。詳細カードでは、とくせい・種族値・わざに加えてもちものとテラスタイプを確認でき、連れているポケモンならその場でもちものを外せます。
</td>
<td width="45%" align="center"><img src="assets/screenshot-pokemon-favorites-ja.png" width="180" alt="おきにいり"><br><br><img src="assets/screenshot-pokemon-held-item-ja.png" width="180" alt="ポケモン詳細カードのもちもの"><br><br><img src="assets/screenshot-collection-pokedex.png" width="180" alt="図鑑"><br><br><img src="assets/screenshot-collection-catchlog.png" width="180" alt="ポケモンのコレクションログ"></td>
</tr>
<tr>
...
<td width="55%" valign="middle">
<h3>ショップとバッグ</h3>
ほしのかけらでタマゴ、ふしぎなアメ、ミント、つながりのヒモ、進化のいし、わざマシン、ふくを購入します。バトルで使うもちもの161種は「バトル」タブにまとまっているので、消費アイテムに埋もれません。まとめて持てる品は所持金で買える範囲までまとめ買いでき、バッグではアイテムを使ったり不要なものをすてたりできます。
</td>
</tr>
<tr>
<td width="55%" valign="middle">
<h3>作業と更新の設定</h3>
設定から、おやすみモード、通知、ログイン時の起動、フローティングペット、更新確認を管理できます。ポップオーバーを開閉するグローバルショートカットもここで指定します(既定は未設定)。更新後の初回起動で新着情報を一度だけ表示し、このウィンドウは設定でオフにできます。
</td>
...
<td width="55%" valign="middle">
<h3>共闘レイド</h3>
同じネットワークのトレーナーと一緒にボスを倒します。午前・午後だけでなくティアごとにも異なるボスが登場し(1★高級・3★希少・5★伝説)、報酬は時間帯・ティアごとに1回受け取れます。1★・3★はいつでも開け、5★は1日3回決まった時刻に出現します。画面の下で連れていくポケモンを選べ、近くに開いている部屋があれば一覧からすぐ参加できます。
</td>
</tr>
<tr>
<td width="55%" valign="middle">
...
<tr>
<td width="45%" align="center"><img src="assets/screenshot-safari-walk-ja.png" width="180" alt="サファリゾーンの歩行画面"><br><br><img src="assets/screenshot-safari-encounter-ja.png" width="180" alt="サファリゾーンのエンカウント画面"></td>
<td width="55%" valign="middle">
<h3>サファリゾーン</h3>
方向キーでフィールドを歩いて野生のポケモンを探します。エンカウントではえさ・どろ・ボール・にげるから選び、相手の性別・タイプ・とくせいを事前に確認できます。
</td>
</tr>
<tr>
<td width="55%" valign="middle">
<h3>ポコピアタウン</h3>
へんしんした種のタイプに合わせて地形を押し広げ、集中セッションを終えるたびにタウンの環境に合ったポケモンが住み着きます。
</td>
<td width="45%" align="center"><img src="assets/screenshot-pokopia-town-ja.png" width="180" alt="ポコピアタウン"></td>
</tr>
</table>
