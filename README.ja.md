...
- **共闘レイド** — 同じネットワークのトレーナーと協力してボスに挑みます。午前・午後で異なるボスが登場し、さらにティアごとにも異なるボスです — 1★は高級種、3★は希少種、5★は伝説。報酬と捕獲のチャンスは時間帯・ティアごとに1回ずつなので、1日最大6回(午前3回・午後3回)挑む価値があります。報酬を受け取った後も参加でき、失敗した挑戦は回数を消費しません。成功すると最後まで部屋に残った参加者ごとに(倒れても抜けていなければ対象)、高級25%・希少15%・伝説5%の確率でボスを捕獲し、チャットと一緒に結果が一人ずつ順番に表示されます。
