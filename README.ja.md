...
- **共闘レイド** — 同じネットワークのトレーナーと協力し、正午の前後で異なるボスに挑みます。報酬は午前・午後に1回ずつ受け取れ、受け取り後も報酬なしで参加できます。失敗した挑戦は回数を消費しません。成功すると生存者ごとに、高級25%・希少15%・伝説5%の確率でボスを捕獲し、チャットと一緒に結果が一人ずつ順番に表示されます。
...
<td width="55%" valign="middle">
<h3>ポケモンと図鑑</h3>
手持ちの確認、パートナー交代、わざ・経験値の確認、発見した種の閲覧ができます。名前で検索すれば、ボックス・チーム選択・交換リストから目的のポケモンをすぐに呼び出せ、同じ図鑑番号を複数持つ種だけに絞り込むこともできます。ポケモンタブ以外の選択一覧は名前順で表示されます。第1〜9世代の全種に対応しています（アニメーションのない第9世代後半の14種を除く）。大切なポケモンに星を付けると、にがす・交換・出品ができなくなり、見出しの星でおきにいりだけを表示できます。
</td>
