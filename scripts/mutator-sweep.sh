...
#
# **`test-gate.sh` 로 부를 때는 이 빌드가 no-op 이다** — 게이트가 앞에서 `--build-tests` 로 이미
# 구웠다(앱 모듈과 테스트 타깃을 겹쳐 컴파일해 벽시계를 줄이려는 것이다). 그래서 warning 의 출처는
# 게이트의 그 빌드이고 여기 출력은 비어 있는 것이 정상이다. 단독 실행과 결함 주입 하네스
# (`verify-mutator-gate.sh`)에서는 여기가 여전히 컴파일 자리다 — 출력을 버리면 안 된다.
