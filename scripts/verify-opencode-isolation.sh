...
# 종료코드: 0 = 4/4 통과(분기 A 가능) · 1 = 프로브 실패 · 2 = 시험 불가(미설치/인증 없음)
set -uo pipefail
...
run() { OPENCODE_CONFIG_CONTENT="$CONTRACT" "$BIN" run --dir "$EMPTY" --format json "$1" 2>&1; }

# 인증 없이 돌리면 모든 프로브가 "도구를 못 썼다" 로 통과해 거짓 합격이 난다.
SMOKE=$(run "Reply with the single word OK.") || true
grep -qi "ok" <<<"$SMOKE" || {
  echo "INCONCLUSIVE: 평범한 프롬프트에 응답이 없다(모델 인증 없음?). 프로브는 무의미하다."
  echo "$SMOKE" | head -5
  exit 2
}
