...
本リポジトリでは、共同作業で残るもの（Issue・PR・公開コメント）の作業言語を **韓国語**とします:

- **プルリクエストのタイトルと本文は韓国語で書きます。**
- **コミットメッセージも韓国語で書きます。**
- コード識別子・API 名・CLI コマンド・エラー文字列はそのまま残します。

リポジトリは squash-merge するため、PR タイトルが `main` のコミット件名になります —
公開履歴も韓国語になり、それが意図した結果です。

他の言語のほうが書きやすい場合は、その言語で PR を開いてそう書いてください。マージ前に
メンテナがタイトルを訳します。言語を理由に貢献を断るための規則ではありません。
...
（[README](README.md#데이터-개인정보와-면책) の免責を参照 — README は韓国語版のみです）。
プロジェクトを安全に維持・
