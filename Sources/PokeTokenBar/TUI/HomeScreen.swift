...
/// 앱이 꺼져 있어도 답한다.
///
/// **이 방의 픽셀 아트는 옮기지 않는다.** 방의 그림·창밖 배색·스프라이트 위치는 여러 겹이
/// 겹친 배치라 20칸으로 접으면 무엇이 어디 있는지 알 수 없다. 남는 것은 사람이 읽는 사실 —
/// 무엇이 놓여 있고, 어느 스타일이며, 누가 같이 살고, 무엇이 기억에 남았는지다.
///
/// 홈 화면(`TUIRender.home`)의 **파트너 그림은 다른 경우**다(`TUISprite`) — 겹치는 것 없이
/// 스프라이트 하나라 사분면 트루컬러로 접힌다. 범위가 다르니 여기 규칙을 그쪽에 적용하지 않는다.
