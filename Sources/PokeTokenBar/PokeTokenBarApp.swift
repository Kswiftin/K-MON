...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, UNUserNotificationCenterDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        pokopiaPresenter = PokopiaTownPresenter(settings: settings, store: companion, visits: memoryHomeVisits)
    ...
    private func startIdleTick() {
        ...
        let timer = Timer.scheduledTimer(withTimeInterval: 60, repeats: true) { [weak self] _ in
            Task { @MainActor in
                guard let self else { return }
                self.companion.tick()
                // 레이드 보스 교체(정오·자정) 알림 재예약. 날짜가 바뀐 첫 틱에서만 실제로
                // 일한다 — 자정에 깨어날 타이머를 따로 두지 않으려고 이미 도는 틱에 얹는다.
    ...
    private func publishTerminalViewIfNeeded() {
        ...
        // 그 규칙에 테스트가 닿지 않는다. 대전이 앞인 이유: 라이브 판이 도는 동안 타이머 줄을
        // 그리면 사용자는 자기 차례를 놓친다.
