...
struct RaidView: View {
    ...
    /// 둔다), 실제로 1시간 로테이션·보상/포획 무제한이 켜졌는지 이 줄 하나로 가늠한다(2026-09-10
            ...
            TimelineView(.periodic(from: .now, by: 60)) { _ in
                VStack(alignment: .leading, spacing: 2) {
                    Label("한정 이벤트 진행 중", systemImage: "sparkles")
                        .font(.caption.bold())
                    Text("이로치 확률 4배 · 보스 1시간마다 교체 · 보상·포획 무제한 · \(eventRemainingText) 남음")
                        .font(.caption2)
    ...
    private var arena: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            ...
            BattleChatPanel(configuration: BattleChatConfiguration(
                ...
                turnSignal: center.combatRound, l: l, onSend: center.sendChat))
        }
