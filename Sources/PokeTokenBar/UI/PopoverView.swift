...
struct PopoverView: View {
    ...
    ///
    /// 파트너 카드가 아니라 여기 있어야 하는 이유: 레벨은 졸업으로 초기화되지 않는 **계정** 값이다.
    /// 라벨에 "트레이너"를 박는 것도 필수다 — `Lv.N` 만 쓰면 파트너·로스터·배틀이 이미 쓰는 표기와
    /// 겹쳐 포켓몬 레벨로 오인지된다. 게이지와 남은 포인트가 있어야 배지가 아니라 쫓을 목표가 되고,
