...
struct FocusRecapView: View {
    ...
    /// 로케일을 **명시**한다. `Date.formatted()` 기본값은 `Locale.autoupdatingCurrent` 라
    /// 시스템 로케일을 읽고, 그러면 영어 로케일 Mac 에서 요일만 영어로 나온다.
    private var locale: Locale { PokemonNaming.locale }
    ...
    private var header: some View {
        PokedoroOverlayHeader(title: "주간 회고",
    ...
    private func summary(_ recap: FocusWeekRecap) -> some View {
        HStack(spacing: 6) {
            Text("\(recap.sessions)세션 · \(recap.minutes)분")
                .font(.caption.weight(.semibold)).monospacedDigit()
            Spacer()
            Text("하루 평균 \(averageText(recap))세션")
    ...
    private func labelList(_ recap: FocusWeekRecap) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            ForEach(recap.labels.prefix(FocusWeekRecap.labelRowLimit), id: \.label) { total in
                HStack(spacing: 6) {
                    Text(total.label ?? "라벨 없음")
                        .font(.caption)
                        .foregroundStyle(total.label == nil ? .secondary : .primary)
                        .lineLimit(1).truncationMode(.tail)
                    Spacer(minLength: 4)
                    Text("\(total.sessions)회 · \(total.minutes)분")
                        .font(.caption2).monospacedDigit().foregroundStyle(.secondary)
                }
            }
            // PRD 성공 지표 3 이 이 한 줄이다 — 라벨을 아무도 안 쓰면 기록의 회고 가치가 없다는 신호.
            Text("라벨 붙은 세션 \(recap.labeledSessions)/\(recap.sessions)")
                .font(.caption2).monospacedDigit().foregroundStyle(.secondary)
                .padding(.top, 2)
        }
    }
    ...
    private func chainLine(_ recap: FocusWeekRecap) -> some View {
        VStack(alignment: .leading, spacing: 1) {
            Text("이어서 시작한 세션 \(recap.chainedSessions)/\(recap.sessions)")
                .font(.caption.weight(.semibold)).monospacedDigit()
            Text("앞 세션이 끝나고 \(FocusWeekRecap.chainWindowMinutes)분 안에 시작한 경우")
