...
enum SaveTransfer {
    ...
    static func sanitized(_ state: CompanionState) -> CompanionState {
        ...
        // 같은 이유로 업적도 경계에서 한 번만 자른다 — 사라진 트랙의 잔재를 버리고 마지막 문턱으로
        // 클램프한다. 클램프된 값은 곧 최고 단계라 손편집으로 넘겨도 보상이 다시 나오지 않는다.
    ...
    static func canonicalString(_ s: CompanionState) -> String {
        ...
        // 업적 카운터가 곧 단계 판정이다 — 서명 밖에 있으면 값을 올려 적는 것만으로 단계 보상을 받는다.
        // 조건부인 이유는 위 두 필드와 같다. 업적은 **새 필드**라 값이 든 기존 세이브가 없어
        // `integrityVersion` 상향이 필요하지 않다(올리면 그 배포에서 모든 세이브가 검사 면제를 받는다).
        //
        // 접두는 `ac` 가 아니라 `ach` 다 — 아래 활성 포켓몬이 `act` 를 쓰고 있어, `ac` 로 두면
        // 기본값 세이브의 `|act-` 가 "업적 세그먼트가 붙었다" 로 읽힌다(`shc` 가 `sec` 를 피한 것과 같은 이유).
        // 해시는 문자열 전체에 걸리니 위조가 되는 건 아니지만, 세그먼트 유무를 보는 테스트가 조용히 거짓이 된다.
