...
enum SaveTransfer {
    ...
    /// 조작 판정된다(2026-09-08).
    /// `homePartyIDs` 저장 형식을 `[active.id] + 예비` 에서 **예비만**으로 바꿨다 → 11 → 12.
    /// 활성을 같이 담으면 활성이 `switchCompanion` 으로 바뀐 뒤 예전 활성 id 가 배열에 남아
    /// "사용자가 고른 예비"로 오인돼 2~6번이 밀렸다(#296). 같은 예비 조합(활성=A, 예비=[X,Y])도
    /// 옛 저장은 `hparty<A>,<X>,<Y>`, 새 저장은 `hparty<X>,<Y>` 로 canonical 문자열 자체가
    /// 달라진다 — 모험 파티를 한 번이라도 편성한 기존 세이브가 전부 조작 판정된다(2026-09-08).
    /// 12 → 13(2026-09-10): 6성 레이드 도전 횟수 제한을 없애며 `weeklyRaidAttemptDate`/
    /// `weeklyRaidAttemptsToday` 필드와 그 조건부 `r6a` 세그먼트를 통째로 지웠다. **새 필드 추가와
    /// 반대 방향이라 위 "안 올린다" 규칙이 적용되지 않는다** — 그 필드가 이미 채워져 있던(6성 레이드를
    ...
    static func canonicalString(_ s: CompanionState,
        ...
        if !s.raidRewardDateTierSix.isEmpty { p.append("r6d\(s.raidRewardDateTierSix)") }
        ...
        if s.raidRewardCountTierSix != 0 { p.append("r6c\(s.raidRewardCountTierSix)") }
        if s.raidCatchCountTierSix != 0 { p.append("c6c\(s.raidCatchCountTierSix)") }
