...
struct L {
    ...
    /// 이름을 늘렸지만 상단 피커의 이상 폭은 그대로다 — 계측값 ko 330pt(변화 없음) ·
    /// en 445pt(변화 없음) · ja 445pt(옛 "ポケモン図鑑" 455pt 보다 10pt 좁다).
    /// 단 en·ja 는 다섯 라벨이 이미 콘텐츠 폭 332pt 를 넘어 macOS 가 라벨을 압축한다 — 이 이름과
    /// 무관한 기존 상태다. **이 값을 더 늘리지 않는다** — 늘리면 그 압축이 더 심해진다.
