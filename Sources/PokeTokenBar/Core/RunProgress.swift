...
/// 하루 첫 웨이브 런 클리어 보상. 도전 횟수는 제한하지 않고, 날짜 원장으로 지급만 한 번 막는다.
enum DungeonDailyReward {
    static let starPieces = 5_000
    static let eggs = 1
}

/// 웨이브 런의 실적. 강화·파티·잡은 포켓몬은 런과 함께 사라지고, 최고 기록과 클리어 횟수만 남는다.
/// 경제 보상은 이 값과 분리된 일일 원장(`waveRunEggRewardDate`)에서 멱등하게 정산한다.
