...
///
/// 왜 접는가: 한 공격은 이벤트 여러 개(`.move` → `.crit` → `.damage`)를 남기고, 팝오버 로그에는
/// 최근 몇 줄만 보인다. 이벤트마다 한 줄을 쓰면 한 턴이 로그를 다 차지한다. 스트림 자체는 그대로
/// 남아 있으므로 재생 애니메이션(Phase 7)은 접기 전 이벤트를 하나씩 읽는다.
///
/// 왜 뷰가 아니라 여기인가: 예전엔 이 판단이 `BattleView.eventLine` 의 if/else 사슬이었다.
/// 뷰 안에 있으면 문구가 맞는지 테스트할 방법이 없고, 1v1 과 멀티가 각자 다른 문구를 만들었다.
enum BattleLog {
