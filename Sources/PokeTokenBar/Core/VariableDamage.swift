...
/// 레벨업으로 배워진다). 사용자가 직접 배우는 경로(레벨업 습득창·하트비늘·`canonicalLevelUpMoves`)
/// 에는 필터가 없어 그 죽은 기술이 그대로 실렸고, 자동 무브셋은 반대로 `power > 0` 으로 걸러
...
enum VariableDamage: Equatable, Sendable {
    ...
    /// - 토해내기: 비축하기 카운터가 필요하다(회복 이벤트는 Phase 5 에서 생겼다 — 남은 건 카운터다).
    /// - 동료(집단구타): 대전이 파티를 안 본다. 1대1 에는 파티 자체가 없다(스냅샷 하나).
    ///   다단 히트는 Phase 5 에서 생겼으므로 남은 건 파티뿐이다.
    /// - 선물: 위력이 확률로 갈리는데(40/80/120/회복) 그 분포가 엔진에 없다. 회복 자체는
    ///   `BattleEvent.heal` 로 표현할 수 있게 됐다(Phase 5).
    /// - 섀도하프: 본편 밖 기술이다.
    ...
    /// `MoveSpec.power` 가 0 이고 위력은 `from` 이 뽑아 준다. 그래서 `power > 0` 으로 가르면
    /// 이 부류가 공격기 칸에도(위력 0) 변화기 칸에도(`hasModeledStatusEffect` 가 false) 못 들어가
    /// **자동 무브셋에서 영원히 안 뽑혔다**. 반면 `isUsable` 을 보는 사용자 습득 경로
    /// (`canonicalLevelUpMoves`·하트비늘)는 그대로 통과시켰다 — "같은 기준" 이라던 두 게이트가
    /// 기술 한 부류를 두고 갈라져 있었다. `isUsable` 을 그대로 재사용해 갈라질 자리를 없앤다.
