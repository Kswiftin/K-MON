...
- **`case` 블록 안의 지역 `let` 은 블록이 끝나면 ARC 가 바로 푼다.** 델리게이트를 가진 객체를
  지역 변수에 만들고 `_ = 객체` 로 붙잡은 척하면, `_ = x` 는 마지막 사용일 뿐 수명을 늘리지
  않아 `RunLoop.main.run()` 에 들어갈 때 이미 해제돼 있다. 시스템 프레임워크 매니저가 함께
  죽으면 결과는 **에러도 로그도 없는 무동작**이다 — 콜백이 한 번도 안 오고, 상태 타이머가
  `[weak self]` 면 첫 발화에서 스스로 `invalidate()` 해 종료 조건까지 사라진다.
- **증상이 권한 문제로 위장한다.** 출력이 아예 없으면 먼저 권한을 의심하게 되는데, 권한 거부는
  대개 상태 콜백이 **찍히기라도 한다**. 한 줄도 없으면 권한이 아니라 객체가 살아 있지 않은 것이다.
- **왜 못 걸렀나**: 런루프 CLI 도구는 단위 테스트가 없고, 컴파일과 리뷰만 통과하면 "동작한다" 로
  보인다. 실제로 한 번 돌려보지 않으면 드러나지 않는다.
...
- **실패를 예외가 아니라 범위 안의 특정 값으로 알리는 API 가 있다.** 그 값을 검사 없이 기록하고
  다시 검사 없이 평균에 넣으면 통계가 통째로 망가진다. 실제로 60초 측정에서 **표본 70개 중 2개**가
  섞여 평균이 -42.7 에서 -37.9 로, 표준편차가 **4 에서 29 로** 튀었다.
- **틀린 값이 그럴듯해서 더 나쁘다.** 결과가 숫자로 멀쩡히 나오므로 오류로 안 보인다. 그 표준편차가
  기능 존폐를 가르는 기준에 그대로 들어가, 눈치 못 챘으면 **판정이 센티넬 2개로 갈릴** 뻔했다.
  관측이 아니라 "관측 실패" 를 관측으로 센 것이다.
- **왜 못 걸렀나**: 값 하나하나는 유효 범위 안처럼 보여(정수·부호 있는 물리량) 형식 검증에 안 걸리고,
  사람이 보는 출력이 평균 한 줄이라 원시 분포를 안 본다. 히스토그램으로 펼치고 나서야 드러났다.
- **처방**: 센티넬은 **생산·소비 양쪽에서 같은 규칙으로** 버린다. 기록하는 쪽이 쓰기 전에 버리고,
  읽는 쪽도 읽을 때 버린다 — 옛 형식으로 찍어 둔 파일이 남아 있어 한쪽만 고치면 조용히 어긋난다.
  버린 개수는 양쪽 다 출력에 드러낸다 — 소리 없이 버리면 장비 고장과 정상 측정을 구분할 수 없다.
- **같은 부류를 의심할 자리**: 계측 API 가 실패를 **범위 안의 값**으로 알리는 모든 곳(신호세기
  `127`, 배터리 `-1`, 좌표 정확도 음수 등). (2026-09-11.)
