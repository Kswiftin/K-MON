...
- **직접원인**: `test-gate.sh` 의 warning 검사는 `swift test` **로그에서 경고를 긁는다.**
  warm build 는 재컴파일하지 않으므로 경고를 다시 찍지 않는다 → 0건으로 읽힌다.
  게이트는 이 한계를 알고 있었고 "신뢰 기준은 매번 cold build 인 CI 다" 라고 적어 두었는데,
  **`ci.yml` 이 게이트 바로 앞에서 `swift build` 를 돌려** 그 전제를 같은 잡 안에서 깼다.
  그 단계는 79초를 아끼려고 일부러 게이트와 플래그를 맞춰 둔 것이었다 — 아낀 것이 검사였다.
- **테스트/리뷰가 왜 못 걸렀나**: 검사가 "경고가 없다" 와 "경고를 볼 수 없었다" 를 구분하지
  않는다. 둘 다 빈 문자열이라 초록이 나온다. 로컬에서도 같은 이유로 안 보였다.
- **부류**: **로그를 증거로 쓰는 검사 전부.** 그 로그를 만드는 단계가 캐시·증분·건너뛰기로
  비어 있을 수 있으면, 검사는 "위반 없음" 이 아니라 "관측 없음" 을 통과로 읽는다.
  (같은 모양: 커버리지 `^0` 을 안 보고 라인 %만 보는 것, 소스 스캔 가드가 주석을 세는 것.)
- **처방**: 검사가 의존하는 조건을 **같은 파이프라인이 깨지 않게** 한다 — 게이트 앞의 빌드
  단계를 뺐다(게이트가 어차피 컴파일하므로 오류 신호도 잃지 않고, 러너 시간은 오히려 줄었다).
  그 자리에 "다시 두면 이 검사가 죽는다" 를 적어 둔다. 근본적으로는 관측 없음을 통과로 읽지
  않는 것이 답이다 — 로그에 컴파일 흔적이 없으면 검사를 통과시키지 말아야 한다.
  (`ci.yml`·`scripts/test-gate.sh`, 2026-09-07.)
