...
## 선행 입력을 안쪽 화면으로 옮기면, 바깥쪽의 **즉시 실행 버튼**도 함께 옮긴다

- **증상**: 친구 목록에서 `배틀 신청`을 눌러도 화면도 신청도 아무 변화가 없었다.
- **직접 원인**: 후보 6마리 선택기를 랭크배틀 화면으로 옮겼지만 친구 행은 여전히 곧바로
  `challenge(peer)`를 호출했다. 후보가 비어 신청이 거절됐고, 배틀 화면 라우팅은
  `phase != .ready`만 보므로 실패 상태(`ready`)에서는 편성 화면도 열지 않았다.
- **왜 테스트가 못 잡았나**: 기존 회귀 테스트는 선택기가 친구 관문에서 제거됐는지만 검사했다.
  제거 후 그 값을 필요로 하는 실행 버튼까지 도달 가능한지는 검사하지 않았다.
- **처방**: 친구 행의 버튼은 배틀 화면으로만 이동하고, 후보 편성 뒤 그 화면의 상대 행이 신청을
  보낸다. 회귀 테스트는 라우팅의 양쪽을 함께 고정한다 — `ready` 상태에서도 배틀 화면이 열리고,
  바깥 관문에는 조기 `challenge(peer)` 호출이 없어야 한다. (`FriendView`, 2026-08-31.)
