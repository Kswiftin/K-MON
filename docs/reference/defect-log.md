...
- **`.help()` 툴팁은 유일한 표시 경로가 될 수 없다 — NSPopover 안에서는 아무것도 안 뜬다.** SwiftUI 의
  ...
  판별법은 렌더 후 `NSView.trackingAreas` 를 세는 것 — "툴팁 문자열이 맞게 조립되는가" 를 검증하는 테스트는
  ...
  *덤*으로만 단다. 회귀 가드는 대조군(`.help()` 만 단 뷰의 트래킹 0개)을 같은 테스트에 넣어 어서션이
  ...
  부류 스윕(2026-08-19): 남은 `.help()` 는 전부 화면에 라벨이 이미 있거나(`Label` 텍스트·링크 제목·필터 안내)
  관례적 아이콘(gear·power)이라 정보 손실이 없다 — 정보가 툴팁에만 있던 곳은 기술 설명 하나였다.
- **호버로 채우는 슬롯은 높이를 고정한다.** 설명 길이를 따라 늘어나면 행 사이로 마우스를 옮길 때마다
  아래 콘텐츠가 밀려 팝오버 안이 떨린다. 높이는 숫자 상수가 아니라 *같은 폰트의 더미 2줄* 에서 유도한다
  (자리표시자 행과 같은 이유 — 상수는 폰트·OS 따라 어긋난다).
