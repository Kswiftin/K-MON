...
- **`?? 기본값`·`max(1, …)` 은 "값이 없다"를 "값이 이거다"로 바꿔 화면에 거짓을 만든다 — 그리고 라인
  커버리지에 잡히지 않는다.** 디코딩이 아니라 *표시·계산* 에서 터지는 부류다.
  - 로그를 접을 때 `damage ?? 0` 을 쓰면 데미지 이벤트가 없는 행동(빗나감·변화기)이 **"0 데미지"** 로
    렌더된다. 옵셔널은 `guard let` 으로 갈라 "숫자가 없는 문구"를 따로 둔다(`BattleLog.Action.text`).
  - 데미지 하한 `max(1, damage)` 은 **위력 0 인 변화기까지** 1 이상으로 올린다 — Gen 2 식의 `+2` 가
    살아남아 상태기가 2 데미지를 넣었다. 하한은 "때린 경우"에만 적용한다(`resolveAttack` 의 `move.power > 0`).
    `moveSet` 은 `power > 0` 으로 걸러도 `canonicalLevelUpMoves`(→ `learnedMoves`)는 안 거른다 —
    **한쪽 경로만 필터가 있으면 필터는 없는 것과 같다.**
  - 커버리지가 못 잡는 이유: `?? 0`·`max(1, …)` 은 한 줄 안의 region 이라 왼쪽만 밟혀도 라인은 초록이다.
    `BattleLog.swift` 는 라인 91% 로 게이트를 통과한 상태에서 `.resisted` 분기가 **0회**였다.
    `llvm-cov --show-regions | grep '\^0'` 으로 직접 본다. **새 enum case 는 만드는 쪽(엔진)만
    테스트하면 렌더러 쪽이 0회로 남는다**(#51).
...
- **처방 두 가지.** ① 고정 크기 컨테이너는 남는 자리를 어디로 보낼지 정한다(`alignment: .top`) —
  ...
  `NSView.cacheDisplay(in:to:)` 로 창을 띄우지 않고 되며, `KMON_SNAPSHOT_DIR` 이 있으면 PNG 로
