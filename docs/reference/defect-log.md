...
- **"상한까지만 그리고 남은 수를 문구로 알린다"도 이 부류의 처방이 아니다.** 중첩 `ScrollView` 를
  ...
  방법은 여전히 없었다. 높이를 260 에서 520 으로 늘렸던 것과 정확히 같은 실수다 — 잘리는 지점을
  옮기거나 잘렸다고 알려 줄 뿐 도달성은 그대로다. **잘림을 사용자에게 고지하는 것과 도달 가능하게
  만드는 것은 다른 일이다.** 게다가 그 문구는 "가까운 순으로 표시됩니다"라고 말했지만 `BattleNet` 은
  이름순 정렬이고 Bonjour 에 거리 정보 자체가 없다 — 잘림을 변명하려다 거짓 문구까지 붙었다.
  처방은 이 부류의 원래 처방 그대로 페이지식이고, 검증도 원래 기준 그대로
  "마지막 하나에 도달할 수 있나"다(`testEveryNearbyTrainerLandsOnSomePage`).
  같은 부류 스윕에서 포켓슬론 릴레이 방 목록도 상한도 페이저도 없이 그리고 있어 함께 고쳤다.
