...
- **광고된 업적 단계를 내 카탈로그 상한으로 클램프하고 내 분모로 그렸다.** 카탈로그는 조절
  손잡이라(`AchievementLadder.catalog`) 언젠가 늘어난다. 그 뒤 18/20 인 상대가 구버전 화면에서
  `16/16`(완료)으로 보인다 — 진행도를 보여주는 기능이 조용히 거짓을 말하고, 클램프가 그걸 가린다.
- **처방**: 분자와 함께 **분모도 광고**한다(`achievementCeiling`). 읽는 쪽은 상대의 분모로 그리고,
  분모를 안 보낸 구버전만 내 카탈로그로 폴백한다. 분모도 상대가 신고하므로 표시 상한
  (`PeerAdvertisement.maximumTierCeiling`)으로 자른다 — 자릿수가 늘면 카드가 밀려 잘린다.
- **타이밍이 본질이다.** 이 키는 카탈로그가 바뀌기 **전에** 배포돼 있어야 효과가 있다. 나중에
  넣으면 그 사이 버전들은 이미 틀리게 그린다 — "지금 안 해도 되는 일"이 아니다.
- 회귀: 새 분모(18/20)·분모 없음(내 상한)·적대적 분모(클램프)·분자 없음(분수 자체 없음)을 각각
  밟는다. 내 카탈로그로 클램프하는 옛 동작을 주입해 실패를 확인했다.
  (`PeerAdvertisement.swift`, 2026-08-22.)

## 네트워크 객체를 취소하지 않고 참조만 버리는 부류

- **`BattleNet` 의 리스너·브라우저 재시작 경로가 `cancel()` 없이 `= nil` 만 했다.** 실패한
  `NWListener`/`NWBrowser` 는 취소해야 큐·포트를 놓는다 — 24/7 메뉴바 앱에서 슬립 복귀마다 누적된다.
  형제인 `MultiplayerRoomCenter`(`leaveRoom` · `restartBrowser`)와 `LocalAppcastServer` 는 **취소한다**
  → "같은 기전을 한 모드에서만 고치는 부류" 의 재발.
- **처방**: 호출부가 아니라 **`startListener`/`startBrowser` 입구**에서 취소한다. 재시작은 반드시
  이 입구를 지나므로 새 재시작 경로가 생겨도 자동으로 덮인다(호출부에 흩뿌리면 한 곳이 빠진다).
- **테스트 없음(한계)**: 실제 실패 상태를 만들려면 실 네트워크가 필요하고, 테스트 바이너리는 로컬
  네트워크 권한이 없어 리스너가 `.ready` 조차 못 간다. 근거는 단일 입구라는 **구조**다.
  같은 이유로 `BattleCenter` 에 정지 API 는 두지 않았다 — 프로덕션 호출부가 없고(LAN 토글 없음)
  테스트 편의로 프로덕션 API 를 만들면 그 API 가 무검증으로 남는다.
