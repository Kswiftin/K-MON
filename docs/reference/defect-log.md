...
- **"다시 받아야 하는가" 같은 수렴 판정에 축을 더하면, 기존 테스트가 새 축 덕분에 통과하거나 새 축
  때문에 실패한다.** `needsDescriptionRefresh`(설명 축)에 랭크 변화 축을 더해 `needsDetailRefresh` 로
  넓히자, "받았지만 빈 설명은 다시 받지 않는다" 를 지키던 테스트가 **랭크 축이 nil 이라** 실패했다.
  반대 방향이 더 위험하다 — 새 축이 늘 참이면 설명 판정이 죽어도 테스트는 초록이다.
...
- **`switch` 를 도우미로 만들면 그중 실제로 쓰이는 case 는 일부다.** 랭크 적용을 위해 만든
  `BattleSide.rawStat` 은 7개 case 인데 데미지 경로가 쓰는 건 4개였고, 스피드·명중·회피 case 는
  아무 호출부도 없었다. 파일 라인 커버리지는 99.44% 였고 게이트도 통과했다 — `--show-regions` 의
  미실행 표시로만 드러났다.
...
- **같은 감사에서 확률 분기의 실패 쪽도 미실행이었다.** 2차효과 랭크 변화를 100%·0% 로만 테스트해서
  "확률 판정이 실패하는" 경로를 한 번도 밟지 않았다 — 굴림을 아예 안 하는 구현도 통과한다.
