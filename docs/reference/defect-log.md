...
- **네트워크 끊김을 승리로 접으면 양쪽이 이긴다 — 그리고 그게 화폐 발행이다.** 두 피어는 각자 *자기*
  연결의 죽음을 보므로 `iWon: true` 로 접으면 라우터가 한 번 껄떡일 때 둘 다 몰수승을 선언하고 둘 다
  판돈을 정산받는다(별의조각이 무에서 생긴다). 게다가 지고 있을 때 네트워크를 끊는 게 최적해가 된다.
