...
- **`?? 기본값` 은 "값이 없다"를 "값이 이거다"로 바꿔 화면에 거짓을 만든다 — 그리고 라인 커버리지에
  ...
  **커버리지가 이걸 못 잡는 이유**: `?? 0` 은 한 줄 안의 region 이라 그 줄이 실행되면 라인은 세어지고,
  왼쪽만 밟혀도 초록이다. 새로 넣은 `BattleLog.swift` 는 라인 91%·게이트 통과 상태에서 `.resisted`
  분기가 **0회**였고 `?? 0` 두 곳이 한 번도 nil 을 안 밟았다 — `llvm-cov --show-regions | grep '\^0'`
  으로 직접 봐야 드러난다. **새 enum case 는 만드는 쪽(엔진)만 테스트하면 렌더러 쪽이 0회로 남는다**:
  엔진 테스트가 `.resisted` 를 검증해도 접기 테스트가 그 case 를 먹이지 않으면 렌더 분기는 죽어 있다.
...
- **뷰가 문구를 *만들면* 그 문구는 테스트 밖에 있다.** 배틀 로그의 3언어 문장은 `BattleView.eventLine`
  ...
  오구현을 잡을 방법이 없었고, 1v1 과 4인 방이 같은 사건에 서로 다른 문구를 만들고 있었다(멀티는
  기술 이름도 급소도 안 나왔다). **결정은 순수 함수로 내리고 뷰는 그 결과만 그린다**(`BattleLog.lines`
  → `[Line]`, 뷰는 `Text(line.text)`). 그러면 문구·언어·접기 규칙이 단위 테스트에 들어온다.
