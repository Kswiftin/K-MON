...
- **처방**: 판정을 사람 기억이 아니라 **종료코드**로 남긴다 — 당시 `verify-opencode-isolation.sh` 가
  `0`(4/4 통과)·`1`(프로브 실패)·`2`(시험 불가)를 구분했다. (스크립트와 `opencode-isolation.md` 는
  대화 기능과 함께 삭제됐다 — 2026-08-25. 남길 것은 파일이 아니라 **"미확인"과 "실패"를 다른
  종료코드로 가른다**는 규칙이다. 외부 CLI 계약을 다시 다룰 때 같은 형태로 복원한다.)
