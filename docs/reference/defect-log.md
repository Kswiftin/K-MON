...
read_when:
  ...
  - 외부 데이터 소스가 새 세대·새 범위를 **어디까지 담고 있는지** 전제할 때
...
- **"상태가 같으니 결론도 반대"는 턴 경계에서만 참이다(2026-09-05 해결).** 1v1 끊김 판정은 두 피어가
  같은 배틀 상태를 본다는 전제로 남은 HP 비율을 비교했다. 그런데 `resolveIfReady` 는 **두 선택이 모이는
  즉시** 각자 해상하므로 한쪽 `.move` 만 도착한 채 링크가 죽으면 상태가 한 턴 어긋나고, 그 창에서는
  양쪽이 모두 "내가 앞선다"를 볼 수 있었다 → 판돈이 두 지갑에 동시에 들어간다(막으려던 그 결함의 좁은
  재발). 결정론적 상태를 판정 근거로 쓰려면 **어느 시점의 상태인지도 합의돼야 한다.**
  처방은 `BattleEngine.AgreedTurnLedger` — 행동마다 자기 해상 턴(`resolvedThrough`)을 실어 보내고,
  판정은 `min(내 해상 턴, 상대가 알린 해상 턴)` 의 상태로 한다. **두 피어가 같은 쌍의 min 에 도달하는
  것**이 결론이 갈리지 않는 근거이고, 합의된 턴이 없으면 판정 대신 환급이다(`rulesVersion` 21).
  - **"판정을 아예 없애고 늘 환급" 은 오답이다.** 이중 지급은 사라지지만 위의 무손실 탈주가 되살아난다.
    화폐를 지키려다 실력 지표를 여는 교환이라, 한 결함의 처방이 다른 결함의 재발이 되는지 먼저 본다.
  - **왜 테스트가 못 걸렀나 — 한 피어 관점만 봤다.** 각 피어는 자기 화면에서 늘 일관돼 보이고, 어긋남은
    두 관점을 **나란히 놓아야** 드러난다. 회귀 테스트는 A·B 상태를 각각 만들어 둘의 결론이 반대인지
    본다(`testJudgingOnTheAgreedTurnMakesExactlyOnePeerWin`).
  - **순수 함수만 테스트하면 프로덕션이 다른 상태를 넘겨도 초록이다.** `AgreedTurnLedger` 만 검증하면
    `connectionDropped` 이 지금 상태로 되돌아가도 아무 테스트가 안 깨진다 — 실제로 처음엔 그랬다.
    `dropConnectionForTesting` 으로 **프로덕션 경로를 밟고**, 지금 상태와 합의 상태가 **반대 결론을
    내는** 픽스처를 쓴다(둘이 같은 값이면 근거를 되돌려도 초록이다).
    (`BattleDisconnectAgreementTests`, 2026-09-05.)
...
- **재발 (2026-09-03): 이미 배포된 필드 `lastCandyDate` 를 서명에 넣고 버전을 안 올려 실사용
  세이브가 전멸했다.** #206 이 일일 사탕의 멱등 키를 canonical 에 추가하며 "새 필드라 조건부 append
  로 충분하다" 고 적었는데, `lastCandyDate` 는 2026-08-13 부터 있던 필드다. 사탕을 한 번이라도 받은
  세이브는 값이 차 있어 세그먼트가 붙고, 구서명이 재현되지 않아 **로드마다 조작 판정 → 전체 초기화**.
  실제 피해: 도감 105·박스 141·별의조각 138,679 이 한 번에 사라졌다(초기화 직전 백업
  `companion-state.pre-reset-*.json` 은 남아 복구했다).
  - **판별법**: 백업 파일을 **구규칙**(문제 세그먼트 제외)으로 재해싱해 저장된 서명과 맞으면 세이브는
    멀쩡하고 규칙 변경이 범인이다. 이번엔 정확히 일치했다.
  - **왜 못 걸렀나**: 세 가드가 전부 기본값만 봤다 — 기본값 canonical 동결(빈 문자열이라 세그먼트
    없음), 접두 동결(fixture 가 `lastCandyDate` 를 안 켬), 신규 전용 테스트(`기본값에 dcd 없음`).
    "값이 든 실사용 세이브" 를 밟는 테스트가 하나도 없었다.
  - **캡처**: `integrityVersion` 9 → 10, fixture·접두 집합에 `dcd` 추가, 접두 동결 실패 메시지에
    "이 필드가 이전 배포에 있었나" 질문 추가, 그리고 **구서명 세이브가 면제되는지**를 직접 밟는
    회귀 테스트(`CandyLedgerSignatureTests.aSaveSignedBeforeTheCandySegmentIsExempt` — 버전을 되내려
    실패하는 것까지 확인).
...
- **찾는 방법**: `canonicalString` 의 `if s.<field>` 목록과 `fullyPopulatedState()` 가 대입하는
  필드 목록을 대조한다. 지금은 `gd`·`rd`·`dcd` 를 메워 일치한다.
  (LAN 협동 레이드 #80 의 `raidRewardDate` 를 넣다가 발견, 2026-09-02.)
- **재발했다 — 하루 뒤 같은 구멍으로 사용자 진행이 전멸했다.** #206 이 일일 사탕 원장(`dcd`)을
  canonical 에 넣으면서 `fullyPopulatedState()` 에 `lastCandyDate` 를 안 켰다. 접두 동결도, 기본값
  동결도(그 필드 기본값이 빈 문자열이라) 초록이었고, 새로 쓴 전용 테스트마저 **기본값 세이브에
  세그먼트가 안 붙는지**만 봤다. 세 가드가 전부 "기본값" 만 보고 있어 실사용 세이브를 아무도 안 봤다.
  위 부류 규칙("한 커밋에 셋을 함께")을 문서로만 두면 다음 사람이 못 읽는다 — 그래서 실패 메시지에
  **"그 필드가 이전 배포에 이미 있었나?"** 질문을 넣었다. 결과는 아래 「이미 배포된 필드…」 항목.
  (2026-09-03.)
...
- **증상**: 협동 레이드 화면만 팝오버가 작게 떴고, 안쪽 `ScrollView` 가 남은 공간만 받아 로그와
  채팅이 잘렸다. 형제 오버레이 다섯(설정·체육관·던전·꾸미기·대화)은 전부 높이를 고정하고 있었고
  **새로 들어온 하나만 빠졌다.**
- **원인**: 오버레이는 팝오버 본체(`mainContent`)의 `.frame(height:)` **밖에** 놓인다. 본체만
  높이를 받는 구조라, 오버레이가 스스로 정하지 않으면 `NSPopover` 가 콘텐츠 자연 높이로 창을 잡는다.
- **왜 테스트가 못 걸렀나**: 폭 예산 테스트는 있었지만(`PopoverLayoutTests`) 전부 **명시적으로
  제안한 폭·높이**로 재고 있었다. 창이 실제로 얼마를 주는가는 그 harness 밖의 일이다.
  빌드도 통과하고 화면도 그려진다 — 작게 그려질 뿐이다.
- **영구 캡처**: `PopoverLayoutTests.testEveryPopoverOverlayPinsItsOwnHeight` — `PopoverView` 의
  오버레이 체인에 있는 파일마다 `.frame(height:` 를 요구한다. **값은 강제하지 않는다**(설정은 460,
  나머지는 `currentHeight(for: .battle)` 로 서로 다르고 그건 각 화면의 판단이다) — 막는 건
  "아예 안 정하는 것" 이다. 대조군으로 목록의 이름들이 실제 체인에 있는지 먼저 확인한다.
  가드는 프레임을 지워 보고 실패하는 것을 확인했다.
  (레이드 화면 #80, 2026-09-02.)

## 리베이스가 만드는 부류 — 상류의 새 경계가 내 브랜치의 전제를 자른다

- **증상**: 레이드 브랜치를 `origin/main` 에 리베이스하니 보스가 와이어를 건너오며 HP 2,800 →
  142 로 접혔다. 상류(#208)가 디코딩 경계에 `hp = min(stats.hp, ...)` 를 넣었고, 레이드 보스는
  **의도적으로 종족값 최대치를 넘는** 유일한 전투원이다.
- **텍스트 병합은 이걸 못 잡는다.** 두 변경이 같은 줄을 건드리지 않아 충돌조차 나지 않았다.
  빌드도 통과했다. 잡은 것은 **와이어 왕복 테스트** 하나뿐이다 — 리베이스 뒤에 `swift build` 만
  보고 넘어갔으면 재생이 붙은 보스가 첫 라운드에 즉사하는 채로 PR 이 올라갔다.
- **부류 규칙**: 리베이스·머지 뒤에는 **빌드가 아니라 전체 게이트**를 돌린다. 특히 상류가
  "경계에서 값을 자른다" 류의 변경을 했으면, 내 브랜치에 그 경계를 일부러 넘는 값이 있는지 본다.
- **해소는 예외가 아니라 천장 교체다.** 상류 클램프의 목적은 재생 중 `hp + amount` 오버플로 방지다
  (그리고 레이드는 그 재생을 쓰는 쪽이라 목적이 더 강하게 적용된다). 그래서 보스만 상한을
  `RaidTier.maxBossHP` 로 바꾸고 **상한 자체는 남겼다** — 목적은 상한이 무엇이든 지켜진다.
  예외를 고정 id 하나로 좁히고, "정말 오늘의 그 보스인가" 는 `RaidBoss.validBoss` 가 등호로 본다.
  대조군(`testTheHPCeilingStillClampsEveryoneElse`)이 다른 전투원의 경계가 그대로임을 잠근다.
  (레이드 #80 × 스냅샷 클램프 #208 리베이스, 2026-09-02.)

## 테스트가 프로덕션 경로 대신 자기 디버그 훅의 사본을 밟는 부류

- **증상**: 레이드 재생을 붙이며 `combatEvents` 를 덮어쓰기(`=`)에서 이어 붙이기(`+=`)로 바꿨다.
  그 사실을 검증하는 테스트를 쓰고, 확인차 프로덕션 경로를 `=` 로 **되돌려 봤더니 초록이었다.**
- **원인**: 소켓 없이 라운드를 밀어 넣으려고 만든 `debugApplyResolvedRound` 가 같은 로직을
  **복사해** 들고 있었다. 테스트는 그 사본을 밟았고, 게스트 수신 경로는 한 번도 안 지났다.
- **부류 규칙**: 테스트용 훅은 **자기 구현을 갖지 않는다.** 프로덕션 함수를 부르는 얇은 껍질이어야
  한다. 껍질이 아니면 그 훅은 테스트 대상이 아니라 **두 번째 구현**이고, 둘이 갈라지는 순간
  테스트는 아무것도 지키지 않는다. 지금은 호스트·게스트·훅이 전부
  `MultiplayerRoomCenter.applyResolvedRound` 하나를 지난다.
- **찾는 방법**: `debug` 접두 메서드의 본문이 한 줄(위임)보다 길면 의심한다.
  기존 `debugApplyGymState`·`debugApplyGymRejection` 은 단순 대입이라 해당하지 않는다.
- **일반화**: 이건 CLAUDE.md 5-whys 의 "테스트가 결함 트리거와 다른 경로로 통과" 의 구체형이다.
  **가드를 새로 쓸 때 결함을 주입해 보라는 규칙이 이 부류를 잡는 유일한 그물이었다** — 주입 없이는
  8개 테스트가 전부 초록인 채로 재생이 죽은 기능이 나갈 뻔했다.
  (레이드 배틀 UX #80, 2026-09-02.)

## 배경 그라데이션을 `ZStack` 층으로 두면 칸이 제안된 높이를 통째로 가져가는 부류

- **증상**: 레이드 아레나의 필드가 `NSHostingController.sizeThatFits` 에서
  `greatestFiniteMagnitude` 를 요구했다 — 팝오버 안에서라면 아래 기술 버튼과 로그가 통째로 밀린다.
- **원인**: `LinearGradient` 는 고유 크기가 없어 **제안된 크기를 그대로 채운다.** `ZStack` 의 한 층으로
  두면 그 층이 스택 크기를 정해 버린다. `WaveRunArenaView` 는 `.frame(height:)` 로 막고 있었는데,
  그건 그 화면이 2대2 고정이라 가능했다 — 파티가 한 줄·두 줄로 바뀌는 화면에선 못 쓴다.
- **부류 규칙**: 크기를 정하는 것은 **내용뿐**이어야 한다. 배경은 `.background(_:in:)`, 연출·팝업은
  `.overlay` 로 붙인다. 고유 크기가 없는 것(그라데이션·`Color`·`Rectangle`·`GeometryReader`)을
  `ZStack` 층에 두면 그 층이 크기의 주인이 된다.
- **왜 눈으로는 못 잡나**: 스크롤 컨테이너 안에서는 무한 높이가 "그냥 여백이 넓네" 로 보인다.
  **기계로 잡는 형태가 `sizeThatFits` 높이 단언이다**(`PopoverLayoutTests` 의 대조군 —
  파티 넷이 하나보다 높아야 한다). 결함을 주입해 이 가드가 실제로 빨개지는 것을 확인했다.
  (레이드 배틀 UX #80, 2026-09-02.)
...
## 여러 생산자가 쓰는 알림·이벤트를 **한 목적지로만** 보내는 부류

- **"지금까지 다 같은 종류였다" 는 설계가 아니라 우연이다.** 알림 탭 처리(`didReceive`)가 종류를
  안 보고 전부 `goToBattle()` 로 보내고 있었다. 주석도 "배틀 신청이면 수락 화면으로" 라고 **전제를
  적어** 두었는데, 그때 알림이 죄다 배틀·레이드 계열이라 맞아떨어졌을 뿐이다. 전제를 적어 둔 것과
  전제를 지키는 것은 다르다 — 적어 둔 전제를 깨는 생산자가 들어와도 아무 테스트가 안 깨진다.
- **깨지는 순간은 새 생산자가 "다른 곳을 가리키는" 알림을 낼 때다.** 집중 체인 알림은 *누르면
  시작하라*는 뜻인데 대전 탭엔 시작 버튼이 없다. 알림을 눌러 앱을 연 사용자가 정작 알림이 시킨
  일을 못 한다 — 앱은 아무 오류도 안 내고, 화면은 멀쩡하다.
- **처방은 목적지를 순수 함수로 빼고 식별자 접두사를 단일 상수로 공유하는 것.**
  `PopoverNavigation.destination(forNotificationID:)` 가 판정하고, 띄우는 쪽과 라우팅하는 쪽이
  `FocusChainRules.notificationIDPrefix` 하나를 본다. 문자열을 두 번 적으면 한쪽만 고쳐졌을 때
  알림이 엉뚱한 화면을 연다. `didReceive` 자체는 `@main` 이라 테스트가 안 닿으므로, **판정을 밖으로
  빼는 것이 곧 검증 가능해지는 것**이다.
- **탐지 질문 하나.** 이벤트 진입점이 `switch` 없이 한 곳으로만 보내고 있으면, 지금 생산자를 전부
  세어 보고 "종류가 늘면 여기가 갈려야 하는가" 를 답한다. 답이 예면 그 자리가 이미 결함이다.
  (집중 체인 알림, 2026-09-05.)

## 판정표는 하나인데 그 **입력을 조립하는 코드**가 프런트엔드마다 복사돼 있는 부류

- **표를 한 벌로 모아 놓고 안심하면, 갈라지는 곳은 표가 아니라 표에 넣는 값이다.**
  `PokedoroSessionGate` 는 헤더 주석까지 써 가며 "프런트엔드마다 조건을 다시 쓰면 한쪽만 고쳐도
  아무 테스트가 안 깨진다" 를 막아 두었는데, 정작 그 입력인 `PokedoroSessionState` 조립은
  대화 도구와 터미널 실행기에 **주석까지 똑같이** 복사돼 있었다. `isRunning` 을 접는 방식이 한쪽만
  바뀌는 순간 "대화로는 거절되는데 터미널로는 통과하는" 상태가 되고, 두 사본이 같은지 확인할
  방법은 손으로 맞대 보는 것뿐이다.
- **세 번째 사본을 쓰기 직전이 가장 싼 시점이다.** 세션 체인이 같은 조립을 또 필요로 했다.
  거기서 복사했으면 사본이 셋이 되고, 셋은 둘보다 훨씬 오래 산다 — 늘리기 전에 하나로 합친다
  (`PokedoroSessionState.init(timer:companion:)`).
- **탐지는 grep 한 줄이다.** 메모버라이즈 이니셜라이저를 직접 부르는 자리를 세면 된다
  (`grep -rn 'PokedoroSessionState(phase:' Sources/` → 0이어야 한다). 순수 판정 타입을 새로 만들 때
  같은 계산을 한 번 해 둔다. (Pokedoro 세션 체인, 2026-09-05.)
...
- **처방 두 겹.** ① 칸이 자기가 그리는 줄 수를 담는지 **산수로** 검증한다(1줄 실측 높이 × 줄 수 +
  ...
  (배틀 로그 칸, 2026-08-20.)
- **같은 부류가 포켓몬 탭 격자에서도 났다.** `PokemonRosterView` 는 `contentHeight` 를 520 으로
  고정해 두고 그 안에서 5행짜리 격자에 예산을 나눠 준다. 여기에 `professorToolbar` 첫 줄(18pt +
  간격 6pt)을 추가하면서 `contentHeight` 를 그만큼 늘리지 않아, 격자 몫이 좁아진 채로 마지막 행이
  찌그러지거나 스크롤 끝에서 잘려 보였다(사용자가 스크린샷으로 직접 보고, 2026-09-22). "260 → 520"
  으로 한 번 늘렸던 값이 새 줄이 생길 때마다 다시 좁아질 수 있는 자리라는 뜻이다.
  **처방**: `contentHeight` 를 새 줄의 실제 예산(높이 + 그 줄이 만드는 `VStack` 간격 한 칸)만큼
  올리고, 그 관계를 산수 테스트로 고정한다(`testRosterContentHeightAccountsForProfessorToolbar`).
  고정 높이 프레임에 **첫 줄**을 추가하는 변경은 항상 이 예산 재계산이 따라와야 한다.
...
- **재발 (2026-09-09, 포코피아 주민).** 받는 자리(`admitTownResident`)와 거르는 자리
  (`PokopiaTown.normalized`)가 조건을 **각자** 적었고, 스프라이트 검사가 받는 자리에만 빠졌다.
  오늘의 유일한 호출자가 미리 걸러서 안 밟혔는데, 그 호출자의 주석이 이 가드의 존재 이유를
  "나중에 터미널·전송이 부를 수 있다" 로 적어 두었다 — 정확히 거르지 않은 값을 넘길 호출자다.
  `normalizeMemoryHomeAccess()` 가 `save()` 에 없어 그 주민은 디스크에 남고 **다음 실행에서
  소리 없이 사라진다**. **테스트가 못 걸른 이유**: `testMalformedResidentsAreRefused` 가 `guard`
  의 조건을 그대로 베껴 세서, `guard` 에 빠진 조건은 테스트에도 빠진 채 초록이었다.
- **처방**: 주민 하나의 술어를 `PokopiaTown.isAdmissible(_:)` 하나로 뽑아 두 자리가 부른다.
  그리고 두 검증기에 **같은 입력을 넣어 답이 같은지 맞대는** 패리티 테스트
  (`testAdmitAndNormalizeAgreeOnEveryMalformedResident`)를 둔다 — 각 자리를 따로 테스트하면
  갈라짐 자체는 영영 안 보인다. 단, 패리티만 있으면 둘이 **같이** 빠진 조건은 못 잡으므로
  절대 조건 테스트(`testNormalizedRejectsEveryUntrustedField`)를 같이 둔다(주입으로 확인).
...
- **재발 (2026-09-08, 포코피아 아이소메트릭).** 캔버스 폭 단정(504 ≤ 512)은 통과하는데 원점이
  반 칸 모자라 **맨 왼쪽 대각선 한 줄이 18pt 캔버스 밖으로** 나갔다. 잘린 곳이 격자의 모서리라
  화면으로는 "원래 그런 모양" 으로 읽힌다 — 총폭이 맞으니 단정도 못 잡는다. 침하한 물이
  아래 여백을 0.4pt 넘긴 것도 같은 자리에서 나왔다.
- **처방**: 좌표계를 새로 만들면 총폭·총높이 대신 **모든 칸을 돌며 각 칸의 네 끝이 캔버스 안인지**
  센다(`PokopiaTownIsoTests.testEveryTileStaysInsideTheCanvas`). 여백 상수를 눈으로 맞추면
  가장자리 한 줄만 어긋나고, 가장자리는 눈으로 검사할 수 없는 자리다.
...
## 세이브에 **쓰기만** 하고 아무 화면도 읽지 않으면, 기능은 와이어에 빈 필드로 나간다

- **증상**: Memory Home 방문이 전송로는 멀쩡한데 절반만 동작했다. 방문자가 받는 카드의 대표
  기억(`sharedMemoryBody`)과 대표 사진(`featuredPhoto`)이 **모든 릴리스에서 nil** 이었고, 내 홈에
  누가 다녀갔는지 볼 화면도 차단할 길도 없었다(TODAY/TOTAL 숫자만 올랐다). §14 파도타기는
  방문 도장을 저장만 하고 읽지 않아 발자국이 화면에 없었다.
- **원인**: `save()` 를 부르는 앨범 API 일곱 개(`pin`·`setSharedPinnedMemory`·`setFeaturedPhoto`·
  `setPeerAlias`·`setMemoryHomeBlocked`·`clearProfileMessage`, 그리고 읽는 쪽이 없던
  `visitedHomeStamps`·`recentRequesters`)의 **호출부가 0곳**이었다. 값은 늘 기본값이므로 카드를
  굽는 코드는 정상인 채로 빈 필드를 실어 보냈다.
- **왜 테스트가 못 걸렀나**: 앨범 메서드의 지속성과 프로토콜 클램프를 직접 호출하는 테스트가
  38개 있었고 전부 통과했다 — **호출자가 화면이 아니어도 통과하기 때문이다.** 커버리지도 못
  잡는다(테스트가 그 줄을 실행하므로 100% 로 찍힌다). 카드를 굽는 `profileCard` 는 `private` 여서
  "무엇을 내보내는가" 를 묻는 테스트가 아예 없었다.
- **부류**: 바로 아래 항목("살아 있는 표현이 둘이면")의 **셋째 형제**다. 타입도 시트도 살아 있는데
  기능이 죽는 세 번째 형태가 "세이브를 바꾸는 API 를 아무도 부르지 않는 것" 이다. 같은 부류로
  (a) 읽는 쪽이 없는 저장 필드 (b) 원격에 보내는 페이로드를 만드는 `private` 함수
  (c) 앨범·설정 mutator 를 부르는 유일한 곳이 테스트인 경우.
- **처방**: 여섯 API 에 화면을 붙이고(방문자 패널·대표 기억·대표 사진·문구 지우기),
  `profileCard` 를 `internal` 로 열어 카드 내용에 테스트를 걸었다. `test-gate.sh` 에 **호출부 없는
  앨범 mutator 스윕**을 넣었다 — `save()` 를 부르는 비-private 함수 중 호출부가 0곳인 것이 있으면
  실패한다. 결함을 되돌려(방문자 패널 삭제) 실제로 실패하는지 확인했다.
  **한계도 같이 적었다**: 잡는 것은 "호출부 0곳" 하나이고, 마운트되지 않은 뷰 멤버 안의 호출은
  못 잡는다 — 뷰 멤버 도달성을 grep 으로 근사하려다 200여 개에 오탐이 쏟아져 접었다.
  (`MemoryHomePresenter`·`MemoryHomeSurf`·`MemoryHomeVisitCenter`·`test-gate.sh`, 2026-09-03.)

## 테스트가 스토어를 공용 temp **디렉토리**에 만들면, 곁방 세이브가 실행 사이에도 남는다

- **증상**: 새로 쓴 방문 카드 테스트가 "전시만 했는데 대표 사진으로 나갔다" 로 실패했다. 그런데
  대표 사진을 지정하는 코드는 그 테스트에 없었다 — **앞선 실행**의 다른 테스트가 지정한 값이었다.
- **원인**: `CompanionStore` 는 기억 앨범을 상태 파일 **옆에** 만든다(`init` 의 `directory`).
  테스트가 상태 파일 이름만 유일하게 하고(`poke-showcase-<uuid>.json`) 공용 temp 디렉토리에 두면,
  곁방 파일(`pokemon-memories.json`)은 **이름이 고정**이라 모든 테스트가 하나를 공유한다.
  temp 디렉토리는 실행 사이에 지워지지 않으므로 `swift test` 를 다시 돌려도 남는다.
- **왜 테스트가 못 걸렀나**: 기존 테스트들은 앨범을 `PokemonMemoryAlbum(fileURL: <유일>)` 로 직접
  만들어 이 경로를 밟지 않았다. 스토어를 거쳐 앨범을 쓰는 테스트가 이 형태로는 처음이었다.
...
- **처방**: `connectionDropped(_:)` 하나로 모은다 — `.failed` 핸들러와 읽기 루프의 모든 실패
  ...
  (`PokemonTrade.swift`, 2026-08-30.)
- **스윕이 다섯째 파일에 안 갔다 (2026-09-03, `#228`)**: 위 처방을 적은 날 `PokemonAuction` 은
  건드리지 않았다. `receiveLength`/`receiveBody` 가 그대로 조용히 리턴했고, 그래서 상대가 앱을
  정상 종료해도 경매 연결은 죽은 채 남았다. **이 부류의 재발 방식이 `loadUnaligned`(`bee4a84`)
  와 똑같다** — 부류를 적는 것으로는 스윕이 완료되지 않는다. 적을 때 형제를 **전수로 세고**
  파일 이름을 함께 남겨야 한다. 지금 상태(다섯 다 EOF 에서 끝낸다):
  `PokemonTrade`·`BattleNet`·`MultiplayerRoomCenter`·`MemoryHomeVisitCenter`·`PokemonAuction`.
- **왜 이번엔 테스트가 열렸나**: `PokemonAuction` 은 `attachForTesting` 이 `connections` 에만
  등록하고 읽기 루프를 걸지 않아 그 경로를 밟을 진입점이 아예 없었다. 새 테스트는 스캐폴딩을
  더하는 대신 **프로덕션 진입점**을 쓴다 — `apply(to:offering:)` 가 `listing.endpoint` 로 진짜
  소켓을 열므로, 게시물의 endpoint 를 루프백 탭의 포트로 주면 실제 경로가 그대로 선다.
  테스트 전용 훅을 새로 만들면 그 훅이 프로덕션과 갈라진다(defect-log: 테스트가 프로덕션 경로
  대신 자기 디버그 훅의 사본을 밟는 부류).

## 종료 국면이 자원을 놓지 않는 부류 — 상한은 회수가 아니다

- **증상**: 경매의 내 제안이 `.declined`/`.failed`/`.completed` 로 끝나도 그 `NWConnection` 이
  `connections` 에 남았다. 제안 여덟 건을 걸고 전부 거절당한 뒤 결과 카드를 안 치우면 소켓
  여덟 개가 앱이 끝날 때까지 산다.
- **부류**: **"정원이 있으니 괜찮다" 로 회수를 미루는 부류.** `maxOutgoingOffers = 8` 은 상한이지
  회수가 아니다. 판별식은 "이 상한이 **모든** 경로를 덮는가" 다 — 덮지 않는 형제가 대개 있다.
  여기서는 게시자 쪽 **반려된 `.apply`** 가 그것이었다: 제안이 서지 않으니 어느 정원에도 세어지지
  않고, 구버전 앱 하나가 재시도하면 소켓이 **무제한으로** 쌓인다. 리포트(`#228`)는 신청자 쪽
  네 곳을 셌는데 실제로는 양쪽 열한 곳이었다.
- **처방 — 규칙은 한 곳**: `reclaimIfIdle(_:)` 하나가 "이 연결에 살아 있는 제안이 남았는가" 를
  묻고, `receive` 는 그것을 **`defer` 로** 부른다. 조기 반환이 여럿(가드 실패·재전송·커밋
  실패·반려된 `.apply`)이라 꼬리에만 적으면 그 갈래가 통째로 빠진다. 호출부마다 적는 쪽을
  택하면 형제 `MemoryHomeVisitCenter` 가 물린 그 부류(회수를 성공 분기에만 걸어 두는 부류)로
  곧장 돌아간다 — 거기서도 끝나는 길이 다섯인데 하나에만 적어 넷이 샜다.
- **관측 창**: `trackedConnectionCount`(파생값). `connections` 가 `private` 이면 "새는지" 를 볼
  방법이 아예 없어 이 부류가 조용히 돌아온다 — `MemoryHomeVisitCenter` 가 같은 이유로 같은
  이름의 창을 열어 뒀다. **선례를 먼저 찾는다**: 이 부류의 관측 창은 이미 설계돼 있었다.
- **가드 검증**: 다섯 갈래를 각각 되주입해 빨간불을 확인했다(EOF·짧은 읽기·길이 불량 각 4건,
  `defer` 제거 7건, `reject` 회수 제거 3건, 전부 되돌리면 0건).
  (`PokemonAuction.swift`, 2026-09-03.)

## 재현 실험이 프로덕션 경로를 모델링하지 못하면, 없는 결함을 고치게 된다

- **증상**: `#228` 은 "`send` 직후 `cancel()` 은 프레임을 버릴 수 있다" 를 전제로 달려 있었다.
  스탠드얼론 실험으로 3회 재현해 **"버릴 수 있다가 아니라 항상 버린다"** 고 단정했고, 그 위에
  "`cancelOutgoingOffer` 때문에 게시자 카드가 `#227` 이후 영구히 `.pending` 이다 — 누수가 아니라
  오늘 보이는 결함" 이라는 더 센 주장을 세웠다. **둘 다 프로덕션 경로에서는 재현되지 않았다.**
  고친 코드에 결함을 되주입했더니 두 프레임이 다 도착하고 테스트는 3회 모두 초록이었다.
- **부류**: **실험이 그럴듯하게 실패해서 믿어 버리는 부류.** 실험은 `NWParameters.tcp` 였고
  프로덕션은 `includePeerToPeer = true` 다. 다른 변인(큐 위치·소켓 온도)을 4×2 로 갈라 20/20
  유실을 재확인해도 **프로덕션 경로에서는 여전히 도착했다** — 즉 통제하지 못한 변인이 남아 있었고,
  실험의 일관성(20/20)이 그 사실을 가렸다. 재현율이 높다는 것은 모델이 맞다는 증거가 아니다.
- **무엇이 잡았나**: 통과 확인이 아니라 **결함 재주입**이다(CLAUDE.md 프로토콜 3). 다섯 갈래 중
  넷은 주입하면 빨간불이 됐고 이 하나만 초록으로 남았다 — 그 비대칭이 신호였다. 재주입을
  안 했으면 "측정으로 확정한 결함" 이라는 문장이 커밋 메시지와 defect-log 에 그대로 남았다.
- **처방**: ① 실험은 프로덕션이 쓰는 **인자·큐·호출 순서**를 그대로 복제하거나, 아니면 결론을
  "이 경로에서 재현" 이라고 쓰지 않는다. ② 테스트가 지키지 못하는 보장은 **코드 주석과 테스트
  주석 양쪽에** 그렇게 적는다. `closeWhenFlushed` 는 남겼지만 근거를 "재현된 결함" 에서
  "순서 보장"으로 낮춰 적었다 — `send` 는 비동기이고 API 에 `forceCancel()` 이 따로 있다는 것은
  `cancel()` 의 flush 가 계약이 아니라는 뜻이며, 여기서 종료 프레임을 하나 잃으면 상대 카드는
  자동 시간 제한이 없어 영구히 `.pending` 이다(개체 소유권이 오가는 두 단계 커밋이다).
  두 줄로 사는 보장이면 싸다 — 다만 **싸다는 것이 검증됐다는 뜻은 아니다.**
- **부수 함정 — 동기점을 당기면 테스트가 플레이키가 된다**: 결함을 밟게 하려고 동기점을 `.apply`
  도착에서 TCP 수락(`tap.peer`)으로 당겼더니, 우리 쪽 `.ready` 보다 앞서는 실행에서 `.apply` 가
  아직 안 나간 채 연결이 접혀 간헐적 빨간불이 됐다. 밟히지도 않는 결함을 위해 만든 경쟁 조건이라
  되돌렸다. (`PokemonAuction.swift`, 2026-09-03.)
- **덧붙임 — "두 줄로 사는 보장" 이 공짜가 아니었다 (2026-09-03, 리뷰)**: `closeWhenFlushed` 는
  회수를 `send` 의 **완료 콜백**에 맡겼는데, 그 콜백이 오지 않는 상태가 있다. `NWConnection` 의
  `.waiting` 은 실패로 승격되지 않고 무한 재시도라(상대가 사라진 게시물에 제안하고 바로 거둬들이면
  정확히 그 상태다), 큐에 넣은 종료 프레임은 영영 처리되지 않는다. 그 사이 연결은 `connections`
  에서 이미 빠졌으므로 `drop` 도 `stateUpdateHandler` 도 그것을 못 찾고, 클로저가 자기 연결을
  강하게 물어 **회수 자체가 없어진다** — 고치려던 부류를 고치면서 같은 부류를 되심었다.
- **부류**: **회수를 비동기 완료 콜백에 맡기는 부류.** 판별식은 "이 콜백이 **안 오는** 상태가
  있는가" 다. 있으면 회수는 콜백 안에만 둘 수 없다. 처방은 `.ready` 가 아닐 때 즉시 접는 것이다 —
  아직 안 나간 프레임은 상대도 못 받았으므로 버려도 상대 카드가 `.pending` 에 남지 않는다.
- **왜 못 걸렀나**: 테스트가 `trackedConnectionCount == 0` 만 재서 **장부가 비는 것**과 **소켓이
  닫히는 것**을 구별하지 못했다. 탭에 `sawEOF` 가 이미 있었는데 아무 테스트도 그것을 단언하지
  않았다 — 관측 창을 만들어 두고 쓰지 않으면 없는 것과 같다. 단언을 넣자 회수를 지운 되주입이
  즉시 빨간불이 됐다. (`PokemonAuction.swift`, 2026-09-03.)
- **부류 — 회수 판정이 프레임을 읽은 뒤에만 걸리는 부류 (2026-09-03, 리뷰)**: `reclaimIfIdle` 은
  `receive` 뒤에서만 도므로, 붙어서 **아무것도 보내지 않는** 수신 연결은 어느 판정에도 걸리지
  않는다. `#227` 이 자동 시간 제한을 걷어내 시간이 끊어 줄 것도 없다. 같은 LAN 의 피어 하나가
  (포트 스캐너로도 된다) 리스너에 붙어 침묵하면 `connections` 가 무제한으로 큰다. 처방으로
  `PokemonAuctionCenter.maxConnections`(64) 를 두고, 수락 경로를 `acceptConnection(_:)` **한 문**
  으로 모았다 — 호출부에 정원을 두면 나중에 생긴 수락 경로가 무제한으로 남는다.
- **남은 구멍(의도)**: 형제 다섯 센터(`PokemonTrade`·`BattleNet`·`MultiplayerRoomCenter`·
  `MemoryHomeVisitCenter`·LAN 레이드)에는 정원이 없다. 유휴 타임아웃은 다섯 곳에 걸친 새 기전이고
  `#227` 의 결정과도 부딪혀 이 변경 밖으로 뒀다 — **부류로 여기 적어 두는 것이 그 스윕의 근거다.**
  형제 쪽 소켓 누수를 다시 만지는 회차에 같은 한 문(`acceptConnection`)부터 만든다.

## 커버리지 총계가 89%인데 방금 넣은 분기가 한 번도 안 돈 부류 (세 번째)

- **증상**: `receiveBody` 의 짧은 읽기 분기를 새로 넣고 테스트 27개가 초록이었다.
  `--show-regions` 를 보니 그 줄의 실행 횟수가 **`0`** 이었고, 프레임 디코드·전달 경로까지
  줄줄이 `0` 이었다. 로직 코어 라인 커버리지는 그때 **89.62%** 였다.
- **원인**: 기존 테스트 스무 개가 `receive(_:connectionID:)` 를 **직접** 부른다. 길이 프리픽스
  읽기·디코드·전달을 통째로 건너뛰므로, 그 루프의 가드를 고쳐도 아무도 해피 패스가 깨진 것을
  모른다. 국면 가드를 재는 데는 그게 맞지만, **그 루프를 고치는 변경에는 맞지 않는다.**
- **처방**: 탭이 소켓으로 세 가지를 보낸다 — 반쯤 끊긴 프레임, 불가능한 길이, 온전한 프레임.
  마지막 하나가 "소켓으로 온 프레임이 제안에 닿는다" 를 재는 **유일한** 테스트다.
- **일반화**: 판별식은 "내가 고친 함수를 밟는 테스트가 있는가" 이고, 답은 총계가 아니라
  `xcrun llvm-cov show <bin> -instr-profile=<prof> <file> --show-line-counts-or-regions` 의
  **그 줄 실행 횟수**다. `--show-regions` 만으로는 열이 안 보인다 —
  `--show-line-counts-or-regions` 를 함께 준다.
- **딸린 원인**: `PokemonAuction.swift` 가 `test-gate.sh` 의 `LOGIC_CORE` **배열 밖**이었다.
  배열에 넣었다(임계값 69% 에 라인 89.62% 로 여유가 있다). 형제 `PokemonTrade`·
  `MemoryHomeVisitCenter` 를 넣은 이유와 같다.
  (`PokemonAuction.swift`·`scripts/test-gate.sh`, 2026-09-03.)
...
## 화폐·원장·화면을 갈아 끼운 뒤 **옆에 남은 것들** — 서명 밖 원장, 죽은 필드의 주석, 중첩 스크롤

2026-09-02 감사에서 나온 중간·낮음 항목 묶음이다. 하나하나는 작지만 공통 뿌리가 하나다:
**무언가를 갈아 끼울 때 그것을 가리키던 자리를 함께 보지 않았다.**

- **일일 사탕의 멱등 키가 서명 밖에 있었다.** `lastCandyDate` 를 어제로 고쳐 적으면 사탕을 매일
  여러 번 받는다. 형제 원장(`gd` 체육관 방어 보상·`ab` 모험 보너스)은 이미 서명 안에 있었다 —
  재화 멱등 가드를 새로 만들 때 서명에 넣는 규칙이 규칙으로 적혀 있지 않아서 빠졌다.
  캡처: `CandyLedgerSignatureTests`(빈 값이면 세그먼트를 안 붙이는 것까지 본다 — 무조건 붙이면
  이 필드가 없던 정상 세이브가 전부 조작으로 판정된다).
- **판돈을 못 낸 베팅이 원장에 남았다.** 호스트가 보는 상한은 참가 시 **신고한** 잔액이라,
  로비에서 별조각을 쓴 뒤 베팅하면 검사는 통과하고 에스크로만 실패한다. 그 항목이 원장에 남으면
  배당이 없는 돈을 나눠 별조각이 생성된다("관전자 사이 이동만" 불변식 위반). 규칙을 순수 함수
  (`PokeathlonPool.withoutUnfundedBet`)로 두고 양쪽이 같은 것을 쓴다. 금액 0 프레임이 관전자의
  철회 경로다(구버전 호스트는 그냥 거절하므로 안전하다). 캡처: `PokeathlonEscrowTests`.
- **`await` 뒤 강제 언랩과 낡은 스냅샷 되쓰기.** `changeRotomForm` 은 `state.active!` 를 썼고
  (조회 중 교환·방생이면 크래시), `detailedMoves` 는 조회 **전** 스냅샷을 통째로 되써서 그 사이
  배운 기술을 지웠다. 둘 다 개체 ID 로 다시 확인하고, 되쓰기는 배열 통째가 아니라 **기술 ID 별
  병합**으로 바꿨다.
- **팝오버 탭 안의 중첩 `ScrollView`.** 상점·가방이 본체 스크롤 안에 자기 스크롤(고정 높이 520)을
...
- **사라진 규약을 설명하는 주석 셋.** 화폐가 `starPieces` 로 바뀌었는데 세 곳이 "재화 =
...
## 게스트가 **호스트만 아는 값**으로 먼저 정산하면, 기여도 항은 0 이고 원장은 타 버린다

- **증상.** 리뷰로 발견했다(2026-09-02). LAN 협동 레이드(#80)에서 5★ 를 캐리한 게스트가
  기여도 항(최대 2,000)을 통째로 못 받았다. 결과 화면은 항목별 합계를 다 그리면서 그 아래에
  "오늘의 레이드 보상은 이미 받았습니다"를 띄워, 방금 지급된 사실 자체를 부정했다.
- **직접원인.** 호스트는 `.roundResolved` 를 **먼저** 보내고 그 뒤 `.raidSettlement` 를 보낸다
  (같은 TCP 연결이라 순서가 고정이다). 게스트는 첫 메시지의 `settleRaidIfFinished()` 에서
  빈 `raidContributions` 로 `myDamage: 0, totalDamage: 0` 을 계산해 지급하고 `raidRewardDate` 를
  오늘로 찍었다. 직후 도착한 정확한 정산은 화면만 갈아 끼우고 `creditRaidReward` 는 이미 받은
  날이라 0 을 돌려줬다. 같은 자리에서 종료 라운드도 갈렸다 — 호스트는 해상 라운드를 명시로
  박고 게스트는 라운드를 올린 **뒤** `combatRound` 로 채워, 남은 턴 보너스가 한 턴씩 달랐다.
- **테스트·검증이 왜 못 걸렀나.** 레이드 테스트가 **전부 호스트 시점이거나 순수 함수**였다.
  게스트 경로는 `receiveGuestLoop` 의 switch 안에 박혀 있어 살아 있는 `NWConnection` 없이는
  어떤 입력으로도 못 밟았고, 와이어 왕복 테스트는 메시지가 **디코딩되는지**만 봤지 그 메시지가
  **어떤 순서로 적용되는지**는 보지 않았다. 호스트는 두 값을 동시에 알아 이 결함이 없다.
- **부류 스윕.** "호스트와 게스트가 갈라지는 자리"를 전수로 봤다. `applyResolvedRound` 는 이미
  공용 함수였고(같은 이유로 만들어졌다), 갈라져 있던 것은 정산 세 자리뿐이었다. 게스트 진입점
  셋(`applyGuestRaidStart`·`applyGuestRaidSettlement`·`applyGuestResolvedRound`)을 소켓 switch
  **밖으로** 뗐다 — 테스트가 못 밟는 코드는 갈라져도 아무도 모른다는 것이 이 결함의 본질이다.
- **회귀 테스트.** `RaidRoomTests` 의 "게스트 시점" 절. 도착 순서 양방향(라운드 먼저 / 정산 먼저),
  중복 정산, 무임승차(기여 0), 종료 라운드 수를 본다. 지급 가드(`contributionsSettled`)를 빼고,
  종료 라운드를 `combatRound` 로 되돌려 **실제로 빨개지는 것을 확인했다**.
- **곁가지.** 지급의 전제를 "둘 다 도착"으로 두면 반대편 실패(정산이 먼저 오면 영영 안 준다)가
  새로 생긴다 — 두 진입점이 **같은 멱등 함수**를 부르게 해서 양쪽을 닫았다. 그리고 정산이 끝내
  안 오면 원장을 안 태우는 쪽으로 실패한다(오늘 다시 돌 수 있다).
  (`MultiplayerRoomCenter.swift`, 2026-09-02.)

## `guard var x = self.x` 로 뜬 **값 복사본**은 그 뒤의 `self` 변경을 못 본다

- **증상.** 리뷰로 발견했다(2026-09-02). 턴 상한(20)으로 끝난 레이드의 결과 화면 위에서 턴
  카운트다운이 계속 돌고, 30초 뒤 마감 처리가 이미 종료된 판으로 다시 진입했다.
- **직접원인.** `finishRoundIfReady` 가 `guard isHost, var battle`(struct 값 복사)로 시작한다.
  `endRaidIfTurnCapReached()` 는 `self.battle` 에 전멸을 적용하고 `turnEndsAt` 을 끄는데, 바로
  다음 줄의 `if battle.isFinished { return }` 은 **상한 처리 전에 복사된 지역 값**이라 항상
  거짓이었다. 그래서 방금 끈 마감 타이머가 30초 뒤로 되살아났다.
- **테스트·검증이 왜 못 걸렀나.** 상한 테스트는 `MultiplayerBattle.endByTurnCap()` 이라는
  **순수 계산**만 검증한다. 그 뒤에 방이 무엇을 하는지는 소켓 밖으로 떼어 두지 않아 아무도
  보지 않았다. 컴파일러도 못 잡는다 — 지역 `battle` 은 문법적으로 완전히 유효하다.
- **부류 스윕.** 방 센터의 `guard isHost, var …` 열두 자리를 전수로 봤다. 나머지 열하나는
  전부 지역 값을 `self` 에 **되쓴 직후에만** 읽어(예: `gymEngine = engine` 다음 줄의
  `engine.snapshot()`) 값이 같다. 자기 자신을 바꾸는 **다른 메서드를 부른 뒤** 지역 값을 다시
  읽는 자리는 이 한 곳뿐이었다.
- **회귀 테스트.** 이 자리는 호스트 전용 private 경로라 직접 못 밟는다 — 대신 판정 자체를
  `self.battle?.isFinished == true` 로 바꿔 **읽는 대상을 하나로** 만들었다.
- **곁가지.** 값 타입을 `var` 로 꺼내 쓰는 함수 안에서 다른 메서드를 부르면, 그 뒤의 판정은
  지역 사본이 아니라 `self` 를 본다. (`MultiplayerRoomCenter.swift`, 2026-09-02.)

## 레이드 리뷰에서 **옆에 남은 것들** — 알림 원장, 창 게이트, 업적 경계, 죽은 상태

2026-09-02 리뷰가 같은 PR 에서 함께 잡아낸 작은 부류들이다. 하나씩은 사소하지만 전부
"경계를 한 자리에만 걸어 둔" 같은 모양이다.

- **알림을 못 내는 상태에서도 "알린 것"으로 적으면, 그 방은 영영 안 알린다.** 교전 중
  (`phase != .idle`)에 뜬 레이드 방까지 `announcedRaidRooms` 에 넣어, 판이 끝나 목록으로
  돌아왔을 때 이미 본 방이 되어 45분짜리 5★ 창을 통째로 놓쳤다. **알린 것만 적는다**; 사라진
  방을 빼는 정리는 국면과 무관하게 매번 한다(안 빼면 껐다 켠 방을 다시 못 알린다).
- **렌더 시점에만 도는 게이트는 화면을 열어 두면 만료되지 않는다.** 5★ 를 부화 창 안으로
  가두는 검사가 티어 피커의 `let hatchIsLive = …` 한 곳뿐이라, 화면을 띄워 둔 채 45분이 지나면
  버튼이 그대로 남아 창 밖에서 5★ 방이 열렸다. 검사를 **누르는 순간**(`createRaidRoom`)으로
  옮겼다. 받는 쪽 검증에는 넣지 않았다 — 신뢰경계의 잔여 위험을 이미 수용한 자리라
  (`lan-raid-design.md`) 여기만 막으면 이득이 없다.
- **불변식을 지키던 "호출부가 하나"가 새 호출부로 깨진다.** `grantBattleReward` 의 배틀 업적은
  "LAN 배틀 승리만 — 연습 배틀은 혼자 무한 반복이라 클릭 노동이 된다"는 이유로 붙어 있었고,
  주석이 그 경계의 근거를 **호출부가 하나**라는 사실에 두고 있었다. 1★ 레이드는 러너 한 명으로
  시작되고 상대가 NPC 보스라, 그 호출부에 이웃 없이 성립하는 승리가 새로 얹혔다. 협동 레이드는
  사람이 둘 이상일 때만 센다.
- **제거 식별자 개수를 한쪽에서만 뽑으면, 다른 쪽이 많아진 날 지워지지 않는 알림이 남는다.**
  부화 알림을 `weekdayBlocks.count` 로만 지웠다 — 지금은 평일·주말이 둘 다 3이라 우연히 맞는다.
  `RaidBoss.hatchBlocksPerDay`(둘 중 큰 값)로 묶고, 주말 블록을 하나 늘려 실제로 빨개지는 것을
  확인했다.
- **읽는 곳이 없는 `@State` 는 두 번째 진실 원천으로 오해된다.** `RaidView.pickedTier` 는 쓰기만
  하고 아무 뷰도 관찰하지 않았다(티어는 눌리는 즉시 센터로 넘어가고 이후 전부 `center.raidTier`
  를 읽는다). 같은 계열로 `raidRewardTurns` 가 `raidTurnsLeft` 와 3개 언어 전부 같은 문자열을
  중복 정의하고 있었고, 새 원장 프로퍼티가 `creditStarPieces` 의 doc 주석과 함수 **사이**에
  삽입돼 주석이 엉뚱한 대상을 설명하고 있었다. 셋 다 지우거나 제자리로 옮겼다.
- **죽은 fallback 은 커버리지에 `^0` 으로 남는다.** `raidFinishedRound` 를 옵셔널로 두어
  `?? RaidBoss.turnCap` 이 붙어 있었는데, 쓰는 자리가 전부 쓰기 뒤라 **어떤 입력으로도 못 밟는
  분기**였다. 비옵셔널로 바꿔 fallback 을 없앴다(`--show-regions` 로 확인).
  (2026-09-02.)
## 상태를 **다른 경로가 정규화**하면, 그 경로의 계약을 안 지킨 생산자는 조용히 되돌려진다

- **증상.** 레이드에서 잡은 보스가 동행 자리에 앉는 순간 **레벨 1 의 1단계**가 됐다(가디안 → 랄토스).
  잡는 자리는 정상이었고 박스에 들어간 직후의 값도 정상이었다.
- **직접원인.** `catchRaidBoss` 는 잡은 그 모습으로 세우려고 `pathIDs: [speciesID]` 를 썼는데,
  같은 개체를 **나중에** 지나는 `loadCurrentLine → normalizedEvolutionState → longestValidPath` 의
  계약이 "첫 ID 가 트리 뿌리와 다르면 뿌리로 복구" 였다. 라인은 최종체를 물어도 체인 뿌리부터
  오므로(`PokeAPIClient.line` 은 `evolution_chain` 을 그대로 트리로 만든다) 두 계약이 정면으로
  어긋난다. 추첨 풀 32종 중 21종이 이전 진화를 가진다.
- **왜 생산자만 고치면 안 되나.** 그 자리에서 뿌리→종 경로를 만들어 넣으면 `totalForms > 1` 이 되어
  졸업 면제(`canGraduate` 의 "레벨로 진화해 온 개체" 규칙)가 레벨 1 전설에 붙는다 — 알 20,000
  짜리 보상이 포획 직후 졸업으로 새는 두 번째 결함이 된다. 되돌리던 쪽(공유 함수)을 고쳐야 한다:
  시작 종이 트리에 **있으면** 거기서부터 잇고, 아예 없을 때만(에셋 정리로 빠진 종) 뿌리로 복구한다.
- **테스트가 왜 못 걸렀나.** 공유 스텁(`stubStore`)이 **어떤 종을 물어도** 단일 노드 트리 하나를
  돌려준다(`EvoLine(baseID: 20, tree: EvoNode(speciesID: 20, children: []))`). 그래서 트리거 조건인
  "저장된 경로가 트리 뿌리에서 시작하지 않는다" 가 테스트에서 **성립 자체가 불가능**했다. 포획
  테스트 넷이 전부 초록이었고, 그중 하나는 detached `loadCurrentLine` 을 기다리지도 않았다.
  → 픽스처가 조건을 성립 불가능하게 만드는 부류다. 다단 라인 시드(`stubThreeStageLine`)와 실제
  private 경로를 부르는 훅(`debugReloadCurrentLine`)을 남겼다.
- **부류 스윕.** `MonState(baseID:` 를 직접 세우는 자리 넷 중 뿌리에서 시작하지 않는 것은 포획뿐이고
  (탈피 껍질 `[290, 292]` 는 뿌리 시작, 부화·스타터도 뿌리 시작), 계약은 이제 공유 함수 한 곳이
  강제한다. (2026-09-03.)

## `hp = 0` 으로만 표시되는 이탈자는 **목록에서 빠지지 않는다**

- **증상(리뷰 발견).** 레이드 포획 추첨이 이미 방을 떠난 사람을 당첨시킬 수 있었다. 그 클라이언트는
  `leaveRoom` 을 지나 정산도 포획도 부르지 않으므로 **보스가 아무에게도 안 가고**, 남은 사람 화면엔
  "동료가 데려갔다" 가 남는다.
- **직접원인.** `battle.forfeit(participantID:)` 는 hp 만 0 으로 만들고 `fighters` 에는 남긴다 —
  명시적 이탈과 연결 끊김이 같은 자리를 지나게 한 설계다. 편성으로 무언가를 **뽑거나 나눠 줄** 때는
  그 사실을 매번 다시 걸러야 한다. 바로 위 `survivorBonus` 는 같은 배열을 `isAlive` 로 걸렀고
  추첨만 안 걸렀다 — **한 함수 안의 비대칭**이 신호였다.
- **처방.** 뽑는 풀은 `runners.filter(\.isAlive)`, 머릿수 게이트는 참가자 전체
  (`RaidBoss.coopTermsApply`) — 같이 싸우다 쓰러진 동료가 협동 판을 포획 없는 판으로 만들면 안 된다.
- **부류 스윕.** 편성에서 세거나 뽑는 나머지 두 자리(전적의 참가자 수·상대 이름)는 "싸운 사람"
  을 세는 것이 맞아 그대로 뒀다. (2026-09-03.)

## 불리언 반환은 **사유가 둘 이상이면** 화면이 반드시 한 번 거짓말한다

- **증상(리뷰 발견).** 오늘의 포획을 이미 쓴 판에서 "보스 정보를 불러오지 못했어요 — 오늘 다시
  도전할 수 있어요" 가 떴다. 두 문장 다 거짓이다: 원장 가드가 네트워크를 건드리기 전에 돌려보냈고,
  오늘 다시 도전해도 결과는 같다. 같은 부류로 성공 문구가 "박스에 들어왔어요" 하나뿐이라, 빈 동행
  자리를 채운 판에서 사용자가 빈 박스를 열게 됐다.
- **직접원인.** `catchRaidBoss -> Bool`. 실패 사유 둘과 성공 목적지 둘이 두 값으로 접혔다.
  호출부는 접힌 값에서 문구를 되살릴 수 없다.
- **같이 드러난 것 — 낙관적 상태를 문구의 근거로 쓰면 모순된 두 문장이 한 화면에 남는다.**
  `raidCatcherID` 는 비동기 포획이 **시작되기 전에** 정해지는데 결과창이 그 값만 보고 "박스에
  있어요" 를 그렸다. 조회가 실패하면 같은 스크롤에 "데려오지 못했어요" 와 나란히 떴고 박스는
  비어 있었다. 결과 문구는 **실제 결과**(`RaidCatchResult`)를 기다린다.
- **테스트가 왜 못 걸렀나.** 원장·실패 경로 테스트가 전부 `XCTAssertFalse(didCatch)` 였다 —
  false 의 **사유**를 아무도 묻지 않았고, 화면이 그 사유로 문장을 만드는 자리는 뷰 안의 private
  계산 프로퍼티라 테스트가 닿지 않았다. 사유를 방 층(`raidCatchResult`)으로 올려 재게 만들었다.
  (2026-09-03.)

## 서명에 **파일 밖 입력**을 넣으면, 그 조회 한 번의 실패가 진행도 전멸이 된다

- **증상.** 사용자가 버전을 올리고 첫 실행에서 진행도가 통째로 사라졌다. 앱 로그는
  `save integrity check failed — tampered save detected, resetting progress` 한 줄뿐이었다
  (2026-09-02, 2.23.3).
- **직접원인.** `SaveTransfer.canonicalString` 의 마지막 세그먼트가
  `sec\(DeviceID.stableIdentifier())` 였다. `stableIdentifier()` 는 IOKit `IOPlatformUUID` 조회에
  실패하면 **호스트명으로 폴백**한다. 그 실행에서 조회가 실패해 시드가 바뀌었고, 시드가 바뀌면
  해시 전체가 어긋나므로 정상 세이브가 손편집으로 판정됐다.
- **왜 조작이 아니라고 단정할 수 있나.** 초기화 직전 보존된 원본 바이트를 같은 빌드의 규칙으로
  다시 해싱하니 저장된 서명과 **정확히 일치**했다(`4e271d8d8e573bae`). 파일도 코드도 정상이었다.
  `canonicalString` 의 입력 중 파일 밖에서 오는 값은 `sec` 하나뿐이므로 소거법으로 확정된다.
- **테스트가 왜 못 걸렀나.** 무결성 테스트가 전부 **한 프로세스 안에서** 서명하고 곧바로 검증한다.
  양쪽이 같은 시드를 쓰니 시드가 무엇이든 통과한다. 트리거는 "서명한 실행과 검증하는 실행의 시드가
  다르다" 인데, 그 축을 가진 테스트가 하나도 없었다. `A && B` 게이트를 B 가 늘 참인 상태로만
  시험하는 부류와 같다.
- **처방 — 검증 불가는 조작이 아니다.** 오탐 비용(진행도 전멸)이 미탐 비용(손편집 세이브 하나)과
  비교가 안 된다. 시드를 못 읽은 실행은 **검사도 서명도 하지 않는다**. 검사만 완화하면 절반만
  막힌다 — 폴백 값으로 서명해 두면 조회가 정상으로 돌아온 다음 실행이 그 서명을 재현하지 못해
  결국 같은 초기화가 난다. 시드는 `DeviceID.hardwareIdentifier()`(폴백 없음, 프로세스당 1회 조회)
  로 읽고, `signed`/`isTampered` 가 `String?` 로 받아 nil 이면 각각 빈 서명·검사 생략으로 간다.
- **영구 캡처.** `SaveTransferTests` 3종 — 시드를 **인자로 갈라** 실행 간 변화를 재현한다:
  ①서명 시드 있음 → 검증 시드 없음이 조작이 아니다, ②시드 없이 쓴 세이브는 시드가 돌아와도
  유효하다, ③시드가 **있는데 다르면** 그대로 조작이다(완화가 검사를 통째로 끄지 않았다는 거짓양성
  가드). 셋 다 결함 주입(폴백 시드로 서명·검사)으로 실패하는 것을 확인했다.
  로그도 남겼다 — `DeviceID` 가 조회 실패 시 한 줄 쓴다. 이 줄이 없어서 원인 규명에 로그가 아니라
  해시 재현이 필요했다.
...
## 같은 예외를 **두 자리에 따로 적으면**, 한쪽만 갱신돼 화면이 잠긴다

- **증상**: 관장이면 친구 탭이 체육관 화면에 갇혔다. 교환도 1:1 배틀도 토너먼트도 2~4인 방
  배틀도 못 열고, 닫기(X)를 눌러도 안 나가진다. #209 를 고치면서 되살아난 회귀다.
- **직접원인**: "방이 켜져 있으면 그 화면으로" 규칙에 **체육관 예외**가 붙어 있는데(관장은 도전을
  기다리는 배경 상태지 배틀 중이 아니다), 그 예외가 `body` 와 `onAppear` 두 곳에 따로 적혀
  있었다. `onAppear` 는 `room != .gym` 으로 예외를 지켰고 `body` 는 `roomDestination == .gym`
  으로 예외를 잃었다. `PlayerGymCoordinator` 가 앱 수명 내내 체육관 방을 열어 두므로 그 조건은
  **항상 참**이고, 갈림길 셋이 통째로 도달 불가가 된다.
- **왜 리뷰가 못 걸렀나**: 바로 세 줄 위 주석이 "관장인 것만으로는 화면을 붙잡지 않는다" 라고
  적혀 있었다. **주석이 규칙을 지키지는 않는다** — 코드가 주석과 반대여도 아무것도 안 막는다.
- **처방**: 규칙을 순수 함수 하나로 내고(`FriendView.screenHeldByRoom`) `body` 와 `onAppear` 가
  **그 함수만** 읽는다. 두 자리가 같은 규칙을 각자 적으면 언젠가 갈라진다 — 갈라지는 시점은
  "고치는 중" 일 때가 가장 잦다.
- (#209 후속, 2026-09-03.)

## "단일 표" 를 만들고 **일부만** 옮기면, 표가 갈라진 사실이 숨는다

- **증상**: `LANRoomList` 가 헤더에 "접두 문자열은 여기 한 곳에만 있다" 라고 선언한 채로,
  `PlayerGym.isGymRoomName`(`"GYM ·"`)·`RaidRoomName.isRaidRoomName`(`"RAID · "`)·
  `MultiplayerRoomCenter.join`(`"TOUR ·"`) 이 각자 접두를 들고 있었다. 자기 방 판정(`#꼬리표`)도
  세 벌 더 있었다. `prefix(for: .gym)` / `prefix(for: .raid)` 는 프로덕션 호출자가 아예 없었다.
- **왜 위험한가**: 통합이 **선언만 되고 안 끝난** 상태가 가장 나쁘다. 다음 사람은 헤더를 읽고
  한 곳만 고치는데, 실제로는 네 곳이라 개설 쪽만 바뀌고 목록은 안 바뀐다 — 만든 방이 어느
  목록에도 안 뜨던 #209 의 한 겹 아래 형태다. `join` 은 잘린 `name` 을 읽는 실수까지 겸했다.
- **처방**: 새 "단일 표" 를 만들면 **그 표를 안 쓰는 호출자가 0 인지 grep 으로 확인한다.** 표에
  올라가 있는데 프로덕션 호출자가 없는 항목(`prefix(for: .gym)`)은 통합이 안 끝났다는 신호다.
  소유를 보는 판정(`isVisible`)과 종류만 보는 판정(`matches`)을 나눠 두면 "종류만 필요한 자리"
  가 자기 접두를 다시 적을 이유가 없어진다.
- (#209 후속, 2026-09-03.)

## 가드의 **검사 대상을 손으로 적으면**, 같은 결함이 그 목록 바로 밖에서 다시 난다

- **증상**: 중첩 `ScrollView` 가드(`NestedScrollGuardTests`)가 있는데도 `RoomBattleView` 가
  팝오버 스크롤 안에 자기 `ScrollView` 를 다시 뒀다.
- **직접원인**: 가드가 탭 본문 **네 파일을 하드코딩**했다. 결함은 탭 본문이 아니라 그 자식에서
  났다(`FriendView` → `RoomBattleView`). 허용 목록이 아니라 **검사 목록**이 하드코딩이면,
  목록 밖은 검사되지 않는다는 사실이 초록 화면에 가려진다.
- **부류 스윕 결과**: 검사 범위를 `PopoverView` 탭 전환에서 시작하는 **전이 폐포**로 바꾸자
  같은 결함 2건이 더 나왔다 — `PokemonTradeView` 의 상대 목록과 트레이너 목록. 넷째
  트레이너부터 화면 밖이라 신청 버튼에 손이 안 닿았다.
- **처방**: 가드의 대상은 **코드에서 유도한다**(진입점에서 참조를 따라간다). 그리고 중첩 자체가
  아니라 **높이가 안 묶인 중첩**만 잡는다 — 82pt 전투 채팅·300pt 대표 포켓몬 고르기는 그 안에서
  실제로 스크롤된다. 규칙을 좁히면 예외 목록이 줄고, 예외 목록이 줄면 진짜 결함이 안 덮인다.
- (#209 후속, 2026-09-03.)

## 소스를 훑는 가드가 **헛통과**하는 세 형태 (0개 훑기 · 한 줄만 보기 · 문자열 안 `//`)

- **증상**: 고아 View 조각 가드(`testEveryDeclaredViewMemberHasACallSite`)가 초록인데, 실제로는
  일부만 검사하고 있었다.
- **세 형태**:
  1. **0개를 훑고 통과.** 파일 수(`files.count > 10`)만 단언해서, 선언 인식이 깨져 멤버를 하나도
     못 찾으면 `orphans` 가 빈 채로 초록이 된다. → **훑은 대상 수를 따로 단언한다.**
  2. **한 줄만 본다.** `private func x(...) -> some View` 의 `-> some View` 가 다음 줄에 있으면
     선언 자체가 안 보였다 — 기존 멤버 여섯 개가 그 상태였다. → **괄호가 닫힐 때까지 줄을 붙인
     뒤** 판정한다. 붙인 줄을 선언 인식과 언급 인식이 **같이** 읽어야 한다(따로 두면 붙기 전 첫
     줄이 자기 이름을 "언급" 으로 세어 그 멤버가 영원히 살아 있는 것으로 읽힌다).
  3. **문자열 안의 `//` 에서 잘린다.** 주석을 떼려고 첫 `//` 에서 자르면
     `URL(string: "https://x")!) { footerBar }` 가 `footerBar` 를 잃어 **멀쩡한 조각이 고아로
     신고된다.** → 따옴표 상태를 따라가며 자른다. 문서에 "문자열도 뺀다" 라고 적어 놨는데 코드는
     안 뺐던 것도 같이 고친다 — 주석과 코드가 어긋난 가드는 다음 사람이 잘못된 전제로 고친다.
- **곁가지**: 같은 기계로 **죽은 `@State`** 도 잡는다. 컴파일러는 안 쓰는 저장 프로퍼티에
  warning 을 내지 않아 `test-gate.sh` 의 warning 게이트도, 라인 커버리지도 못 본다
  (`BattleView` 가 방 배틀을 넘긴 뒤 `roomMode`·`multiplayerTargetID` 가 그렇게 남았다).
- **검증**: 세 가드 모두 **결함을 일부러 주입해** 빨강을 확인했다(중첩 `ScrollView` 재삽입,
  여러 줄 고아 함수, 죽은 `@State`). 주입 없이 초록만 본 가드는 아무것도 안 지킬 수 있다.
- (#209 후속, 2026-09-03.)

## 소스를 grep 하는 가드는 **이름이 바뀐 것만** 안다 — 조건이 뒤집힌 것은 못 본다

- **증상.** 레이드 출전 개체 선택(#80 후속)의 게이트를
  `let pick = activity == .raid ? nil : raidPickedMonID` 로 **뒤집어 주입했는데 2085개 테스트가
  전부 초록이었다.** 그 상태의 앱은 피커가 아무것도 안 바꾸고(레이드에 선택이 안 실린다), 동시에
  피커가 없는 화면(체육관·토너먼트·퀴즈·포켓슬론)에 레이드 선택이 조용히 출전한다 — 가드가 막겠다고
  적어 둔 결함 두 개가 한꺼번에 나간다. (코드리뷰 2026-09-03.)
- **직접원인.** 가드(`testThePickIsOnlyEverPassedOnRaidPaths`)가 `MultiplayerRoomCenter.swift` 를
  읽어 `raidPickedMonID` 가 나오는 줄을 세고, 각 줄이 `== .raid` 또는 `isRaidRoomName` 이라는
  **글자를 포함하는지**만 봤다. 뒤집힌 삼항식에도 그 글자는 그대로 있다. 반대로 무해한 줄바꿈
  (`? raidPickedMonID` 를 다음 줄로 내리기)이나 `raidPickedMonID` 를 쓰는 헬퍼 하나만 늘어도
  개수 검사가 빨개진다 — **놓치는 쪽과 헛뜨는 쪽이 동시에 틀렸다.**
- **테스트가 왜 못 걸렀나.** 가드를 소스 grep 으로 쓴 이유는 정당했다: 그 두 줄은 살아 있는
  `NWConnection` 이 있어야 밟히는 `createRoom`/`join` 안이라 `swift test` 가 못 간다. 문제는
  거기서 **판단까지 그 줄에 남겨 둔 것**이다. 실행할 수 없는 자리에 조건을 두면 남는 검사는
  글자 비교뿐이고, 글자 비교는 조건의 *방향*을 못 본다.
- **처방 — 검사할 수 없는 자리에는 판단을 두지 않는다.** 호출부는 자료만 넘기고
  (`Self.raidEntry(pick: raidPickedMonID, activity: activity)`), 레이드인지 아닌지와 스냅샷 레벨은
  `nonisolated static func raidEntry` 안에서 정한다. 그러면 뒤집기는 **실행되는 코드**를 고치는
  일이 되고, `RoomActivity.allCases` 를 도는 테스트가 전 분기를 밟는다. 소스 가드는 지웠다 —
  같은 것을 두 번 지키지 않는다.
- **영구 캡처.** `RaidRoomTests` 3종(활동별 게이트·파티 레벨·방 이름 게이트). 주입으로 확인했다:
  `activity != .raid` 로 뒤집으면 8건 실패, 파티 레벨을 빼면 2건 실패. 둘 다 이 변경 전에는 초록이었다.
- **일반화.** **소스 grep 가드는 "이 규칙이 어느 자리에 존재하는가"에만 쓴다**(예: 이 파일에
  ScrollView 가 없어야 한다, 이 필드가 서명 목록에 있어야 한다). *조건의 방향*·*값의 범위*처럼
  뒤집힐 수 있는 판단에는 쓰지 않는다 — 그건 순수 함수로 내려서 실행으로 검증한다. 판단을 못 옮길
  자리(진짜 I/O 안)라면, 가드가 무엇을 못 막는지 그 자리에 적는다.

## 사람만 지키는 릴리스 체크리스트 항목은 **조용히 새어나간다**

- **증상.** `scripts/build-app.sh` 의 `DEFAULT_VERSION` 이 `2.23.5` 인 채로 v2.24.0 이 나갔다.
  배포 산출물은 태그에서 버전을 주입받으므로 릴리스 자체는 멀쩡했고, **손으로 빌드한 앱만** 두 버전
  뒤처진 번호를 달았다 — 아무 게이트도 빨개지지 않아 다음 릴리스 준비 중에야 발견됐다.
  (v2.25.0 준비, 2026-09-03.)
- **직접원인.** 이 항목을 지키는 것이 `RELEASE.md` 체크리스트 한 줄과 사람뿐이었다. 예전
  `release.sh` 에 있던 문서·에셋 하드 게이트가 "태그 전용" 으로 축소되면서 함께 사라졌고
  (`release-workflow.md` §1 이 그 사실을 적어 두고 있다), 그 뒤로 지키는 쪽은 기억뿐이었다.
- **테스트가 왜 못 걸렀나.** 테스트가 볼 수 있는 값이 아니다. `DEFAULT_VERSION` 은 CI 경로에서
  **쓰이지 않는 값**이라 빌드·테스트·커버리지 어디에도 나타나지 않는다. 값이 틀렸다는 사실은 태그
  이름과 대조할 때만 드러나는데, 그 대조를 하는 코드가 없었다.
- **처방.** `release.sh` 가 태그를 만들기 전에 `DEFAULT_VERSION` 과 인자로 받은 버전을 비교하고,
  다르면 멈춘다. 값이 CI 에서 안 쓰인다는 이유로 검사까지 빼면, 그 값은 영원히 아무도 안 본다.
- **영구 캡처.** `scripts/release.sh` 의 `BUILD_DEFAULT` 게이트. 주입으로 양방향을 확인했다 —
  버전을 어긋나게 주면(`release.sh 9.9.9`) 게이트에서 멈추고, 맞추면 통과해 다음 검사로 넘어간다.
- **일반화.** **릴리스 산출물 중 "사람이 기억해서 올리는 값"은 전부 이 부류다.** 값이 배포
  경로에서 안 쓰일수록 더 위험하다 — 틀려도 아무것도 깨지지 않아 다음 릴리스까지 남는다. 체크리스트
...
- **범위를 넓히면 추출기의 근사가 드러난다(리뷰에서 발견, 같은 날 수정).** 한 파일에서는 안
  ...
  - **동명 mutator 상호 은폐 (#233, 2026-09-04 해소)**: 판정이 이름 단위 산술이라 같은 이름이
    여러 타입에 있으면 서로를 가렸다(`send` 9선언/120등장, `record` 6, `delete`·`tick`·
    `clamped` 3). 살아 있는 쪽의 호출부가 죽은 쪽의 0곳을 메운다. **grep 의 천장이었다** —
    `store.send(...)` 가 어느 `send` 선언으로 해석되는지는 타입 추론이 필요하고, swift-syntax
    만으로도 안 된다(선언 위치는 정밀해지지만 호출부 해석이 없어 이름 풀링이 그대로 남는다).
    해소한 방법: **SwiftPM 이 매 빌드마다 이미 쓰는 인덱스 스토어**(`bin-path/index/store`)를
    읽는다 — 선언마다 USR, 참조마다 그 USR 이 달려 있어 판정이 선언 위치 단위가 된다.
    판정 본체는 `Tools/MutatorGate`(앱 패키지와 분리 — 안 그러면 릴리스 빌드·서명 경로까지
    새 의존성을 탄다), 정책(예외 목록·`debug` 규칙)은 `scripts/mutator-sweep.sh` 가 쥔다.
    실측 비용: 의존성 cold 빌드 21초, 스윕 웜 5초. Periphery 는 상류가 archived(brew formula
    deprecated)라 CI 를 묶을 수 없어 뺐다.
  - **여기서 드러난 네 번째 비대칭 — `func` 이 아닌 멤버 안의 `save()`.** awk 를 지우자 후보
    집합이 세 군데 달라졌고 셋 다 추출기의 근사였다: ① `clamped` 는 유령이었다
    (`PokemonChat.swift:675` 의 `init(fileURL:)` 이 687행에서 `save()` 를 부르는데 394~687
    사이에 `func` 선언이 하나도 없어, **다른 타입**의 `clamped(x:y:)` 이름이 그 호출을 지고
    있었다) ② `merge` 도 유령이었다(`backfillMissingOwnedNames()` 안의 **중첩 지역 함수**)
    ③ 그래서 `backfillMissingOwnedNames()` 자신은 **구조적으로 못 보던 mutator** 였다 —
    자기 안의 `func merge` 가 `save()` 줄 앞에서 이름을 가로챘다. 호출부가 있어 죽지는
    않았지만, 게이트가 지금까지 그 함수를 한 번도 보지 않았다.
  - **남은 천장(미해소)**: ⓐ 프로토콜·상위 메서드 경유 호출은 요구사항 USR 로 기록되므로
    구현이 둘 이상이면 한쪽만 실제로 불려도 나머지가 살아 있어 보인다(동적 디스패치의 천장,
    periphery 도 같다) ⓑ 서로만 부르는 죽은 뭉치(A→B→A)는 양쪽 다 참조가 있어 통과한다
    ⓒ 접근자(`didSet`·set)의 `save()` 는 후보가 아니다(오늘의 클래스를 함수로 유지했다).
  - **인덱스로 옮기며 새로 생긴 실패 모드는 셋 다 가드하고 주입으로 확인했다.**
    낡은 인덱스를 읽으면 판정이 **조용히 거짓**이 된다 → 스윕이 먼저 빌드한다 / 스토어가
    없거나 비면 `exit 2` 로 세운다(빈 스토어를 "결함 0건" 으로 통과시키면 게이트가 아무것도
    안 보면서 초록이다 — 실측으로 없는 경로·빈 디렉터리 모두 exit 2) / 지워진 파일의 낡은
    유닛은 선언 줄을 읽을 수 없으면 건너뛴다(파일 있을 때 1건 → 파일만 옮기면 0건).
  - **판정을 스크립트로 굳혔다 — `scripts/verify-mutator-gate.sh`.** 결함 6종을 주입해
    종료코드로 대조한다(동명 죽은 mutator / 유일 이름 죽은 mutator / 호출부 있는 mutator /
    `debug` 접두 ±Tests 호출부 / 프로토콜 경유 mutator). 이름 산술 판정에서는 **정확히
    한 케이스(동명)만 빨강 기대에 초록**이었고 나머지 5개가 일치했다 — 하네스가 전부
    빨강으로 칠하는 것이 아니라 판별한다는 증거이자 #233 의 재현이다.
    하네스 자체도 **주입 파일이 있는지 먼저 검사**한다: 첫 판에서 cleanup 이 프로브
    디렉터리를 지워 케이스 2~5 가 주입 없이 돌았고 그중 셋이 초록으로 "통과" 했다 —
    하네스가 잡으려는 거짓 확신을 하네스가 낸 셈이라 같은 부류를 기계로 막았다.
...
- **증상**: 경매 신청자는 제안을 **한 번에 하나만** 걸 수 있었다. 답 없는 제안 하나가 제한
  시간(90초) 동안 다른 기회를 전부 막았다. (그 시간 제한 자체는 #227 이 없앴다 — 늦게 수락한
  게시자를 억울하게 끊었기 때문이다. 지금 대기 중인 제안을 접는 길은 수동 취소뿐이고, 그래서
  "제안을 여러 건 걸 수 있다" 가 더 필요해졌다.) 원인은 기능 부재가 아니라 국면 표현이다 —
  `PokemonAuctionCenter` 가 진행 중인 제안을 스칼라 8개(`outgoingConnectionID`·`outgoingPokemonID`·
  `outgoingStardust`·`outgoingStardustEscrowed`·`outgoingOfferID`·`outgoingListing`·
  `outgoingReceived`·`outgoingStatus`)로 들고 있어서, 두 번째 제안을 허용하면 앞 제안의 상태·
  타이머·에스크로를 통째로 덮었다. 그래서 `guard outgoingConnectionID == nil` 로 막아 둔 것이었다.
- **부류**: **"하나만" 가드가 진짜 제약이 아니라 국면 표현의 그림자인 경우.** 값을 배열로
  바꾸면 제약이 사라지므로, 그 가드가 지키던 다른 불변식이 무엇이었는지 먼저 세어야 한다.
- **여기서 드러난 것 — 상한 가드는 "같은 것을 두 번 내주지 않는다" 도 겸하고 있었다.** 제안이
  하나뿐이면 같은 개체를 두 자리에 걸 방법이 없다. 여럿이 서는 순간 그 보장이 사라지고, 둘 다
  수락되면 같은 포켓몬이 두 번 커밋된다(하나는 실패로 끝나지만 그 실패가 어느 쪽인지는 순서
  나름이다). **게시(`localListings`)와 제안 사이에는 그 구멍이 이미 열려 있었다** — 제안이
  하나뿐이던 때에도 내가 게시한 개체를 남에게 제안으로 또 걸 수 있었고, 기존 테스트
  (`cancellingMyListingLeavesMyOwnOfferAlone`)가 바로 그 픽스처였다. 상한을 풀 때는 **그 상한이
  우연히 지키고 있던 불변식**을 명시적 가드로 옮긴다(`PokemonAuctionCenter.isCommitted`, 게시와
  제안 두 호출부가 공유 — 호출부마다 두면 새 호출부가 무검사로 남는다).
- **에스크로처럼 "나중에 걷는" 값은 미결분을 세지 않으면 상한이 뚫린다.** 별의모래 에스크로는
  **수락 시점**에 걷히므로 등록만 보면 지갑을 넘는 제안을 여러 건 걸 수 있고, 뒤에 수락된 제안이
  잔액 부족으로 실패한다. 판정은 `availableTokens - (아직 안 걷힌 약속분)` 이다 — **이미 걷힌
  제안은 `availableTokens` 에서 빠져 있으니 두 번 세면 안 된다**(`!stardustEscrowed && status.isLive`).
- **와이어는 안 건드렸다.** 다섯 프레임 전부 이미 `offerID` 를 싣고 있어서 스칼라를 제안별
  국면으로 접는 것만으로 끝났고 `protocolVersion` 은 3 그대로다. 국면이 하나였던 이유가
  프로토콜이 아니라 **자기 쪽 표현**일 때는 버전을 올릴 필요가 없다 — 올리면 구버전 피어와의
  호환을 이유 없이 깬다.
- **프레임 라우팅은 연결만 보면 안 된다.** 제안마다 연결이 따로이므로 `connectionID` 만 맞춰도
  될 것 같지만, 대조는 **연결과 제안 ID 를 둘 다** 본다(`outgoingIndex`). 하나만 보면 지연 도착·
  재사용 프레임이 남의 제안 국면을 움직인다.
- **검증**: 새 가드 세 개(개체 이중 등록·게시된 개체 제안·별의모래 미결 합계)에 **결함을 일부러
  주입해** 빨강을 확인했고, `llvm-cov` 로 각 가드 줄의 실행 횟수를 직접 봤다(`^0` 없음).
  주입 없이 초록만 본 가드는 아무것도 안 지킬 수 있다.
- **뒤이은 리뷰가 같은 전환에서 더 찾은 것 넷.**
  - **에스크로를 되돌리는 버튼이 커밋 중에도 눌렸다.** `.accepted` 는 "내 별의모래는 이미
    걷혔고 상대는 곧 개체를 넘긴다" 는 뜻인데, 결과 카드의 "확인" 이 그 국면에도 떠서
    `clearOutgoingResult` 가 에스크로를 환불했다 — 게시자는 `completeAuctionSale` 로 이미
    지급받은 뒤라 **별의모래가 복제되고 개체는 아무에게도 가지 않는다.** 치우는 동작에는
    "끝난 것만" 이라는 가드가 붙어야 한다(센터와 화면 양쪽).
  - **같은 `.accepted` 가 또 오면 실패로 끌어내렸다.** 재전송·지연 도착 한 장이 진행 중인
    커밋을 되돌려 위와 같은 복제를 만든다. **되돌릴 수 없는 단계의 중복 프레임은 무시한다.**
  - **정원을 "살아 있는 제안" 이 아니라 배열 길이로 셌다.** 치우지 않은 결과 카드 여덟 장이
    시장을 통째로 잠그고, 버튼은 회색인데 이유는 어디에도 안 적힌다. 상한이 세는 대상은
    **자원을 붙들고 있는 것**이지 화면에 남은 것이 아니다.
  - **파생값을 화면이 한 벌 더 구현했다.** 미결 별의모래 합계·"이미 걸린 개체" 판정이
    센터와 뷰에 두 벌 있었다(`CompanionStore.canPerformTrade` 가 같은 이유로 정본 하나를
    고집한다). 한쪽만 넓어지면 **버튼은 켜지는데 센터가 조용히 거절**하고, 그때 화면이 고른
    값까지 지우면 아무 일도 없었던 것처럼 보인다. 정본은 센터
    (`unpledgedTokens`·`canRegisterOffer`·`isCommitted`), 화면은 그것만 읽는다.
  - 덧붙여 전역 `lastError` 는 이 전환에서 **지우는 자리를 전부 잃었다**(옛 `apply`·
    `clearOutgoingResult` 가 지우고 있었다). 한 번의 수락 실패가 세션 내내 빨간 줄로 남는다.
  - **검증**: 넷 다 결함을 일부러 되돌려 넣어 빨강을 확인했다(정확히 새 테스트 네 개만 실패).
  - **보내는 쪽이 수신 쪽 검증을 안 봤다.** `apply(offeringStardust:)` 가
    `SaveTransfer.maxTokenValue` 를 안 봐서, 상한을 넘는 제안은 게시자의 `isValid` 에서만
    걸려 **왕복 한 번이 통째로 거절로 버려졌다**(`creditStarPieces` 는 상한을 걸지 않으므로
    지갑이 상한을 넘은 세이브에서 실제로 밟힌다). 양쪽이 같은 상한을 보는 프로토콜에서는
    **보내기 전에** 자른다 — 받는 쪽 검증만 믿으면 원인 모를 거절이 남는다. 결함을 되돌려
    넣어 빨강을 확인했다.
- (경매 다중 제안, 2026-09-03.)

## 배열 첨자를 **스토어를 건드리는 호출 너머로** 들고 있으면, 관측자 하나가 남의 자리에 쓴다

- **증상**: 경매 신청자 국면(`outgoingOffers`)을 고치는 코드가 첨자 하나를 조회 시점부터 여러
  변이 동안 들고 있었다. `.accepted` 는 첨자 하나로 subscript 를 일곱 번 접근하면서 중간에
  `escrowStarPieces` → `save()` 를 지났고, `.completed` 는 `performTrade`·`receiveAuctionPokemon`
  (세이브·추억 앨범·대화 기록을 통째로 흔든다) 너머로 들고 있었고, `clearOutgoingResult` 는
  환불(`creditStarPieces` → `save()`) 너머로 들고 있다가 그 첨자로 `remove(at:)` 를 했다.
  받은 제안(`offers`)의 `.commit` 도 같은 모양이었다.
- **부류**: **첨자는 배열이 안 바뀐다는 약속 위에서만 유효한 좌표다.** 조회와 쓰기 사이에 스토어를
  건드리는 호출이 하나라도 끼면 그 약속은 코드 어디에도 적혀 있지 않은 전제가 된다. `@Observable`
  스토어에서는 저장 한 번이 관측자를 깨우므로, **관측자를 하나 붙이는 것만으로** 그 전제가 깨진다 —
  새 옵저버·새 콜백은 "아무것도 안 건드리는 변경" 처럼 보이기 때문에 더 위험하다.
- **결함은 조용하지 않고 돈이 샌다.** 첨자가 뒤로 밀리면 ⓐ `.accepted` 에서는 지갑에서 별의모래가
  빠졌는데 `stardustEscrowed` 표시가 **옆 제안**에 찍혀, 정작 낸 제안은 환불 경로에서 통째로
  건너뛴다(유실). ⓑ `.completed` 에서는 완료 표시가 옆 제안에 찍혀 낸 제안이 `.accepted` +
  에스크로인 채로 남고, 게시자는 이미 지급받았는데 신청자 쪽에서 환불이 한 번 더 나간다(복제).
  배열이 첨자보다 짧아지는 방향이면 subscript 가 그대로 터진다.
- **왜 리뷰가 못 걸렀나.** 리포트(#229)조차 "살아 있는 버그는 아니다 — 지금은 `outgoingOffers` 로
  되돌아오는 경로가 없다" 로 읽었다. 실제로는 **지갑을 보는 평범한 관측자 하나**로 재현된다.
  "재진입 경로가 없다" 는 코드베이스 전수 확인이 아니라 그 순간의 호출 그래프에 대한 인상이었고,
  `@Observable` 은 호출 그래프에 안 보이는 경로를 만든다.
- **영구 캡처 — 첨자를 표현할 수 없게 만든다.** ID 로 다시 찾는 규칙을 호출부마다 손으로 지키면
  새 호출부가 무검사로 남는다(`guard let index = …` 가 여섯 번 생기고, 그중 하나를 빼먹는다).
  첨자가 사는 자리를 **헬퍼 두 개 안으로** 가둔다: `PokemonAuctionCenter.withOutgoing(_:on:_:)`
  (제안 하나를 클로저 동안만 열어 준다)와 `setStatus(_:for:)`. 파일 전체에서
  `outgoingOffers[` · `offers[` 첨자는 그 두 함수 안에만 있다. 여는 본문 안에서 `companion` 을
  부르지 않는다 — 그 호출이 되돌아오면 같은 원소를 두 번 잡는다(이건 조용한 오작동이 아니라 트랩이라
  차라리 낫다).
- **회귀 테스트는 훅이 아니라 실제 `Observation` 으로 만든다.** 테스트 전용 훅을 프로덕션에 심으면
  그 훅이 프로덕션 경로의 사본이 되어 정작 실사용 경로가 무검증으로 남는다(이 문서의 "테스트가
  프로덕션 경로 대신 자기 디버그 훅의 사본을 밟는 부류"). `withObservationTracking { _ =
  store.state.starPieces } onChange:` 의 `onChange` 는 `state` 가 바뀌는 **그 자리에서 동기로**
  불리므로, 에스크로/커밋 정확히 한가운데서 앞 제안을 치울 수 있다. 관측자 안에서는 스토어를
  되건드리지 않는다(같은 값을 두 번 잡는다) — 별의모래가 0 인 제안을 치우면 된다.
- **첨자가 세 칸 이상 필요하다.** 제안 둘로는 앞을 치우면 배열이 한 칸이라 낡은 첨자가 그냥
  터진다(테스트가 죽어 실패 원인을 못 읽는다). 앞·낸 제안·뒤 셋을 세우면 표시가 **어느 제안에**
  잘못 찍혔는지가 그대로 단정문이 된다.
- **검증**: 두 테스트 다 결함을 일부러 주입해 빨강을 봤다 —
...
- **부류 스윕**: `Sources` 전체 `firstIndex` 30곳(`firstIndex(where:)` 28 + 트레일링 클로저 2 — 스윕을
  ...
  (`performTrade` 박스 분기)·`:1858`·`:2133` 은 사이에 `preserveDexRecord`·`memoryAlbum`
...
## 디스크 캐시가 시간(TTL)만 보면, 그 캐시를 만든 **조건**이 바뀐 것은 아무리 기다려도 안 반영된다

- **증상**: 종 범위를 1~9세대(#1025)까지 열었는데(#212), 기존에 앱을 쓰던 사용자에게는 알에서
  6~9세대 포켓몬이 계속 안 나왔다. 새로 설치한 사용자는 문제없었다.
- **원인**: `PokeAPIClient.baseSpeciesIndex()` 가 부화 후보 GraphQL 조회 결과를 `base-index.json`
  에 **30일 TTL로만** 캐싱했다. 종 범위를 649(5세대) → 1025(9세대)로 넓힌 커밋(#212)이 서버
  쿼리(`id: {_lte: maxID}`)는 정확히 고쳤지만, **이미 디스크에 있는 캐시가 그 범위 확장을 알
  방법이 없었다** — `fetchedAt` 만 보고 "아직 30일 안 지났으니 유효"로 계속 재사용됐다. 세션 재시작·
  앱 업데이트로는 안 지워지는 파일이라, 옛 범위로 받은 캐시를 든 사용자는 최대 30일간(또는 파일을
  손으로 지우기 전까지) 새 범위가 열린 걸 체감할 방법이 없었다.
- **왜 테스트가 못 걸렀나**: 이 캐시 경로를 도는 단위 테스트가 아예 없었다 — 실제 파일 I/O·네트워크가
  섞인 `actor` 메서드라 유닛 테스트 대상에서 빠져 있었다. "캐시가 유효한가"라는 판정 자체가 함수
  안에 인라인으로만 있어서, 판정 조건이 날짜 하나만 보는지 검증할 방법이 없었다.
- **부류**: `SaveTransfer.integrityVersion`(#206 lastCandyDate 사고)과 같은 모양이다 — **"이 데이터를
  만든 조건이 바뀌면 캐시/서명도 같이 바뀌어야 한다"** 는 불변식이, 값 자체가 아니라 **그 값을 낳은
  변수**(세이브는 canonical 구성, 여기는 조회 범위)에 걸려 있다. 시간 하나(TTL)나 값 하나(해시)만
  보면 "만든 조건이 바뀐 것"은 영원히 못 잡는다 — 그 조건 자체를 캐시에 같이 적어 두고 읽을 때
  대조해야 한다.
- **처방**: `BaseIndexSnapshot` 에 캐시를 만들 당시의 `PokemonAssets.speciesRange.upperBound` 를
  `maxSpeciesID` 로 함께 적는다. 읽을 때 지금 범위와 **다르면** TTL 이 안 지났어도 새로 받는다(단
  네트워크가 죽어 있으면 오프라인 폴백으로 범위가 달라도 오래된 캐시를 쓴다 — 없는 것보단 낫다).
  판정을 `isBaseIndexSnapshotUsable` 순수 함수로 뽑아 파일 I/O 없이 테스트한다.
- **검증**: `BaseIndexCacheTests` — 범위가 다른(옛 범위) 스냅샷은 방금 받은 것이어도(TTL 안 지남)
  `false`, 범위가 같으면 TTL 안에서 `true`, TTL 지나면 범위가 같아도 `false`. 범위 비교를 지우면
  첫 테스트가 빨강이 된다(고의 주입 확인).
- (`PokeAPIClient.baseSpeciesIndex`·`BaseIndexSnapshot`, 2026-09-04. 사용자 보고 "9세대까지
  추가했었는데 알에서 안 나오는거 같아"로 발견.)
## 프로세스 사이 우편함은 "읽고 지우기" 가 아니라 나이와 id 로 걸러야 하는 부류

- **파일로 부탁을 주고받을 때, 처리한 요청을 지우는 것은 답이 아니다.** 지우려면 받는 쪽이 보낸
  쪽의 파일에 손을 대야 하고, 그 순간 한 파일에 쓰기 주체가 둘이 된다 — 잠금이 없어서 파일 IPC 를
  고른 건데 같은 문제가 그 파일에서 되살아난다. 대신 **파일마다 쓰는 쪽을 하나로** 두고
  (`pokedoro-request.json` 은 터미널만, `pokedoro-reply.json` 은 앱만), 받는 쪽은 두 가드로 거른다.
- **가드 ①: 나이.** 받는 쪽이 꺼져 있던 사이 남은 요청을 그대로 실행하면, 사용자가 세 시간 전에
  포기한 90분 집중이 앱을 켜는 순간 시작된다. 사용자는 무엇 때문에 타이머가 켜졌는지 알 수 없다.
  **어긋남은 양쪽 대칭으로 본다** — 과거만 막으면 시각을 미래로 적은 요청 하나로 제한이 통째로
  우회된다(요청 파일은 그냥 JSON 이라 손으로 고칠 수 있다).
- **가드 ②: 마지막 실행 id.** 폴링 주기마다 파일이 그대로 남아 있으므로, 기억하지 않으면 같은
  요청이 매 틱 다시 실행된다(1초 틱 × 10초 = 열 번).
- **파일 내용은 신뢰경계다.** 명령 파서가 인자를 검증했다고 안심하면 안 된다 — 파일을 직접 고친
  경로는 파서를 지나지 않는다. 클램프는 **파서가 아니라 실행기 쪽**에 둔다.
- **왜 테스트가 못 걸렀을 뻔했나**: 조각별 테스트(우편함 왕복·실행 판정·실행기)는 각자 통과하면서
  배선 순서가 어긋날 수 있다. 받는 쪽이 앱 루트라 테스트가 안 닿기 때문에, **앱이 배선하는 순서
  그대로 밟는 왕복 테스트**를 따로 둔다(`testTheAppSideRoundTripFromRequestFileToReplyFile`).
- **검증**: 나이 검사를 `return true` 로 바꾸면 셋이 빨강(오래된·미래·경계).
  (`PokedoroRequestBus`·`PokedoroMailbox`, 2026-09-04.)

## 프런트엔드가 늘 때 거절 조건표가 프런트엔드 수만큼 복제되는 부류

- **같은 동작에 프런트엔드가 셋이 되면(화면 버튼·대화 도구·터미널), 조건을 세 벌 쓰게 된다.**
  한쪽만 고쳐도 아무 테스트가 안 깨진다 — 갈라진 걸 알아챌 방법은 손으로 맞대 보는 것뿐이다.
  실제로 대화 도구가 휴식 단계를 안 봐서 **화면이 못 하는 일을 대화만 할 수 있는** 구간이 있었다.
- **처방**: 사유를 enum 으로 내는 순수 판정표를 하나 두고(`PokedoroSessionGate`) 전 프런트엔드가
  그것만 읽는다. **문구는 표에 넣지 않는다** — 대화는 모델용 영문 한 줄, 터미널은 사람이 읽는
  한국어라 표현이 서로 다르다. 표는 사유만 내고 표현은 각자 한다.
- **제안 조건(`isReady`)도 같은 표를 읽게 한다.** 칩이 뜨는 규칙과 실행이 막히는 규칙이 갈라지면
  사용자는 앱이 권한 것을 눌렀다가 거절당한다. 의도적으로 좁힌 자리(`stopFocus`)는 **왜 좁혔는지**
  주석으로 남긴다 — 안 남기면 다음 사람이 "버그" 로 보고 넓힌다.
- **검증**: 시작 가드를 `phase == .focus` 로 좁히면 휴식 단독 분기가 빨강.
  (`PokedoroSessionGate`·`PokemonChatToolbox`, 2026-09-04.)

## 목록 행을 전체 폭으로 조립하면 커서 표식이 오른끝 값을 밀어내는 부류

- **폭을 맞추는 곳과 표식을 붙이는 곳이 다르면, 오른끝 정렬 값이 조용히 사라진다.**
  `TUIRender.list` 는 행 앞에 커서 표식(`"▸ "`, 2칸)을 붙이고 `TUIText.pad` 로 폭을 맞추는데,
  `pad` 는 넘치는 만큼 **오른쪽을 자른다**. 그래서 전체 폭으로 조립한 행은 라벨만 남고 개수·진행도가
  없어진다 — 잘린 것이 값이라 사용자는 무엇이 없어졌는지도 모른다.
- **왼쪽이 잘리는 부류와 다르다.** 라벨 truncate 는 좁은 터미널에서만 보이지만 이쪽은 **폭과 무관하게**
  항상 2칸이 밀리므로 넓은 화면에서도 일어난다. "80칸에서 확인했다" 가 방어가 되지 않는다.
- **처방**: 표식 칸을 상수로 두고(`listMarkerWidth`) 행을 만드는 쪽이 `listRowWidth(width)` 로
  조립한다. 값을 두 곳에서 손으로 세지 않는다 — 표식이 3칸이 되는 날 한쪽만 고쳐진다.
- **왜 테스트가 못 걸렀을 뻔했나**: 기존 렌더 테스트는 ⓐ 홈 화면의 줄 폭과 ⓑ `list` 의 줄 수·커서
  위치만 봤다. 둘 다 통과하면서 값이 사라질 수 있다 — 폭은 지켜지고(오히려 잘라서 지킨다) 커서도
  제자리다. **행 내용이 살아남는지**를 보는 축이 없었다.
- **검증**: `TUIRenderTests.testRightAlignedValuesSurviveTheCursorMarker` — `listRowWidth` 를
  `max(1, width)` 로 넓히면 그 테스트만 빨강이고, 실패 메시지가 증상 그대로다
  (`"▸ 이상한 사탕           "` — 값이 없다).
  (`TUIRender.list`·`listRowWidth`, 2026-09-04.)

## 이름과 인자를 따로 실어 "그 동작에 있을 수 없는 인자" 가 표현 가능해지는 부류

- **동작 이름과 그 인자를 별개 필드로 두면, 어느 쪽도 틀리지 않은 채 조합만 틀린 값이 만들어진다.**
  요청 파일이 `{"verb":"stop","minutes":90}` 인 경우가 그것이다 — 두 필드는 각각 유효하다.
  프로세스 사이 파일은 **손으로 고칠 수 있으므로** 이 조합은 이론이 아니라 실제 입력이다.
- **처방**: 인자를 든 enum 한 값으로 접는다(`PokedoroRequest.Action`). 표현할 수 없는 상태는
  걸러 낼 필요도 없다 — 검증 코드가 아니라 타입이 막는다. 인자 칸은 그것을 받는 동작에만 쓴다
  (`encodeIfPresent`): 남겨 두면 `{"action":"stop","minutes":null}` 이 정상으로 보이고, 파일을
  고치는 사용자가 그 칸이 뭔가 한다고 믿는다.
- **어긋난 조합은 "무시" 가 아니라 "요청이 아님" 이다.** 인자를 버리고 실행하면 사용자는 자기가
  적은 값이 무시된 걸 모른 채 다른 일이 벌어진 것을 본다. 디코딩을 실패시켜 **깨진 파일과 같은
  길**로 보내면 기존 가드(크래시 없이 nil)를 그대로 재사용한다.
- **평평한 JSON 은 계약이다.** 인자를 든 enum 을 Swift 의 자동 Codable 에 맡기면
  `{"action":{"start":{"minutes":25}}}` 로 중첩되는데, 문서가 약속한 모양이 아니라서 사용자가 고친
  파일이 조용히 무시된다. 그래서 `init(from:)`·`encode(to:)` 를 손으로 쓴다.
- **왜 테스트가 못 걸렀을 뻔했나**: 왕복 테스트가 **인자를 든 동작 하나**(`start`)만 보냈다. 그
  하나만 보면 인코딩이 그 동작에만 맞는 구현도 초록이고, 조합이 어긋난 파일은 아무도 안 만들어
  봤다. 이제 세 동작 전부 왕복하고, 파일을 직접 써서 어긋난 조합을 넣는 테스트를 따로 둔다.
- **검증**: `init?(name:minutes:)` 에서 `where minutes == nil` 을 떼면 `minutes:90` 이 실린 파일이
...
## 새 화면이 "그 밖의 화면" 분기에 얹히면, 옛 화면의 키를 조용히 물려받는 부류

- **`if 목록이면 … else 홈` 처럼 부정형으로 갈라 둔 분기에 화면을 하나 더 두면, 그 화면은 아무
  선언 없이 `else` 쪽의 키 표를 그대로 받는다.** 웨이브 런 화면을 넣자 기술을 고르는 `1` 이 홈의
  집중 세션 25분을 켰다 — 컴파일도 되고 읽기도 자연스러운데 눌러 보기 전엔 드러나지 않는다.
- 같은 부정형이 `TUIScreen.isList = (self != .home)` 에도 있었다. 판 화면을 더할 때마다 그 화면이
  **목록으로 분류돼** 아무 데도 안 쓰이는 커서가 움직인다.
- **처방**: 갈래를 **열거한다.** `switch screen { case .home: … case .wave: … case 목록들: … }` 로
  두면 화면을 더할 때 컴파일러가 그 자리를 막는다. 불리언 파생 값도 부정형(`!= .home`)이 아니라
  양쪽을 다 적는다.
- **검증**: 새 화면의 숫자 키가 **옛 화면의 동작이 아님**을 못 박는다
  (`testNumbersOnTheWaveScreenAreNotFocusLengths`). 화면 키 중복도 함께 본다
  (`testNoTwoScreensShareAKey`) — 겹치면 뒤에 선언된 화면에 영영 못 간다. 이 가드는 분기를 `case
  .wave, .home:` 으로 되돌려 **실제로 빨강을 확인**했다.
  (`TUIKeymap`·`TUIScreen`, 2026-09-04.)

## 진행이 "그 국면에서 받는 동작이 없는" 상태로 멈추면, 되살리는 자리를 따로 둬야 하는 부류

- **웨이브 런의 `.loadingWave` 처럼 사용자가 보낼 동작이 하나도 없는 국면은, 그 국면으로 들어가는
  코드가 실패하면 영영 멈춘다.** 앱 화면은 `.task { resume() }` 가 창을 열 때마다 이어 열어서 이
  문제가 보이지 않았는데, 터미널은 창이 없다 — 판이 그 상태로 서면 어떤 명령도 그것을 풀 수 없다.
- 더 나쁜 건 **여는 코드가 화면의 `private`** 이었던 점이다. 터미널에서 판을 시작조차 못 했고,
  시작해도 누군가 팝오버를 열 때까지 진행되지 않았다("앱이 서버" 인데 서버 코드가 뷰에 있었다).
- **처방**: ⓐ 그 절차를 화면 밖으로 꺼내 두 입구가 함께 쓴다(`WaveRunLoader`). ⓑ **모든 동작의
  머리에서** 멈춘 상태를 이어 받는다(`PokedoroRequestExecutor.currentRun()`) — 실패해도 상태는
  그대로 두고 다음 호출이 다시 시도한다.
- **덤으로 테스트가 열린다.** 화면이 `PokeAPIClient.shared` 를 직접 부르던 동안은 판을 여는 경로
  전체가 단위 테스트 밖이었다. 스토어의 `provider` 를 지나게 하고(`wildSnapshot`·`evolutionLine`,
  `moveSet` 을 `PokeProviding` 으로 올렸다) 나서야 네트워크 없이 돈다.
- **검증**: `testARunStalledWhileLoadingIsResumedByTheNextAction` — 되살림을 지워 빨강을 확인했다.
  (`WaveRunLoader`·`PokedoroRequestExecutor`, 2026-09-04.)

## 거절 문구가 **사실이 아닌 것을 주장하면**, 사용자는 맞는 사용법까지 의심한다

- `wave move 1 2 3` 이 "`wave move` 은 **인자를 받지 않는다**" 로 거절됐다. 그 명령은 인자를 받고
  `wave move 1 2` 는 정상 입력이다 — 문구를 믿은 사용자는 맞는 사용법을 버린다. 앞서 `switch 0` 이
  "숫자가 **아니다**" 로 거절된 것과 같은 부류다(0 은 숫자다, 목록에 없는 번호일 뿐이다).
- 원인은 **사유 하나를 두 뜻에 겹쳐 쓴 것**이다. "인자를 안 받는다" 와 "개수가 넘쳤다" 는 사용자가
  고쳐야 할 것이 다르므로 갈라야 한다.
- **테스트가 왜 못 걸렀나**: 파서 테스트는 *던지는지*만 봤다. 문구는 사람이 읽는 값이라 `throws:`
  단정으로는 내용이 검증되지 않는다. **빌드한 바이너리를 실제로 쳐 보고** 나서야 드러났다(지난
  단계의 `switch 0` 도 같은 방식으로 잡혔다).
- **처방**: 사유마다 case 를 두고, 두 문구가 **서로 다름**을 테스트로 못 박는다
  (`testTooManyArgumentsDoesNotClaimTheCommandTakesNone`). 그리고 새 명령을 더할 때마다 잘못된
  입력 몇 개를 실제 바이너리로 쳐 본다 — 순수 테스트가 전부 초록인 상태에서도 문구는 틀릴 수 있다.
- **곁가지**: 도움말의 왼쪽 칸을 상수(21)로 두었더니 `wave route <safe|risky>`(23칸)가 `pad` 에
  **잘려** 없는 명령을 알려 줬다. 칸 폭은 표에서 잰다(`rows.map(displayWidth).max() + 2`).
  (`PokedoroCommandError`·`PokedoroCommandParser`, 2026-09-04.)

## "바뀔 때만 쓴다" 캐시는 **안 바뀌는 것이 정상인 화면**에서 살아 있음 신호까지 끊는다

- 프로세스 사이 화면 채널을 ⓐ 내용이 바뀔 때만 쓰고 ⓑ 파일 나이로 낡음을 판정하도록 두면, **내용이
  오래 그대로인 것이 정상인 화면**은 반드시 한 번 낡은 상태를 지난다. 그 순간 읽는 쪽은 진행 중인
  것을 지운다 — LAN 대전 화면이 상대를 기다리는 30초 동안 터미널에서 사라졌다.
- **왜 첫 생산자에서 안 드러났나**: 집중 타이머는 매초 글자(`12:34`)가 바뀌어 늘 다시 쓰였다.
  두 규칙의 충돌이 **생산자의 성질에 가려져** 있었고, 성질이 다른 두 번째 생산자가 붙는 순간 드러났다.
- **처방**: 쓰기 판정에 **하트비트**를 넣는다 — 내용이 같아도 `refreshInterval` 마다 한 번은 쓴다.
  그 값은 낡음 한계보다 **짧아야** 한다(같거나 길면 경계에서 여전히 한 번 걸린다).
- **검증**: 주기 직전엔 안 쓰고 주기에 닿으면 쓰는 것을 함께 본다(`testAnUnchangedScreenIsRefreshed
  BeforeItGoesStale`), 그리고 **두 상수의 관계**를 따로 못 박는다(`testTheRefreshIntervalStaysUnder
  TheStaleLimit`) — 나중에 한쪽만 조정하면 그 테스트가 먼저 깨진다. 하트비트를 지워 빨강을 확인했다.
  (`PokedoroViewChannel`, 2026-09-04.)

## 한 줄에 이어 붙인 뒤 자르는 안내는, 목록이 늘면 **필수 항목부터** 조용히 사라진다

- 키 안내를 `entries.joined()` + `truncate(width)` 로 만들면 넘친 오른쪽이 말없이 버려진다. 화면을
  하나 더한 순간(웨이브 → 대전) 이동 키 목록이 70칸을 넘겨 **`q 종료` 가 잘려 나갔다** — raw mode 에
  들어간 사용자가 나갈 방법을 화면에서 찾을 수 없게 된다.
- **부류의 특징은 "늘 때만 터진다"** 는 것이다. 목록이 짧던 동안은 잘림이 없어 테스트도 사람도
  못 본다. 도움말 왼쪽 칸을 상수 21 로 두었다가 `wave route <…>` 에서 잘린 것과 같은 부류다 —
  **폭이 걸린 레이아웃은 항목 수에 따라 조용히 깨진다.**
- **처방**: 자르지 말고 **접는다**(`TUIRender.hintLines`). 조각 단위로 폭에 맞춰 여러 줄로 나누고,
  조각 하나가 폭보다 길 때만 그 조각을 자른다. 그리고 **나가는 키를 앞에 둔다** — 아주 좁은 폭에서도
  첫 줄에 남는다.
- **검증**: 여러 폭(40·60·70·100)에서 ⓐ 모든 줄이 폭 안에 들고 ⓑ `q 종료` 가 살아 있는지 본다
  (`testTheQuitKeyStaysVisibleAsScreensAreAdded`). 접기 자체도 조각이 하나도 빠지지 않는지로 본다.
  한 줄로 되돌려 빨강을 확인했다.
  (`TUIRender.hintLines`·`screenHintLines`, 2026-09-04.)

## 전역 단축키를 더할 때 **화면 안쪽 키와의 충돌**은 컴파일도 리뷰도 못 잡는다

- 화면 이동 키(`TUIScreen.key`)는 화면별 키 표보다 **먼저** 해석된다. 그래서 새 화면에 이미
  어딘가에서 쓰이는 글자를 주면 그 화면의 동작이 조용히 죽는다 — 교환 화면에 `t` 를 줬더니
  웨이브 런의 볼 던지기(`t`)가 눌러도 화면만 바뀌는 키가 됐다.
- **중복 검사만으로는 안 잡힌다.** 이동 키끼리는 겹치지 않았다(`testNoTwoScreensShareAKey` 통과).
  겹친 것은 이동 키 하나와 **다른 화면 안쪽 키**였고, 그 둘은 서로 다른 표에 살아 비교되지 않았다.
- **처방**: 전 조합을 훑는 가드를 둔다 — 모든 화면 × 모든 이동 키에서 결과가 **반드시 `.show`**
  여야 한다(`testNoScreenKeyShadowsAnInScreenKey`). 새 화면이나 새 화면 내 키를 더할 때 이
  테스트가 먼저 깨진다. 목록에 손으로 적는 형태로 두면 다음 화면에서 또 빠뜨린다.
- 앞선 두 부류와 같은 뿌리다: **표가 둘로 갈리는 순간 그 사이의 관계는 아무도 안 본다**
  (거절 조건표·아이템 이름·상점 목록에서 이미 겪었다).
  (`TUIScreen.key`·`TUIKeymap`, 2026-09-04.)

## 고정된 우선순위로 화면을 고르면, **동시에 참인 두 화면** 중 뒤에 있는 것은 영영 안 나온다

- 프로세스 사이 화면 채널이 한 번에 한 화면만 나르고 그 선택을 **순위 하나로** 하면, 후보가 서로
  배타적인 동안은 잘 돈다. 경매가 그 가정을 깼다 — 경매 시장은 이웃이 하나만 올려 둬도 참이라
  판처럼 서고 지지 않는다. **앞에 두면 게시물 하나가 집중 타이머를 영영 가리고, 뒤에 두면 그
  화면을 볼 방법이 아예 없다.** 어느 자리에 놓아도 고장이라는 것이 이 부류의 표식이다.
- **왜 앞선 셋에서 안 드러났나**: 대전·방·교환은 판이 서고 지므로 한 번에 하나만 참이었다. 순위와
  배타성이 우연히 같은 답을 내던 것이고, 성질이 다른 네 번째 생산자가 붙는 순간 갈라졌다
  (하트비트 부류와 같은 모양 — **규칙의 결함이 생산자의 성질에 가려져** 있었다).
- **처방**: 읽는 쪽이 **무엇을 보고 있는지 말한다**(`PokedoroAttachment.screen` →
  `preferred(_:wanted:)`). 요청한 화면이 지금 없으면 순위로 되돌아간다 — 빈 화면을 내놓으면 앱이
  죽은 것과 구분되지 않는다. 새 칸은 **옵셔널로 넣는다**: 필수로 두면 새 앱이 옛 신호를 통째로
  못 읽어 화면이 죽는다.
- **검증**: 순위로 고르는 경우·요청이 이기는 경우·요청한 화면이 없어 순위로 되돌아가는 경우를
  각각 본다(`testTheTerminalCanAskForTheScreenItIsShowing`), 그리고 **칸이 없는 옛 파일이 디코딩
  되는지**를 따로 못 박는다(`testTheAttachmentCarriesTheScreenAndOldFilesStillDecode`). 요청 처리를
  지워 빨강을, 칸을 필수로 바꿔 또 한 번 빨강을 확인했다.
  (`PokedoroViewChannel.preferred`·`PokedoroAttachment`, 2026-09-05.)

## 번호 공간이 셋 이상이면 **번호를 접는 자리**가 목록마다 하나여야 한다

- 화면 하나가 여러 목록을 함께 보여 주면 같은 `1` 이 목록마다 다른 것을 뜻한다. 경매는 넷이다
  (내 개체·근처 게시물·받은 제안·내가 건 제안). 한 목록의 번호가 다른 목록에서 풀리면 사용자가
  고른 것과 **다른 게시물·다른 제안**에 손이 간다 — 그리고 그건 개체 소유권이 오가는 자리다.
- **테스트가 그냥 통과할 수 있는 부류다.** 목록 셋의 첫 항목이 우연히 같은 자리에 있으면 잘못
  접은 것도 값이 맞는다. 그래서 검증은 **세 id 가 서로 다름을 먼저 못 박고**(`Set(...).count == 3`)
  그 위에서 각 접기를 본다 — 그 한 줄이 없으면 위의 세 검사는 아무것도 증명하지 못한다.
- **처방**: 목록마다 접는 함수 하나(`AuctionScreen.listingID`·`incomingID`·`outgoingID`)와,
  개체 번호는 로스터를 아는 창구가 접는다. 명령 이름이 **어느 목록의 번호인지 말하고**, 거절
  문구는 어느 목록을 봐야 하는지와 **몇 건이 있는지** 함께 말한다. 번호 오류 타입도 갈라 둔다 —
  `party` 를 보라고 하면 다른 목록을 뒤진다.
- **키를 주지 않는 것도 처방이다.** 숫자 한 자리로는 넷 중 어느 것인지 정할 수 없고 되물을 입력
  줄도 없다. 대신 **칠 명령을 줄에 싣는다** — 키 안내만 두면 그 화면은 영영 빈 줄을 그린다.
- 접기를 다른 목록으로 바꿔 빨강을 확인했다.
  (`AuctionScreen`·`TerminalAuctionControl`, 2026-09-05.)

## 읽기 전용은 **그 객체가 만드는 옆 파일까지** 가야 한다

- `CompanionStore(isReadOnly:)` 는 Phase 0 부터 있었고 자기 세이브를 정확히 지켰다. 그런데 그
  스토어는 **파일 셋**을 만든다 — 세이브·기억 앨범(`memory.json`)·대화 기록(`chat.json`) —
  그리고 뒤 둘에는 가드가 **아예 없었다.** 읽기 전용으로 연 터미널이 앨범 mutator 를 한 번만
  부르면 앱이 켜져 있는 동안 그 파일을 덮는다(잠금이 없는 것은 세 파일이 다 같다).
- **왜 다섯 단계 동안 안 드러났나**: 터미널이 앨범을 부른 적이 없었다. "지금 아무도 안 부른다"
  는 이유로 안전했던 것이고, 그건 안전이 아니라 **아직 안 밟은 것**이다. 실제로 앨범을 건드리는
  첫 단계(Memory Home)에서 그대로 밟혔다.
- **처방**: 가드를 **각 `save()` 안**에 둔다(호출부에 두지 않는다 — 앨범만 15곳이고 늘기만
  한다), 그리고 소유자에서 **전파**한다. 읽기는 막지 않는다: 막을 대상은 쓰기뿐이고, 읽기를
  막으면 이 프런트엔드의 쓸모(앱 없이 조회)가 사라진다.
- **검증**: 쓰기 가능한 객체로 파일을 만들고, 읽기 전용 객체로 mutator 를 여러 개 부른 뒤
  **바이트가 같은지** 본다(`testAReadOnlyAlbumCannotWriteItsFile`). 소유자를 통해 얻은
  객체에도 같은 검사를 한 번 더 한다 — 하나만 막으면 다음 호출부가 소유자를 지나 무가드
  객체를 얻는다. 가드를 지워 빨강을 확인했다.
- 부류의 이름: **"플래그가 객체 경계에서 멈춘다."** 소유 관계로 만들어지는 자원이 있으면
  플래그도 그 관계를 따라가야 하고, 안 따라가면 조용히 절반만 지킨다.
  (`PokemonMemoryAlbum`·`PokemonChatStore`·`CompanionStore`, 2026-09-05.)

## `guard … else { return }` 로 시작하는 테스트는 **아무것도 안 지킨 채 초록**일 수 있다

- 픽스처가 조건을 못 채우면 조용히 통과하는 테스트를 썼다 — `guard entries.count >= 2 else
  { return }`. 새 세이브에 개체가 하나뿐이라 그 테스트는 **본문을 한 줄도 밟지 않고** 초록이었고,
  룸메이트 처리 전체가 무테스트로 나갈 뻔했다.
- **커버리지 퍼센트는 이것을 못 잡는다.** 게이트는 통과했고 숫자도 높았다. 드러낸 것은
  `llvm-cov show --show-line-counts-or-regions` 로 읽은 `^0` 줄이다 — 함수 본문이 통째로 0회다.
- **처방**: 픽스처가 조건을 **세운다**(개체 다섯을 만들어 정원까지 넘겨 본다). 조건을 세울 수
  없어 정말 건너뛰어야 하면, 건너뛰었다는 사실 자체를 검증으로 남긴다
  (`#expect(others.count > roommateLimit, "정원을 넘겨 볼 수 있어야 이 테스트가 뜻을 갖는다")`).
- 형제 규칙: 새 조건 분기를 넣었으면 **`^0` 을 직접 본다.** 이 저장소에서 그 절차가 잡은 것이
  도감 이로치 집계(87% 통과, 무테스트)·경매 거절 문구 여섯·이 항목이다.
  (`MemoryHomeExecutorTests`, 2026-09-05.)

## 열거만 하고 투영하지 않은 `case` 는 **컴파일러만 만족시킨다**

- `RoomScreen.kind` 의 `switch` 가 `.battling, .pokeathlon, .pokemonQuiz, .tournament` 를 한
  묶음으로 받았다. 컴파일은 통과했고 리뷰도 통과했다 — 그 넷이 실제로 덮이는 것처럼 **보였기**
  때문이다. 그런데 뒤 셋의 판은 `combatFighters` 에 없다(각자 `tournamentState.currentMatch`·
  `pokeathlonRace`·`pokemonQuizGame`). 빈 목록으로 떨어져 "판을 준비하는 중이다" 가 판이 끝날
  때까지 남았다.
- **계획서가 그 거짓말을 그대로 물려받았다.** 다음 단계 설계 노트에 "전투 턴은 이미 덮는다,
  각자의 투영만 남았다" 고 적혀 있었다. 코드가 아니라 `case` 목록을 읽은 결과다.
- **처방**: 망라형 `switch` 에 케이스를 **묶어 넣기 전에**, 그 케이스가 같은 자료를 보는지
  확인한다. 자료가 다르면 묶지 말고 갈라서 `fatalError` 든 TODO 든 **눈에 보이는 자리**를
  남긴다. 조용히 이웃 케이스의 동작을 물려받는 것이 가장 오래 사는 결함이다.
- 형제 규칙: 국면(`phase`)으로 화면을 가르기 전에 **판의 유무**를 먼저 본다. 체육관은 판이
  도는 동안 `phase` 가 `.hosting` 그대로라, 국면부터 보면 판 중에 로비를 그린다.
  (`ArenaTerminalTests`, 2026-09-05.)

## 활동마다 다른 함수를 부르는 자리에서, **한 활동의 함수만** 부르면 나머지는 조용히 무동작이다

- `pokedoro room start` 가 활동을 안 보고 `startRaid()` 를 불렀다. 그 함수는
  `lobby.activity == .raid` 를 먼저 보고 아니면 그냥 돌아 나오는데, 실행기는 **"판을 시작했다"**
  고 답했다. 없던 성공을 보고한 것이고, 방 대전·토너먼트·포켓슬론·퀴즈 전부 그 상태였다.
- 화면도 같은 거짓말을 했다 — `RoomScreen.keys` 가 `isHost && canStart` 만 보고 `s 시작` 을
  권했다. 체육관은 호스트가 시작하는 방이 아닌데도 키가 떴다.
- **처방**: 활동 → 함수 매핑을 **표 하나**로 두고(`MultiplayerRoomCenter.starter(for:)`),
  화면이 키를 낼지 정하는 사실(`RoomActivity.isHostStarted`)과 **짝을 검사한다.**
- **함수 본문 안의 `switch` 는 밖에서 검사할 수 없다.** 처음엔 `startActivity()` 안에 `switch`
  로 뒀는데, `.battle` 갈래를 `break` 로 바꿔도 **전 스위트가 초록**이었다(주입으로 확인). 실행기
  테스트는 가짜 창구를 쓰고 진짜 시작 함수는 소켓·스냅샷이 필요해 효과를 볼 수 없기 때문이다.
  값으로 돌려주는 표로 바꾸고 나서야 같은 주입이 빨강이 됐다.
  (`ArenaSeamTests.testEveryHostStartedActivityHasAStarter`, 2026-09-05.)

## 외부 데이터의 "0" 은 값이 아니라 **없음**일 수 있다 — 그대로 읽으면 기능이 조용히 죽는다

PokéAPI 는 안 빗나가는 기술의 명중률을 null 이 아니라 **0** 으로 준다(타키온커터·파이어월·
드래곤치어). `MoveSpec.from` 이 그 값을 그대로 실어 명중 0% 가 됐고, 해당 기술은 영영 빗나갔다.
같은 응답에서 하드프레스는 위력을 **0** 으로 준다(가변 위력 기술인데 `null` 이 아니다) — 위력 0 은
데미지 0 으로 접히고, 데미지 0 이면 `applyAttack` 이 이벤트를 안 내서 화면에 기술명 한 줄만 남는다.

- **판단 기준**: 그 필드에서 0 이 **실제로 존재할 수 있는 값인가**. 명중 0% 인 기술은 도감에 없고,
  위력 0 인 공격기도 없다. 존재할 수 없으면 0 은 "값 없음"이지 값이 아니다.
- 규칙은 **한 곳에 두고 판정도 같은 곳을 본다**(`MoveSpec.neverMisses`). 디코딩만 고치면 이미
  0 이 박힌 옛 세이브와 구버전 피어의 데이터는 계속 틀린 채로 돈다.
- **범위 검사가 그 0 을 반려하면 피해가 번진다.** `validMoves` 는 무브셋 단위로 반려하므로 그런
  기술 하나가 팀 전체를 입장에서 막았다. 정상 값을 반려하는 가드는 가드가 아니라 결함이다.

## 외부 데이터 소스의 **커버리지 경계**는 조용하다 — 빈 값과 구분되지 않는다

PokéAPI 의 `move_meta` 테이블은 7세대에서 끊긴다. 8세대 이후 177개 중 92개는 meta 행이 **아예 없어**
다단 히트·드레인·상태이상·풀린치·급소 보정이 전부 nil 로 디코딩된다. 파퓰레이션밤은 10히트가
1히트가 되고(위력 20 짜리 잡기술), 솔트큐어·사이코노이즈는 부가효과가 통째로 사라진다.
에러도 로그도 없다 — "효과가 없는 기술"과 "데이터가 없는 기술"이 같은 모양이기 때문이다.

- 새 세대·새 지역의 데이터를 읽기 시작하면 **커버리지를 전수로 세어 본다.** 표본 몇 개가
  멀쩡하다고 경계가 없는 것이 아니다(`scripts/extract-showdown-moves.mjs` 가 그 계수기다).
- 소스가 못 주는 값은 **보정층으로 메우고 출처를 고지한다**(`ShowdownMoveOverrides.swift`,
  쇼다운 MIT). 보정은 **메우기지 덮어쓰기가 아니다** — 원 소스가 준 값은 그대로 둔다.
- 보정 대상이 늘면 `rulesVersion` 을 함께 본다. 단발기가 다단기가 되면 **rng 소비 횟수가 갈려**
  그 뒤 모든 판정이 밀린다(값이 아니라 경로가 바뀌는 부류).

## 필드를 **갈라 두고** 옛 칸을 읽는 화면을 남기면, 두 프런트엔드가 다른 숫자를 보여 준다

- 현행 체육관 리그는 첫 승리를 `gymLeagueBadges` 에 적는다(`recordGymVictory`). 이전 배포의
  `gymBadges` 와 겹치지 않게 **일부러 갈라 둔** 칸이다. 앱의 `GymLeagueView` 는 새 칸을 읽는데
  `pokedoro challenge` 는 옛 칸을 셌다 — 체육관을 이겨도 배지 줄이 안 움직이고, 옛 세이브를
  가진 사용자에게는 이긴 적 없는 배지가 잡혔다. 같은 파일을 두고 두 화면이 다른 답을 냈다.
- 왜 안 걸렸나: 그 줄을 보는 테스트가 **하나도 없었다**(Phase 1 이 읽기 전용 화면을 낼 때 렌더만
  봤다). 그리고 옛 칸은 이름이 그럴듯해서, 읽는 쪽에서는 틀렸다는 신호가 전혀 없다.
- **처방**: 칸을 가를 때 **옛 칸을 읽는 자리를 전수 grep** 한다(`gymBadges` 는 세이브 이전
  코드와 이 한 줄뿐이었다). 그리고 화면이 세이브 값을 세는 자리는 **값을 바꿔 놓고 줄이
  움직이는지** 본다 — 상수만 비교하는 테스트는 옛 칸을 세도 통과한다.
  (`GymLeagueTerminalTests`, 2026-09-05.)

## 한 개짜리 함수를 `allSatisfy` 로 N 번 돌리면 **트랜잭션이 아니다** — 도중에 멈춘 자리의 값은 안 돌아온다

- `pokedoro buy egg 5` 를 `(0..<quantity).allSatisfy { companion.buyEgg(nil) }` 로 처리했다.
  `allSatisfy` 는 **첫 false 에서 단락**하는데, 그 시점엔 앞선 호출들이 이미 별의조각을 깎고
  알을 넣은 뒤다. 저장고가 997 개인 상태로 5 개를 사면 두 개가 들어가고 세 번째가 상한에
  막혀, 답은 "지금은 신비한 알을 살 수 없다" 인데 **지갑은 6 만 줄어 있다.**
- 바로 옆 `CompanionStore.buy(_:quantity:)` 는 정반대 계약을 주석으로 못박아 두고 있었다 —
  "전량 구매 가능할 때만 처리한다 … 부분 구매를 하지 않는다". 알만 그 계약 밖에 있었다.
- 왜 안 걸렸나: 구매 테스트가 **잔액 부족**(총액 가드가 먼저 잡는 경로)과 **여유 있는 성공**만
  봤다. 부분 실패는 "총액은 되는데 상한이 모자란" 좁은 창에서만 열리고, 그 창을 밟는
  fixture 가 없었다. `allSatisfy` 가 Bool 을 돌려주니 반환값 검사도 **통과하는 것처럼 보인다.**
- **처방 — 수량은 반복이 아니라 인자다.** 수량 구매는 `canBuy…(_:quantity:)` 가 전량 기준으로
  한 번 판정하고 실행이 한 번 일어나게 둔다. 반복 호출로 수량을 흉내내면 실패 지점마다
  롤백 코드가 필요해지고, 그 롤백은 아무도 쓰지 않는다.
- **부류 스윕**: 코드베이스의 `allSatisfy` 30 곳 중 **부수효과를 가진 것은 이 한 곳뿐**이었다
  (나머지는 전부 순수 술어). 새로 쓸 때도 `allSatisfy`/`contains`/`first(where:)` 의 클로저
  안에서 상태를 바꾸지 않는다 — 단락 평가가 곧 "몇 번 돌았는지 모른다" 는 뜻이다.
- 곁가지로 드러난 것: 같은 상한(`999`)이 `canBuyEgg`·`addStoredEggs`·`SaveTransfer.sanitized`
  세 곳에 리터럴로 박혀 있었다. `CompanionStore.storedEggLimit` 로 모은다. 액터 밖(정규화)에서
  읽으므로 `nonisolated` 여야 한다 — 액터에 묶으면 세 번째 사본이 다시 생긴다.
  (`FreshEggTests.testBuyingMoreEggsThanFitChargesNothing`, 2026-09-05.)

## 화면이 안내하는 번호와 실행기가 받는 번호가 갈리면, **키는 있는데 눌러도 아무 일이 없다**

- 터미널 화면들이 `numbers(_:)` 로 "누를 수 있는 번호"를 계산해 두고, 정작 안내 문구(`keys`·
  `hints`)는 `1-\(team.count)` / `1-4` 같은 **범위**를 찍었다. PP 가 떨어진 기술, 쓰러진 멤버,
  현재 나와 있는 자리가 `numbers` 에서 빠져도 문구는 그대로였다. 기술 2 번만 PP 가 남았는데
  "1-1 기술" 이라고 안내한 자리도 있었다(`RoomScreen`) — 유일하게 쓸 수 있는 번호가 안내에
  아예 없다.
- 같은 부류가 **네 화면 중 셋**에 있었다(`WaveRunScreen`·`NetBattleScreen`·`RoomScreen`).
  `ArenaScreen.duelKeys` 만 이 함정을 주석으로 적어 두고 피해 갔는데, 그 주석이 형제 화면으로
  옮겨가지 않았다.
- 왜 안 걸렸나: 렌더 테스트가 **문구가 나오는지**만 봤다. "1-3 기술" 은 어떤 경우에도 나오므로
  통과한다. 번호와 실행 가능성을 **짝지어 보는** 테스트가 없었다.
- **처방 — 안내는 `numbers()` 에서 파생시킨다.** 사람이 범위를 다시 적지 않는다. 불변식은
  "제안 ⊆ 실행 가능" 이고, 테스트는 안내에 찍힌 **모든 숫자를 실제로 눌러** 거절이 없는지
  본다(빈틈이 있는 목록 — `[1,3,4]` — 을 만드는 fixture 가 필수다. 연속 범위만 쓰면 `1-N`
  축약이 정답과 우연히 같아진다).
- 형제 규칙: `watch` 의 숫자 키도 같은 문제였다. 화면 종류만 보고 `battle.move`/`room.move` 로
  고정 매핑하면, 앱이 광고한 "1-2 교체"·"1 왼쪽" 같은 키는 전부 죽은 키가 된다. 앱이
  **숫자마다 무슨 동작인지**(`PokedoroViewSnapshot.numberActions`) 같이 publish 하고,
  터미널은 그걸 되접는다 — 의미를 두 곳에서 따로 정하면 반드시 갈라진다.
  (`WaveRunTerminalTests`·`NetBattleTerminalTests`·`RoomTerminalTests`, 2026-09-05.)

## "밟는 경로가 0 이다" 로 접은 전제는 트립와이어가 없으면 부채다

- **유예 자체는 부채가 아니다. 전제가 깨져도 아무도 모르는 것이 부채다.** 엔진에는 "오늘 도감엔
  그런 기술이 없으니 이 경로는 안 밟는다" 로 접은 자리가 있다 — `resolveAttack` 이 위력을 다단기
  루프 **안**에서 뽑는 것, 실패와 면역을 `effectiveness == 0` 하나로 합치는 것. 둘 다 주석에
  해제 조건이 적혀 있었지만, 조건이 충족되는 순간을 **알려 주는 것이 아무것도 없었다.**
- **전제가 어디에 걸려 있는지 먼저 가른다 — 로컬이면 테스트, 원격이면 로그.** 위 두 전제는
  `VariableDamage` 의 스위치 한 곳(로컬 `MoveID` 상수)에서 결정되므로 **소스 동결 테스트**로 잡힌다
  (`BattleAssumptionGuardTests`, `LanguageSplitGuardTests` 와 같은 방식). 반면 `validMoves` 의
  `위력 × 히트 ≤ 250` 은 도감(PokéAPI)에 걸려 있어 **코드를 안 건드려도 깨진다** — 테스트로는
  못 잡으므로 `MoveSpec.from` 이 스펙 조립 때 `AppLog` 로 한 번 남긴다(ailment·스탯 로그와 같은 규칙).
  깨지면 증상은 조용하다: 내 화면엔 멀쩡한 기술이 상대에게서만 반려된다.
- **왜 커버리지가 못 걸렀나.** 두 자리 다 실행은 됐다. 안 밟힌 건 *조건의 반대편*이고, 그건 애초에
  오늘 도감에 존재하지 않는 입력이다 — 라인·리전 커버리지가 답할 수 있는 질문이 아니다. 물어야 할
  것은 "이 줄이 돌았나" 가 아니라 **"이 줄의 전제가 아직 참인가"** 이고, 그건 별도 가드가 답한다.
- **천장은 두 곳에 리터럴로 두지 않는다.** 도감 쪽 검출(`exceedsTurnDamageCap`)과 와이어 쪽 반려
  (`validMoves`)가 각자 `250` 을 들면 경고와 반려가 갈라져, 넘었다고 찍은 기술이 통과하거나 통과한다고
  본 기술이 반려된다. 상수(`turnDamageCap`)가 아니라 **규칙 자체**를 공유시킨다.
- **동결 목록 테스트는 "목록만 갱신하고 넘어가기" 가 기본 실패 모드다.** 그래서 실패 메시지가
  갱신 방법이 아니라 **먼저 답해야 할 질문**을 적는다("새 기술이 물·전기인가", "다단기인가").
- 가드 검증은 결함 주입으로 했다 — `VariableDamage` 스위치에 `.noEffect` 와 `.power(` 를 함께 내는
  case 를 하나 넣자 두 테스트가 `차이: ["injectedProbe"]` 로 지목하며 빨개졌고, 되돌리자 초록으로
  돌아왔다. (`BattleAssumptionGuardTests`, 2026-09-05.)

## 기간 경계 가드를 기준 시각 **하나**로 쓰면 그 기간의 끝을 영영 안 밟는다

- **증상**: 주간 회고의 "가장 오래된 주가 보존 기간(90일) 안인가" 가드가, `maxWeeksBack` 을
  파생식(`retentionDays / 7 - 1`)에서 리터럴 `12` 로 망가뜨려도 **초록이었다.**
- **직접원인**: 오프셋을 *이번 주 월요일*에서 재는데 그 월요일은 `now` 로부터 0~7일 전이다. 즉
  여유가 요일에 따라 최대 7일 흔들린다 — 수요일 기준이면 12주도 들어가고, **일요일 밤에 열어야**
  비로소 넘친다. 테스트가 기준 시각 하나(수요일)만 써서 결함 트리거와 다른 경로로 통과했다.
- **부류**: *주기(週·月·시즌)를 다루는 판정에서 여유가 가장 좁아지는 지점은 주기의 끝이다.* 기준
  시각 하나로 쓴 경계 테스트는 그 끝을 밟을 확률이 1/7(주) · 1/28(월)이라, 대개 통과하면서
  아무것도 안 지킨다. `dayKey` 자정 테스트가 36시간을 넘기는 것과 같은 부류의 반대편이다
  (그쪽은 DST 로 *경계를 못 넘는* 경우를 막는다).
- **처방**: 경계 가드는 **주기 전체를 훑는다** — 요일 일곱, 필요하면 월 경계까지. 한 지점만 볼
  거라면 그 지점이 *가장 빡빡한 쪽*(주의 마지막 순간)이어야 하고, 왜 거기가 최악인지 주석에 남긴다.
  (`FocusWeekRecapTests.testTheOldestReachableWeekFitsEntirelyInsideRetentionOnEveryWeekday`, 2026-09-05.)

## 완화가 지목한 테스트가 그 표면을 아예 안 그리면, 그 완화는 없는 것이다

- **증상**: 계획서가 "집중 카드에 버튼이 늘어 파트너 카드가 밀린다" 위험의 완화로
  `PopoverLayoutTests` 를 지목했다. 그 스위트는 초록이었지만 **`FocusTimerView` 를 한 번도
  렌더링하지 않는다** — 통과가 이 변경에 대해 증명하는 것이 0이었다.
- **직접원인**: 스위트 이름이 표면과 겹쳐(`PopoverLayout` ↔ 팝오버 안의 카드) 덮는 범위를
  이름으로 추정했다. 실제로 무엇을 렌더링하는지는 확인하지 않았다.
- **부류**: *"이 테스트가 그걸 잡는다" 는 주장은 이름이 아니라 **그 파일이 대상 타입을 실제로
  인스턴스화하는지**로 확인한다.* 리뷰·계획서·PR 본문에서 완화로 스위트 이름을 적을 때마다 해당한다.
- **처방**: 완화로 지목하기 전에 `grep -n "<TypeName>" <TestFile>` 한 번. 안 나오면 완화가
  아니라 **공백**이고, 그 자리에 가드를 새로 세운다. 새 예산 가드에는 **대조군**을 짝으로 둔다
  (`testAButtonOnItsOwnRowBlowsTheFocusTodayRowBudget` — 버튼을 자기 줄에 두면 예산을 넘는다).
  대조군이 없으면 그 예산이 애초에 도달 불가능한 조건이었는지 구별할 수 없다.
  (`PopoverLayoutTests` 집중 카드 오늘 줄 예산, 2026-09-05.)

## 같은 기능의 코어인데 일부만 `LOGIC_CORE` 에 있으면 나머지는 무측정으로 나간다

- **증상**: 포모도로 마일스톤 1–3(원장·체인 규칙)이 배포된 뒤에도 `FocusSessionLog.swift` 와
  `FocusChainRules.swift` 가 `test-gate.sh` 의 `LOGIC_CORE` 밖에 있었다. 같은 기능의
  `PokedoroSessionGate`·`PokedoroViewChannel` 은 이미 배열 안이었다.
- **부류**: 배열은 **파일 단위로 손으로** 늘린다. 기능이 여러 파일로 자라면 나중에 생긴 파일만
  들어가거나 처음 것만 들어간 채로 남는데, 배열 밖 파일은 "몇 %인가" 를 **물을 기회조차 없다**
  (`PokemonAuction` 을 넣은 그 이유와 같다).
- **처방**: 순수·결정적 판정 파일을 새로 만들 때, 그 파일 **하나만** 넣지 말고 같은 기능의
  형제 파일이 전부 배열에 있는지 함께 본다. 넣은 뒤 게이트 총합이 임계값을 못 넘으면 숫자를
  낮추지 말고 `llvm-cov show … --show-line-counts-or-regions` 로 `^0` 줄을 직접 덮는다.
  (`scripts/test-gate.sh`, 2026-09-05.)
- **v2.26.0 스윕**: 릴리스 전에 `v2.25.2..HEAD` 의 신규 `Core/` 파일 17개를 전수 대조하니 8개가
  배열 밖이었다. **무테스트가 아니라 무측정이었다** — 넣고 돌리니 `TerminalRoomControl` 99.09%,
  `WaveRunLoader` 91.94%, `CompanionActionOutcome` 100% 로 이미 테스트가 닿아 있었고 총합도
  91.31% 로 올랐다. 이 부류의 비용은 "테스트가 없다" 가 아니라 **"있는지 없는지 물을 수 없다"** 다:
  다음에 그 파일에 분기를 더하는 사람은 아무 신호도 못 받는다. 남은 다섯(`TerminalTradeControl`
  20줄 등)은 판정이 없는 얇은 껍데기라 뺐다 — 뺀 것도 판단이므로 여기 적는다.
  (`scripts/test-gate.sh`, 2026-09-05.)

## 릴리스 문서에 **상태**를 적으면 그 문장이 일보다 오래 산다

- **증상**: `RELEASE.md` 체크리스트가 "트레이너 꾸미기(옷장) 화면을 담은 스크린샷은 다음 릴리스에서
  갱신이 필요하다" 고 지시했다. 실제로는 이미 최신이었다 — `OutfitView.swift` 의 **유일한** 커밋이
  2026-08-27 14:49(#132)이고 `assets/screenshot-outfit*.png` 는 같은 날 16:37(#139)에 그려졌다.
  v2.26.0 이 그 지시를 따라 이미 최신인 에셋을 다시 그릴 뻔했다.
- **직접원인**: 손으로 적은 "다음 릴리스에 X 하라" 에는 만료가 없다. X 를 실제로 한 릴리스(#139)가
  그 문장을 지우지 않았고, 지울 이유도 없었다 — 자기가 지운다는 것을 아무도 안 적어 뒀다.
- **테스트/리뷰가 왜 못 걸렀나**: 체크리스트는 산문이라 게이트도 테스트도 안 본다. 더 나쁜 것은
  **불필요한 일을 해도 체크는 통과한다**는 점이다 — 낭비는 실패로 보이지 않으므로 신호가 0이다.
  (같은 이유로 오래 살아남은 문장이 하나 더 있었다: `release-workflow.md` 의 "한국어 README 는
  스크린샷을 싣지 않으므로 `-ko` 는 기존 파일에만 남아 있다" — 세 README 모두 표에 이미지를 싣는다.)
- **부류**: 릴리스 문서가 **세상의 현재 상태**를 주장하면(어느 에셋이 낡았다, 어느 README 가
  무엇을 싣는다) 그 주장은 코드·에셋보다 느리게 썩고, 다음 사람은 확인 없이 그대로 따른다.
- **처방**: 릴리스 문서에는 상태를 적지 않고 **상태를 묻는 명령**을 적는다. 옷장 건은 두 시각
  대조로 바뀌었다 — 뒤가 더 최신일 때만 다시 그린다.
  ```bash
  git log -1 --format=%ci -- assets/screenshot-<화면>.png
  git log -1 --format=%ci -- Sources/PokeTokenBar/UI/<View>.swift
  ```
  (`RELEASE.md`·`docs/reference/release-workflow.md`, 2026-09-05.)

## 소스를 문자열로 스캔하는 가드는 **주석까지 세면** 호출이 사라져도 초록이다

- **증상**: 순풍(#순서 계산)을 붙이면서 "순서를 재는 모든 모드가 `BattleEngine.orderingSpeed` 를
  써야 한다" 는 소스 스캔 가드를 넣었다. 결함 주입으로 `MultiplayerBattle` 의 호출 두 줄을
  `side.effectiveSpeed` 로 되돌렸는데 **테스트가 통과했다** — 바로 위 주석에
  "마비·순풍 보정은 `orderingSpeed` 가 들고 있다" 가 남아 있어 `text.contains` 가 참이었다.
- **직접원인**: 스캔이 "코드에 그 호출이 있나" 를 묻는다고 생각하고 실제로는 "파일 어딘가에 그
  글자가 있나" 를 물었다. 주석은 호출을 지울 때 같이 지워지지 않는다(오히려 설명이라 남는다).
- **테스트/리뷰가 왜 못 걸렀나**: 가드 자체가 테스트라서 그 위의 테스트가 없다. 초록만 보면
  "아무것도 안 지키는 스캔" 과 "지키는 스캔" 이 구별되지 않는다 — 결함을 주입해 봐야 갈린다.
- **부류**: 소스 텍스트를 증거로 쓰는 가드 전부. `applyAttack` 의 `attackerTeam:` 스캔,
  `beginTurn` 스캔처럼 **인자 이름·함수 이름을 문자열로 찾는** 기존 가드가 같은 부류다
  (지금은 그 이름이 주석에 없어서 우연히 맞는 상태다).
- **처방**: 세 가지를 같이 한다.
  1. 스캔 전에 **주석 줄을 떼어낸다**(`//` 로 시작하는 줄을 비운다).
  2. 정의한 파일은 **선언 한 번을 빼고** 센다 — 안 빼면 엔진이 자기 함수를 안 부르는 채 통과한다.
  3. 새 스캔 가드는 **결함을 주입해 빨개지는 것을 확인**한 뒤에만 초록을 믿는다.
  (`BattleTailwindTests.testEveryTurnOrderSiteAsksTheEngineForSpeed`, 2026-09-07.)

## 대상마다 부르는 **다른 입구**가 있으면, 한쪽 입구의 게이트는 그 모드만 지킨다

- **증상**: 방어(protect) 부류를 붙일 때 "상대가 지켰으면 막는다" 판정을 `applyAttack` 에 뒀다.
  1대1·방·웨이브의 단일 대상 공격은 전부 그 함수를 지나 정상이었지만, **웨이브 런의 광역기는
  대상마다 `applyHit` 을 직접 부른다** — 그 경로에서는 방어가 통째로 무시됐다. 커밋 전에 부류
  스윕으로 잡았다(화면에서는 "왜 이 기술만 방어를 뚫지" 로 보였을 것이다).
- **직접원인**: 같은 일을 하는 입구가 둘인데(`applyAttack` = 기술 하나, `applyHit` = 대상 하나)
  대상별 판정을 기술 단위 입구에 뒀다. 판정의 단위와 입구의 단위가 어긋난 것이다.
- **테스트/리뷰가 왜 못 걸렀나**: 처음 쓴 테스트가 전부 `applyAttack` 을 불러서 초록이었다.
  광역기 경로는 **다른 함수**라 같은 단언을 아무리 늘려도 밟히지 않는다.
- **부류**: `applyHit` 을 직접 부르는 자리(광역기)와 `applyAttack` 만 부르는 자리가 함께 있는
  판정 전부 — 데미지 감쇠·상태 부여·장막·방어. 판정을 어디 두는지가 "대상마다" 인지 "기술마다"
  인지로 갈린다.
- **처방**: **대상 단위 판정은 대상 단위 입구(`applyHit`)에 둔다.** 그러면 기술 단위 입구도
  자동으로 지나간다(역방향은 성립하지 않는다). 테스트도 두 입구를 각각 부른다 —
  `BattleGuardTests.testAGuardAlsoBlocksTheSpreadPathThatSkipsApplyAttack` 이 광역 경로를 직접 만진다.
  (2026-09-07.)
## 화면들이 **서로 같은가**는 어느 화면의 테스트에도 속하지 않아 아무도 안 본다

- **부류**: 화면마다 손으로 베끼는 크롬 — 닫기 버튼, 현재 위치 표시, 재화 이름, 아이콘 버튼의
  접근성 이름, 상시 애니메이션의 "동작 줄이기" 판정. 화면별 테스트는 자기 화면 안에서만 옳은지를
  본다. "여덟 화면이 닫기를 같은 자리에 두는가" 는 그중 누구의 요구사항도 아니다.
- 실제로 갈렸던 것들(2026-09-07 스윕): 닫기 버튼이 아홉 화면에서 좌상단 "‹ 뒤로" · 우상단
  `xmark` · `xmark.circle.fill` 셋으로 갈렸고, 툴팁은 한 곳에만 있었다. 상점 · 가방은 `PopoverTab`
  이면서 탭바에 자리가 없어 그 화면에 들어가면 선택 표시가 **화면 전체에서** 사라졌다. 재화 이름은
  한국어 `별의조각`/`별의모래`, 일본어 `ほしのかけら`/`ほしのすな`/`累計トークン`, 영어
  `Star Pieces`/`Stardust` 로 갈려 세이브 내보내기 설명 한 줄이 세 언어에서 서로 다른 재화를
  말했다. 아이콘 버튼 18곳에 접근성 이름이 없었고, `repeatForever` 세 곳 중 두 곳이 시스템
  "동작 줄이기" 를 무시했다.
- **처방은 정본 + 전수 가드 두 짝이다.** 정본만 만들면 다음 화면이 안 쓰고, 가드만 걸면 쓸 것이
  없다. `PokedoroOverlayCloseButton` · `PokedoroOverlayHeader` · `PopoverTab.tabBarTabs` 가 정본,
  `OverlayChromeTests` · `IconButtonLabelGuardTests` · `ReduceMotionGuardTests` ·
  `TerminologyGuardTests` 가 전수 가드다.
- **가드는 표기가 아니라 하는 일로 짜라.** 닫기 가드를 `Button(action: onClose)` 라는 **문자열**로
  짰더니, 그 표기를 치운 뒤에도 `Button { close() }` 로 쓴 닫기가 네 화면에 그대로 살아 있었다 —
  가드가 초록인 채로 규칙이 절반만 지켜졌다. "아이콘만 있는 버튼이 닫기 동작을 부른다" 로 넓히고
  나서야 네 곳이 드러났다. 같은 이유로 재화 가드는 낱말 경계를 보고(`totalStardust` 는 타입 이름이라
  규칙 밖), 모션 가드는 값이 맞는지가 아니라 **판정을 하긴 했는지**를 본다(애니메이션이 실제로
  도는지는 테스트가 볼 수 없다).
- **스캐너 자체가 미탐의 자리다.** 아이콘 버튼 가드의 첫 판이 modifier 사이에 낀 주석에서 수집을
  멈췄다 — 그런데 `accessibilityLabel` 은 "왜 이렇게 두었나" 를 적은 주석 **바로 아래**에 달린다.
  달아 둔 라벨을 없다고 신고했고, 반대로 다른 자리에서는 있는 것을 못 보고 지나칠 수 있었다.
  가드를 새로 만들면 **결함을 주입해 실패시키는 것뿐 아니라, 통과가 어디까지 통과인지**를 주석에 적는다.
- **전수 치환은 문법을 깬다.** `별의모래 → 별의조각` 을 47곳에 sed 로 돌리자 조사가 어긋났고
  (`별의조각가`), 영어는 수일치가 깨졌으며(`Star Pieces that piles up`), 일본어는 수식어가
  통째로 사라졌다(`累計トークン → ほしのかけら`, "누적" 이 증발). 치환 뒤 **바뀐 줄을 사람이 읽는다**.

## 접근성 우려는 **그 플랫폼에서 일어나는지** 먼저 재라 — 안 일어나는 결함을 고치면 진짜 결함이 남는다

- **증상**: UI/UX 감사(2026-09-07)가 "7~9pt 고정 폰트 40곳이 Dynamic Type 을 안 타고, 팝오버가
  780pt 고정이라 글자를 키운 사용자는 트레이너 바 · 타이머 · footer 에서 잘린다" 를 과제로 올렸다.
  **실측하니 macOS 에서 일어날 수 없는 시나리오였다.**
- **직접원인**: macOS 에는 Dynamic Type 이 없다. `NSHostingView.fittingSize` 로 재면
  `dynamicTypeSize` 를 `.large` → `.accessibility5` 로 올려도 `.system(size: 8)` 은 물론
  **`.caption2` 조차 13pt 그대로**다(환경값 자체는 주입된다 — 같은 뷰에서 값 문자열 폭이 20→55pt 로
  바뀐다). 시스템 쪽도 전역 슬라이더가 아니다: 접근성 텍스트 크기는
  `com.apple.universalaccess` 의 `FontSizeCategory` 딕셔너리에 **앱별로 등록된** 앱(Mail · Notes ·
  iCal · Finder 등)에만 걸린다. 우리 앱은 그 목록에 없다.
- **왜 이렇게 적혔나**: iOS 감사 상식(고정 pt = Dynamic Type 위반 = 잘림)을 플랫폼 확인 없이
  옮겼다. 코드에는 위험 신호(고정 pt · 고정 높이)가 실제로 있었고, 신호가 맞았기 때문에 결론도
  맞다고 읽혔다. 게다가 "잘림" 은 화면을 안 봐도 그럴싸해서 반박 비용이 커 보인다.
- **부류**: 플랫폼 기능을 전제한 결함 보고. 전제가 그 OS 에 없으면 처방이 헛돌고, 더 나쁜 것은
  **그 옆에 있는 진짜 결함이 같은 항목에 묶여 함께 미뤄진다**는 점이다. 여기서 진짜 결함은
  "사용자가 키울 수 없는데 우리가 7~9pt 로 적었다" 였다 — 읽는 글자 104곳이 macOS 의 가장 작은
  텍스트 스타일(`caption2` = 10pt) 밑이었고, 그중엔 종 이름 · 스탯 값 · 도감 번호 · 전투 로그가 있었다.
- **처방**:
  1. **정본 + 하한**: `PokedoroTheme.minimumTextSize`(10pt)가 하한이고, 그 아래는
     `badgeFont`(캡슐 배지 · 스프라이트 위 표식) · `glyphFont`(SF Symbol · 이모지)로만 쓴다.
     이름이 곧 "읽는 글자가 아니다" 라는 선언이라 다음 사람이 숨기려면 일부러 거짓말을 해야 한다.
  2. **전수 가드**: `MinimumTextSizeGuardTests` 가 `UI/` 에서 숫자를 직접 적은 10pt 미달
     `.system(size:)` 를 전부 잡는다.
  3. **축소도 같은 하한을 받는다** — 부류 스윕에서 `minimumScaleFactor(0.7~0.8)` 7곳이 나왔다.
     넘칠 때만 작동하지만 작동하는 순간 폰트에 적힌 pt 는 의미가 없어진다(10pt × 0.7 = 7pt).
     0.9 까지만 열어 뒀다 — 축소를 막으면 그 자리가 `…` 로 잘리고, 도감 격자 칸은 44pt 라 이름
     대부분이 사라진다. 잘림보다 9pt 가 낫다고 보고 한 단계만 허용한다.
- **폰트를 키우면 고정 높이 칸이 먼저 깨진다 — 그걸 잡은 것은 새 가드가 아니라 기존 예산 테스트다.**
  전투 로그를 9pt → 10pt 로 올리자 `BattleFieldTests.testTheLogBoxIsTallEnoughForTheLinesItDraws`
  가 "4줄이 64pt 칸에 안 들어간다(68 필요)" 로 실패했다. 고정 높이 칸은 넘쳐도 **보고하는 높이가
  그대로**라 눈으로도 다른 테스트로도 안 보이고, 넘친 줄은 아래 기술 버튼 위에 겹쳐 그려진다.
  글자 크기를 손대는 변경은 고정 높이 상수(`BattleFieldMetrics.logHeight` 같은 것)를 함께 본다.
  (`PokedoroTheme`·`MinimumTextSizeGuardTests`·`BattleFieldMetrics`, 2026-09-07.)

## 페이지 컨트롤을 고정 높이 탭의 **하단**에 두면, 뷰포트 밖으로 밀려 없는 것과 같다

- **증상**(2026-09-07 사용자 리포트): 포켓몬 탭에서 다음 페이지 버튼이 안 보인다. 세이브에는
  159마리(11페이지)가 있는데 15칸짜리 첫 페이지가 전부인 것처럼 보였다.
- **직접원인**: 격자 탭은 520pt 프레임을 고정으로 요청하는데 팝오버 창은 780pt 다. 위쪽 크롬
  (패딩 28 · 트레이너 바 46 · 집중 카드 169 이상 · 탭 줄 48 · 하단 탭 줄 40 · 간격 48)이 380pt 를
  넘게 먹으므로 탭에 남는 자리는 400pt 미만이다 — 하단에 있던 페이저는 스크롤해야 닿는다.
  창 높이는 클리핑을 피하려고 화면에 맞춰 잘리므로(#9) 창을 키우는 것은 처방이 아니다.
- **왜 못 걸렀나**: 세로 예산 테스트가 **카드 단위**로만 있었다(미션 카드 100pt, 집중 카드 오늘 줄
  22pt, 도감 목표 줄 …). "탭 콘텐츠 전체가 팝오버 뷰포트에 들어오나" 는 어느 화면의 테스트도 아니고,
  집중 카드가 커질 때 줄어드는 것은 **다른 파일의** 자리라 그 파일의 테스트는 계속 초록이었다.
  페이징 테스트도 `pageCount` 산수만 봤다 — 마지막 한 마리가 어느 페이지에 있는지는 맞았고,
  그 페이지로 **가는 버튼이 보이는지**는 산수 밖이다.
- **부류**: 스크롤 밖에서 자리를 먹는 크롬이 늘 때, 고정 높이 탭의 아래쪽 컨트롤이 조용히
  뷰포트를 벗어난다. 페이지 컨트롤은 그중 최악이다 — 안 보이면 콘텐츠가 없는 것으로 읽힌다.
- **처방**: 페이지 컨트롤을 격자 **위**(검색칸과 같은 줄)로 올린다 — 로스터와 도감 둘 다.
  하단 줄은 알 개수·선택한 칸 표시만 남기고 높이 예약은 유지한다(격자 높이가 흔들리지 않게).
- **가드**: `PopoverLayoutTests.testPagedGridScreensDrawThePageControlAboveTheGrid` 가 두 화면의
  소스에서 페이저 호출이 격자 호출보다 위인지 본다(화면 안 좌표는 순수 함수로 못 재고, 높이만
  재면 위·아래가 같은 값이라 순서로 본다). 짝인 대조군
  `testPagedGridTabsAreTallerThanThePopoverViewport` 가 "크롬 + 520 > 창 높이" 를 실제로 재
  뷰포트 밖으로 밀리는 조건이 성립함을 확인한다. 결함을 되돌리면(페이저를 격자 뒤로) 실패한다.
- **같은 부류로 보이지만 아닌 것**: 상대 목록(`BattleView`)·포케슬론 방 목록·기술 재습득 카드의
  페이저는 자연 높이 콘텐츠 안에 있어, 사용자가 읽고 있는 목록 바로 아래에 붙어 함께 스크롤된다.
  기준은 "그 페이저가 **고정 높이 프레임의 하단**인가" 다. `TeamPicker` 는 이미 필터 줄(상단)에 있다.

## 부분일치로 이름을 찾는 가드는 `accessibilityLabel(` 을 글자로 세어, 막으려던 것을 통과시킨다

- **증상**: "오버레이를 여는 버튼은 글자를 가진다" 가드를 세우고 결함을 주입했는데(글자 라벨을
  아이콘으로 되돌림) **테스트가 통과했다.**
- **직접원인**: 공용 스캐너의 `SwiftSourceScan.isIconOnly` 가 `!block.contains("Label(")` 로 판정한다.
  버튼 블록에는 `.accessibilityLabel("주간 회고")` 가 붙어 있고 그 문자열이 `Label(` 을 **포함**하므로,
  글자가 하나도 없는 버튼이 "글자 있음" 으로 읽혔다.
- **왜 이렇게 남았나**: 원래 이 스캐너를 쓰던 가드(`IconButtonLabelGuardTests`)의 규칙이 "아이콘 전용
  버튼에는 `accessibilityLabel` 이 있어야 한다" 였다. 그 규칙에서는 오판정 방향이 우연히 맞아떨어져
  (라벨이 있는 버튼은 검사 대상에서 빠지고, 없는 버튼은 그대로 걸린다) 결함이 드러나지 않았다.
  **한 곳에서 우연히 맞는 판정은 재사용되는 순간 틀린다.**
- **처방**: `isIconOnly` 가 `accessibilityLabel(` 을 먼저 지우고 본다 — 화면 판독기용 이름은 화면에
  보이는 글자가 아니다.
- **부류**: 가드 자체의 거짓음성. **새 가드는 반드시 결함을 주입해 실패를 확인한다** — 통과만 보면
  아무것도 안 지키는 가드와 지키는 가드를 구별할 수 없다(이번엔 그 주입이 스캐너 결함을 잡아냈다).

## `hp == 0` 하나로 "쓰러짐"과 "이탈"을 같이 판정하면, 남아서 싸운 사람까지 걸러낸다

- **증상**: 레이드에서 쓰러졌지만 방은 나가지 않은 참가자가 포획 추첨에서 아예 빠졌다. 사용자
  보고: "왜 살아남은사람 대상으로만 포획할 수 있도록 되어있어?"
- **직접원인**: `drawRaidCatcher` 가 `runners.filter(\.isAlive)` 로 대상을 걸렀다. `isAlive` 는
  `hp > 0` 뿐이라, 싸우다 쓰러진 것과 `retireFighter`(방 나가기·연결 끊김)가 만드는 `hp = 0` 을
  구분하지 못했다 — 후자만 걸러야 하는데 전자까지 같이 빠졌다.
- **테스트/리뷰가 왜 못 걸렀나**: 기존 회귀 테스트(`testAFaintedRunnerIsNeverDrawn`)가 애초에
  "쓰러짐"과 "이탈"을 하나의 테스트로 뭉쳐 `mate.side.hp = 0` 한 줄로만 표현했다 — 두 사건이
  같은 값으로 표현된다는 것 자체를 테스트가 전제로 깔고 있어서, 이 전제가 틀렸다는 걸 어떤
  테스트도 물을 수 없었다.
- **부류**: `state-predicate-precision.md` 의 정확히 그 부류다 — 하나의 값(`hp == 0`)이 서로 다른
  두 사건(쓰러짐/이탈)을 가리킬 수 있는데 한쪽만 보고 판정했다.
- **처방**: `MultiplayerFighter.hasLeft` 를 별도 필드로 두어 `forfeit()`(이탈 경로에서만 호출)
  에서만 세운다. 추첨은 `!hasLeft` 로 걸러 이탈만 배제하고, 회귀 테스트도
  `testARunnerWhoLeftIsNeverDrawn`(이탈 배제)과 `testAFaintedButPresentRunnerIsStillDrawn`(쓰러짐
  포함)으로 갈라 각자의 트리거를 따로 검증한다. 와이어 모양이 바뀌므로 LAN 프로토콜 버전도 올린다
  (`MultiplayerBattle.swift`, `MultiplayerRoomCenter.swift`, `RaidRoomTests.swift`, #270).

## "표시를 못 하니 계산도 하지 마라" — 화면 문제를 코어 게이트로 고치면 규칙이 깨진다

- **증상**: 반나절 두 번째 이후 승리에서 포획 결과 화면이 통째로 비었다("이미 받았다"는 지급
  안내만 뜨고 포획 결과는 아무것도 안 보임). 처음엔 "포획 추첨 자체를 지급 여부와 무관하게 항상
  돌리자"로 고쳤는데, 사용자가 "한번 추첨 돌았으면 그 레이드는 더이상 포획추첨은 되면 안돼" 라고
  정정 — 설계 문서에 이미 "추가 판에도 참가하고 채팅할 수 있지만 같은 구간의 보상·포획은 없다"
  라고 적혀 있었다.
- **직접원인**: `applyRaidSettlement` 가 `drawRaidCatcher` 호출 자체를 `raidPayout > 0`(이 구간의
  첫 승리인가) 뒤에 걸어 뒀다. 이 게이트는 **의도된 설계**였는데, 그 게이트 때문에 두 번째 승리의
  **화면**이 아무 설명 없이 비어 사용자에게는 버그로 보였다. "게이트가 잘못됐다"와 "화면 안내가
  없다"를 구분하지 않고 게이트부터 없앤 것이 잘못이었다.
- **테스트/리뷰가 왜 못 걸렀나**: 설계 문서를 먼저 확인하지 않고 사용자 증상만 보고 코어를 고쳤다
  — `root-cause-rigor.md` 가 말하는 "증상 완화 ≠ 근본원인"의 반대 실수다(원인은 맞았는데 처방이
  설계를 어겼다).
- **부류**: 사용자가 보고하는 것은 항상 **화면**이지 코어 로직이 아니다. "화면이 이상하다"의
  원인이 반드시 "코어 게이트가 틀렸다"인 것은 아니다 — 코어는 의도대로 맞고 화면 안내만 없는
  경우를 먼저 배제해야 한다.
- **처방**: 게이트(`raidPayout > 0` 뒤에서만 추첨)는 되돌리고, `RaidView.finishedFooter` 에 "이번
  승리는 포획 대상이 아닙니다 — 포획 추첨은 이 구간의 첫 승리에서만 진행됩니다" 안내를 추가했다.
  잘못됐던 회귀 테스트(`testASecondRewardOfTheDayStillShowsCatchResults`, "항상 돈다"를 검증)는
  반대로(`testASecondWinOfTheDayDrawsNoCatcher`, "안 돈다"를 검증) 다시 썼다(#270).

## 폭 계산기가 escape 를 만나면 글자로 센다 (그 줄만 조용히 잘린다)

- **증상**: 터미널 홈에 파트너 그림을 얹었더니 그림 줄이 중간에서 끊기고, 그 줄부터 커서 열이
  밀렸다. 전체 다시 그리기로도 복구되지 않는다 — 터미널에 반 칸 되돌리기가 없다.
- **직접원인**: `TUIText.displayWidth`·`truncate`·`pad` 는 유니코드 폭 표로 **글자마다** 칸을
  센다. SGR escape(`ESC[38;2;…m`)는 화면에서 0 칸인데 여기서는 30칸쯤으로 세어져, 20칸짜리
  그림 한 줄이 700칸으로 판정되고 `truncate` 가 escape **중간**을 자른다.
- **부류**: 폭·정렬 계산기는 "보이는 글자만 온다" 를 전제한다. 색·커서 제어가 들어오는 순간 그
  전제가 깨지는데, 깨진 자리는 **그 줄에만** 나타나 다른 줄은 정상으로 보인다.
- **테스트/리뷰가 왜 못 걸렀나**: 처음 쓴 가드는 **아무것도 지키지 않았다** — 폭 41에 39자짜리
  그림 줄을 넣어 자를 것이 없었고, 자르는 코드를 일부러 넣어도 통과했다. 통과만 보고 넘겼으면
  가드가 없는 채로 나갔다. 실제 그림(20칸 = 약 700자)으로 바꾼 뒤에야 결함 주입이 실패로 나왔다.
- **처방**: ⓐ escape 가 든 줄은 폭 계산기를 **통과시키지 않는다**. 칸 수는 만드는 쪽이 지키고
  (`TUISprite.block`), 화면은 그대로 얹는다. ⓑ 여러 종류의 줄이 섞이는 총폭 단정은 escape 를
  0 칸으로 세는 계산기로 바꾼다(`TUISprite.visibleWidth`). ⓒ 새 가드는 **결함을 주입해 실패를
  확인**한다 — 입력이 한계보다 작으면 자르는 코드가 있어도 통과한다.
  (`TUISprite`·`TUIRender.home`, 2026-09-07.)
## 새 스토어 테스트가 공용 임시 디렉토리를 직접 쓴다

- **증상**: 던전 일일 보상 회귀 테스트를 추가한 PR 에서 제품 빌드는 성공했지만 픽스처 격리
  스윕이 CI 를 막았다.
- **직접원인**: 테스트가 `FileManager.default.temporaryDirectory` 아래에 직접 디렉토리를 만들었다.
  UUID 로 디렉토리를 나눴어도 저장소가 정한 유일한 픽스처 생성 경로가 아니어서, 곁방 파일과
  정리 수명 규칙을 앞으로 일관되게 바꿀 수 없었다.
- **테스트/리뷰가 왜 못 걸렀나**: 로컬 `swift build` 는 테스트 소스를 검사하지 않고, 이 기기의
  Command Line Tools 에는 XCTest 가 없어 `swift test` 도 테스트 컴파일 전에 멈췄다. 코드 리뷰에서
  "고유한 디렉토리면 충분하다"고 보고 저장소의 더 강한 계약을 확인하지 못했다.
- **부류 스윕**: `test-gate.sh` 의 픽스처 격리 스윕이 전체 테스트 트리를 검사해 같은 위반은 이
  한 건뿐임을 확인했다.
...
- **증상**: `main` 에 미사용 지역화 바인딩 경고 3건이 있는데 CI 는 초록이었다
  (`PokeTokenBarApp.swift` 의 `let l`, `Localization.swift` 의 `enTurn`·`jaTurn` — #274 가
  세 언어 문구를 한국어 하나로 접으면서 마지막 사용처가 사라졌고 바인딩만 남았다).
- **직접원인**: `test-gate.sh` 의 warning 검사는 `swift test` **로그에서 경고를 긁는다.**
  재컴파일이 없는 빌드는 경고를 다시 찍지 않으므로 0건으로 읽힌다. 그런데 **게이트 안에서 모듈을
  컴파일하는 자리는 `swift test` 가 아니었다** — 그보다 앞서 도는 `mutator-sweep.sh` 의
  `swift build --enable-code-coverage` 가 그것이고, 그 출력은 `>/dev/null` 로 버려졌다.
  `swift build` 는 진단을 **stdout** 에 찍으므로 stdout 을 버리는 것이 곧 진단을 버리는 것이다.
  게이트와 플래그를 일부러 맞춰 둔 탓에(그 자리 주석의 의도대로) 뒤따르는 `swift test` 는 재컴파일이
  없어 아무것도 다시 찍지 않았다 → 경고는 어디에도 남지 않았다. `ci.yml` 의 게이트 앞 `swift build`
  는 이 구조 위에 한 겹 더 얹힌 것이었다(그 단계를 빼도 스윕의 빌드가 그대로 남으므로 **혼자서는
  아무것도 고치지 못한다** — 미사용 바인딩을 주입해 실측 확인했다).
- **테스트/리뷰가 왜 못 걸렀나**: 검사가 "경고가 없다" 와 "경고를 볼 수 없었다" 를 구분하지
  않는다. 둘 다 빈 문자열이라 초록이 나온다. 로컬에서도 같은 이유로 안 보였다. 그리고 처방을
  `ci.yml` 한 곳에서 멈춘 것도 같은 뿌리다 — **로그를 만드는 단계를 전수로 찾지 않고 눈에 띈
  한 곳만 고쳤다**(`grep -rn 'swift build' scripts/` 한 번이면 스윕의 빌드가 나왔다).
- **부류**: **로그를 증거로 쓰는 검사 전부.** 그 로그를 만드는 단계가 캐시·증분·건너뛰기·출력
  버리기로 비어 있을 수 있으면, 검사는 "위반 없음" 이 아니라 "관측 없음" 을 통과로 읽는다.
  (같은 모양: 커버리지 `^0` 을 안 보고 라인 %만 보는 것, 소스 스캔 가드가 주석을 세는 것.)
- **부류 스윕 결과**(같은 게이트 안에서 셋 더 나왔다):
  - 영어 로케일 재실행이 **0건도 통과**로 읽었다 — `xcrun xctest` 는 한 건도 못 돌려도 0 으로
    끝나고, 개수는 출력만 하고 있었다. 개수 0/누락을 실패로 못 박았다.
  - 그 재실행은 XCTest 만 돈다 — 같은 번들의 swift-testing 케이스는 열거되지 않아 로케일 패리티
    밖이다. `Package.swift` 가 새 테스트를 그쪽으로 안내하므로 적용 범위가 계속 줄어든다(한계로
    적어 두었다; `swift test` 가 테스트 바이너리에 인자를 넘길 수 있게 되면 해소).
  - `llvm-cov report ... 2>/dev/null` 이 **없는 경로 불만을 지운다** — `LOGIC_CORE` 의 파일을
    옮기면 그 파일이 TOTAL 에서 조용히 빠지고 커버리지가 오히려 올라가며 통과한다. 배열 경로의
    존재를 먼저 대조하게 했다(빠뜨림을 막는 반대 방향 가드는 이미 위에 있다).
- **처방 — 검사가 자기 증거를 갖게 한다**(주석이 아니라 메커니즘):
  1. `mutator-sweep.sh` 의 빌드가 출력을 버리지 않는다. 성공하면 조용하되 `PTB_BUILD_LOG` 에
     남기고, 실패하면 본문을 stderr 로 낸다(전에는 컴파일 **오류** 본문도 함께 사라져서, 빌드가
     깨지면 게이트가 아무 진단 없이 죽었다).
  2. `test-gate.sh` 가 그 로그와 `swift test` 로그를 **함께** 긁는다.
  3. **관측 없음을 통과로 읽지 않는다** — 두 로그에 모듈 컴파일 흔적(`Compiling PokeTokenBar `)이
     없으면 CI 에서는 실패시키고, 로컬에서는 조용한 ✓ 대신 "아무것도 보지 못했다" 를 찍는다.
     한계: 파일 하나만 재컴파일된 부분 빌드는 통과한다(그 파일의 경고만 보인다). 전수 보장은 매
     실행 clean build 뿐이라 값이 안 맞는다.
  4. `ci.yml` 의 게이트 앞 빌드 단계는 그대로 빼 둔다 — 이제 되돌려도 3번이 빨강을 낸다.
  5. **`swift test` 실패가 warning 검사를 건너뛰지 못하게 한다.** `set -e` 로 즉시 나가면 경고를
     한 번도 보고하지 않고, 테스트를 고쳐 재실행할 때는 그 빌드가 warm 이라 경고가 다시 찍히지
     않는다 — 결함을 유발한 바로 그 실행에서 검사가 두 번 연속 눈을 감는다. 종료코드를 들고
     있다가 warning 검사 뒤에 낸다. 로케일 재실행·커버리지는 테스트 산출물이 필요하므로 그
     자리에서 끝낸다.
- **명시적 결정 — 스쿼시 머지 커밋은 게이트 대상이 아니다**(2026-09-07). cold build 앵커인
  `ci.yml` 의 `build-test` 는 `if: github.event_name == 'pull_request'` 라 `main` push 에서는
  돌지 않는다. macOS 러너가 사설 저장소에 10배로 과금돼 한 달 분을 닷새에 태운 실측 때문이고,
  그 절충을 유지한다. **따라서 두 PR 이 교차 머지된 `main` 조합은 어느 잡도 검사한 적이 없다** —
  warning 검사도 그 조합을 본 적이 없다(`ec7114c` 핫픽스가 그 부류였다: 두 PR 이 `reset()` 에서
  교차해 `main` 이 컴파일되지 않았다). 다음에 이 자리를 의심하는 사람이 "왜 main 은 안 도나" 를
  결함으로 읽지 않게 여기 박아 둔다. 되돌릴 조건: macOS 분 과금이 문제가 아니게 되면 `if` 를
  지워 `main` push 도 게이트에 넣는다.
- **같은 부류의 두 번째 구멍 — 충돌한 PR 도 검사받지 않는다**(2026-09-07 실측). `pull_request`
  워크플로는 `refs/pull/<N>/merge` 위에서 도는데, PR 이 `main` 과 충돌하면 GitHub 이 그 ref 를
  만들지 못해 **런이 아예 생성되지 않는다**. 이 PR(#279)이 그 상태였다: `gh pr checks` 는
  `no checks reported`, `gh api .../actions/runs?head_sha=…` 는 `total_count: 0`. 체크가
  **빨강이 아니라 없는 것**이라, 목록만 보면 "아직 안 돌았나" 와 구별되지 않는다 — 이 항목의
  주제인 "관측 없음을 통과로 읽는다" 가 CI 층에서 한 번 더 나타난 것이다. 리베이스로 충돌을
  없애자 같은 sha 에서 런이 즉시 큐에 들어갔다. **머지 전에 `mergeable` 을 본다**:
  `gh pr view <N> --json mergeable,mergeStateStatus` 가 `CONFLICTING`/`DIRTY` 면 초록도
  빨강도 없는 상태다.
  (`ci.yml`·`scripts/test-gate.sh`·`scripts/mutator-sweep.sh`, 2026-09-07.)

## 파생 프로퍼티가 **옛 축**으로 묻고 있으면, 그 축이 갈리는 순간 새 값이 조용히 틀린 답을 받는다

- **증상**: `BattleVolatile.healDivisor` 가 `targetsUser ? 16 : nil` 이었다. 자기에게 거는
  volatile 이 아쿠아링·뿌리박기 둘뿐일 때는 맞는 식이었지만, 배율만 얹는 다섯(기합충전·
  레이저포커스·작아지기·방어태세·충전)이 같은 축에 들어오자 **기합충전이 매 턴 최대 HP 의
  1/16 을 회복하는 기술**이 됐다. 컴파일은 통과했다.
- **직접원인**: "자기에게 거는가" 는 대상을 정하는 축이고 "턴 끝에 회복하는가" 는 효과의 축인데,
  둘이 우연히 같은 두 값으로 갈리던 시기에 앞의 축을 뒤의 축의 대리로 썼다.
- **테스트/리뷰가 왜 못 걸렀나**: `Bool` 축을 재사용한 삼항식은 case 가 늘어도 컴파일러가
  아무 말을 하지 않는다. 같은 파일의 `residualDamage` 는 `switch` 라 case 를 늘리는 순간
  "switch must be exhaustive" 로 멈춰 섰다 — 갈래를 열거한 쪽만 새 값을 물어봤다.
- **부류**: 열거형의 파생 프로퍼티가 `switch self` 대신 **다른 파생 프로퍼티/불리언**으로
  답하는 자리 전부. 그 축이 지금 몇 갈래인지와 무관하게, 축이 하나 갈리면 조용히 틀린다.
- **처방**: 효과를 답하는 파생 프로퍼티는 **`switch self` 로 모든 case 를 열거한다**(`default`
  도 쓰지 않는다 — 새 case 가 기본값으로 흘러들면 같은 침묵이다). 새 case 를 더할 때
  컴파일러가 멈춰 세우는 것이 이 부류의 유일한 방어선이다.
  잠근 자리: `BattleBoostVolatileTests.testNoBoostVolatileMovesHPAtTheEndOfATurn` 이 다섯 전부
  턴 끝에 HP 를 만지지 않는지 본다. (2026-09-07.)

## 생존 가드가 뒤 분기를 대신 막아 주면, 그 분기를 지운 결함이 초록으로 지나간다

- **증상**: 입장 데미지(압정뿌리기 부류)를 밟는 루프는 개체가 쓰러지면 남은 것을 밟지 않고 나온다
  (`if !side.isAlive { break }`). 그 `break` 를 지우는 결함을 주입했는데 **아무 테스트도 걸리지
  않았다.** 이유는 뒤에 오는 것이 독압정 하나였고, 상태를 붙이는 `inflict` 가 자기 입구에서 이미
  `isAlive` 를 본다는 것이다 — 즉 뒤 분기의 가드가 앞 분기의 실수를 덮어 준다.
- **왜 그래도 결함인가**: 데미지형(스텔스록·압정)이 뒤에 오면 가려 주는 가드가 없다. 쓰러진 뒤에
  한 번 더 깎으려 하면 실제로 깎이지는 않아도 `.damage(amount: 0)` 줄이 로그에 남아 "맞았는데 0"
  으로 읽힌다.
- **왜 테스트가 못 걸렀나**: 기절 테스트를 **상태형 하나**(독압정)로만 썼다. 트리거 브랜치를
  재현한다는 것은 "쓰러진 뒤에 무엇이 오는가" 의 **두 종류**(데미지형·상태형)를 다 지나는 것이다.
- **부류 일반화**: 새 가드를 넣을 때는 **그 가드가 없으면 어떤 값이 나가는지**를 단언한다. 뒤에
  오는 코드가 우연히 같은 조건을 다시 보고 있으면, 그 코드를 밟는 테스트는 앞 가드를 재지 않는다.
- **영구 캡처**: `BattleEntryHazardTests.testNoZeroDamageLineIsLeftAfterAHazardFaint` — 스텔스록으로
  쓰러진 뒤 압정이 줄을 하나 더 내지 않는지를 이벤트 배열 전체 비교로 본다.
  (`BattleModel.applyEntryHazards`, 2026-09-07.)

## 새 훅을 모드마다 배선할 때, 테스트가 **내 쪽 입구만** 밟으면 상대 쪽 입구는 무측정이다

- **증상**: 압정 밟기를 출전 자리마다 배선했는데, 웨이브(2대2)의 **상대 자동 출전** 자리에서
  호출을 지우는 결함이 아무 테스트도 깨지 않았다. 배선한 자리는 셋(내 보충 출전·내 턴 교체·상대
  자동 출전)이었고 동작 테스트는 앞의 둘만 있었다.
- **왜 테스트가 못 걸렀나**: 소스 스캔 가드(`testEverySendOutSiteAppliesEntryHazards`)는 **파일
  단위**로 "이 파일이 그 함수를 부르나" 를 본다. 한 파일에 입구가 셋이면 하나만 남겨도 통과한다 —
  스캔은 "새 모드가 통째로 빠지는 것" 을 막고, **입구 하나가 빠지는 것**은 막지 못한다.
- **부류 일반화**: 자동 진행(내가 아니라 CPU·상대가 트리거하는) 경로는 손으로 눌러 보는 흐름에서
  잘 안 밟혀 테스트가 늘 내 쪽으로 쏠린다. 훅을 배선하면 **입구를 세고** 그 수만큼 동작 테스트를
  둔다(스캔은 그것을 대신하지 못한다).
- **영구 캡처**: 모드·입구별 동작 테스트 여섯
  (`testTeamPractice*`·`testWave*`·`testNetBattle*` in `BattleEntryHazardTests`) + 파일 단위 스캔.
  (`WaveBattle.advanceFainted`, 2026-09-07.)

## 한 아이템을 네 자리에서 따로 묻는 부류 — `default:` 가 새 종류를 조용히 삼킨다

- **증상**: 가방은 아이템 하나에 같은 질문을 네 번 한다(쓸 수 있나·효과 힌트·실제 사용·설명 문구).
  네 자리가 각각 `ItemKind` 를 switch 하고 `default:` 로 진화 아이템 40여 종을 받고 있었다. 진화가
  아닌 새 아이템(테라피스)은 **네 곳에 다 적어야** 했고, 한 곳만 빠뜨리면 컴파일은 통과한 채 그
  아이템이 "진화 가능할 때 사용" 을 띄우고 `useEvolutionItem` 으로 흘러가며 설명이 빈 문자열이 된다.
- **왜 테스트가 못 걸렀나**: 처음엔 소스 문자열 스캔으로 "가방이 이 아이템을 언급하나" 를 물었다.
  스캔은 **파일 단위**라, 네 자리 중 하나만 지우는 결함이 나머지 셋에 걸려 통과했다(결함 주입에서
  실제로 지나갔다 — 위의 "내 쪽 입구만 밟으면" 과 같은 부류다).
- **영구 캡처**: 축을 하나 뽑았다(`ItemKind.bagUse`). 네 자리가 그 열거형을 **`default:` 없이**
  switch 하므로 빠뜨림은 이제 컴파일 오류다(주입해 확인: `switch must be exhaustive` 2건).
  남은 테스트는 축이 진화 규칙과 어긋나지 않는지(`testTheBagAxisAgreesWithTheEvolutionRule`)와
  모든 아이템에 세 언어 설명이 있는지(`testEveryItemHasADescriptionInEveryLanguage`)만 본다.
- **부류 일반화**: 같은 질문을 여러 자리에서 물으면 스캔이 아니라 **축**을 뽑는다. 문자열 스캔은
  "새 파일이 통째로 빠지는 것" 을 막고, "한 파일 안 여러 자리 중 하나" 는 컴파일러만 막는다.
  (`ItemKind.bagUse`·`BagView`·`Localization.itemDescription`, 2026-09-07.)

## 후보 풀에서 현재 값을 빼는 코드는 **모든 시작값으로** 돌려야 잠긴다

- **증상**: 테라피스는 "현재와 다른 타입" 을 뽑는다(민트가 성격에서 하는 것과 같다). 제외
  필터(`filter { $0 != current }`)를 지우는 결함을 주입했는데 테스트가 통과했다 — 시작 타입을
  하나(`.fairy`)만 넣었고, 그 seed 가 우연히 다른 타입을 뽑았다.
- **왜 테스트가 못 걸렀나**: 18종 중 하나를 고른 셈이라 결함이 드러날 확률이 1/18 이었다.
  `docs/reference/defect-log.md` "테스트가 시스템 RNG 를 밟고 있는 부류" 와 뿌리가 같다 —
  **결정적으로 실패하게 만들지 않으면 확률에 기댄 초록**이다.
- **영구 캡처**: 18종을 다 시작값으로 돌린다(`testUsingAShardAlwaysChangesTheTeraTypeAndSpendsOne`).
  제외를 안 하는 구현은 그중 한 번 반드시 자기 값을 뽑는다.
- **부류 일반화**: "현재 값을 뺀 무작위" 는 시작값 **전 범위**를 돌린다. 범위가 커서 못 돌리면
  풀 자체를 반환하는 함수를 갈라 그 집합을 직접 단언한다.
  (`CompanionStore.useTeraShard`, 2026-09-07.)

## 결함 주입 **판정기**가 고장 나면, 통과 보고가 통째로 거짓이 된다

- **증상**: 지닌물건 3종의 새 분기 15곳에 결함을 주입해 빨개지는지 자동으로 확인했는데, 7건이
  "초록으로 지나감" 으로 보고됐다. 손으로 하나를 다시 돌려 보니 실제로는 빨갰다 — 판정기가
  `swift test` 출력에서 `with (\d+) failures` 를 찾고 있었고, 실패가 **하나뿐일 때** XCTest 는
  `with 1 failure`(단수)를 찍는다. 정규식이 안 맞으면 실패 수를 못 읽고, 그 자리를 "0건" 으로
  접어 초록으로 보고했다.
- **왜 못 걸렀나**: 판정기 자체에는 테스트가 없다. 그리고 **거짓 초록이 그럴싸했다** — 새 테스트가
  한 결함에 한 건만 빨개지는 것은 정상이라, 보고를 보고도 이상해 보이지 않았다. 15건 중 7건이
  한꺼번에 초록이라는 **비율**이 유일한 신호였다.
- **영구 캡처**: 주입 판정은 실패 **수**로 접지 말고 실패한 **테스트 이름**으로 판정한다
  (`error: -[…Tests (test\w+)\]` 를 모아 비면 초록). 이름 목록은 단수·복수를 타지 않고, 어느
  테스트가 잡았는지까지 남아 "의도한 그 테스트가 잡았나" 를 함께 볼 수 있다.
- **부류 일반화**: 검증 도구의 출력 파싱은 **양쪽 극단을 한 번씩 손으로 확인**한다(0건과 1건).
  자동 판정기가 조용히 관대해지는 방향으로 고장 나면, 그 뒤의 모든 "확인했다" 가 근거를 잃는다.
  (`/tmp/inject.py` 임시 스크립트, 2026-09-07.)

## 앞 단계가 데미지를 **이미 잘라 놨으면**, 뒤 단계의 중복 가드는 죽은 조건이다

- **증상**: 기합의띠(만피에서 한 방 버티기)를 인내와 같은 자리에 넣으면서 `!endured` 를 조건에
  덧붙였다("인내가 버텼으면 띠는 일하지 않는다"). 그 가드를 지우는 결함을 주입했는데 테스트가
  통과했다 — 인내가 위에서 데미지를 `hp - 1` 로 잘라 두므로 띠의 `damage >= hp` 가 이미 거짓이다.
- **왜 테스트가 못 걸렀나**: 테스트는 "둘 다 발동하지 않는다" 는 **결과**를 봤고, 그 결과를 만드는
  것은 두 조건 중 하나뿐이었다. 즉 가드는 처음부터 도달 불가였고 커버리지도 그것을 말해 주지
  못한다(그 줄은 매번 평가된다).
- **영구 캡처**: 죽은 가드를 **지웠다.** 지운 뒤 같은 테스트는 인내의 자르기를 없애는 결함에
  빨개진다 — 이제 그 테스트가 실제 메커니즘을 잠근다(주입해 확인, 2건 빨강).
  왜 두 개가 동시에 발동하지 않는지는 주석이 그 자리에 남긴다.
- **부류 일반화**: 같은 값을 두 단계가 차례로 자르는 자리에서는, 뒤 단계에 "앞 단계가 일했나" 를
  다시 묻지 않는다. 물으면 그 조건은 검증할 수 없는 채로 남아 앞 단계의 회귀를 덮는다. 앞의
  "생존 가드가 뒤 분기를 대신 막아 주면" 과 같은 뿌리다.
  (`BattleEngine.applyHit` 의 인내·기합의띠, 2026-09-07.)

## 개체에 붙는 소모품은 **와이어 값과 소모 여부를 갈라** 둬야 한다

- **증상**: 기합의띠는 배틀 안에서 1회용이다. "썼다" 를 스냅샷의 `heldItem` 을 `nil` 로 만들어
  표현하면 그 스냅샷을 다시 쓰는 자리(재입장·정산)가 아이템 없는 개체를 보고, 반대로 세이브의
  재고를 깎으면 배틀이 가방을 소모하는 새 규칙이 생긴다.
- **영구 캡처**: 소모는 `BattleSide.heldItemConsumed`(와이어에 없는 지역 값)에만 두고, **읽는
  자리를 파생 프로퍼티 하나로 접었다**(`BattleSide.heldEffect`). 그 프로퍼티가 소모 플래그를
  무시하는 결함과, 소모를 기록하지 않는 결함이 각각 같은 테스트를 빨갛게 만든다
  (`testTheFocusSashIsSpentAfterItTriggers`, 주입해 확인).
- **부류 일반화**: 와이어로 온 값(무엇을 지녔나)과 이 판에서만 사는 값(그것을 썼나)은 다른 칸에
  두고, 둘을 합친 답은 **파생 프로퍼티 한 곳**에서만 낸다. 원본을 지워 상태를 표현하면 그 원본을
  읽는 다른 자리가 전부 조용히 틀린다.
  (`BattleSide.heldEffect`·`heldItemConsumed`, 2026-09-07.)

## 본가가 "행동할 때마다" 인 대가는, **대상마다 부르는 입구**에서 대상 수만큼 과금된다

- **증상**: 생명의구슬의 자해는 본가에서 "공격이 데미지를 넣을 때마다 최대 HP 의 1/10" 이다. 이
  엔진의 그 자리(`applyHit`)는 **광역기가 대상마다 직접 부르는** 입구라, 거기 두면 2체를 때린 턴에
  1/5 를 잃는다(화면에는 그냥 HP 가 더 줄어든 것으로 보인다).
- **영구 캡처**: 대가를 **턴 끝 한 자리**(`endOfTurnResidual`)로 옮겼다 — 개체당 턴당 한 번 도는
  자리라 네 모드가 같은 규칙을 받고 난수도 안 쓴다. 본가와 갈리는 선택(공격하지 않은 턴에도
  깎인다)이므로 코드 주석과 아이템 설명 문구에 그렇게 적었다.
- **부류 일반화**: "행동마다" 로 적힌 규칙을 옮길 때는 **그 함수가 한 턴에 몇 번 불리는지**를 먼저
  센다. 방어 판정·상대 volatile 이 그 입구로 내려온 것과 같은 이유이고, 방향만 반대다(그 둘은
  대상마다 필요해서 내려왔고, 이것은 대상 수와 무관해서 올라갔다).
  (`BattleEngine.endOfTurnResidual`, 2026-09-07.)

## 회복원이 둘이 되는 자리는 **대체인지 합산인지**를 테스트가 말해야 한다

- **증상**: 턴 끝 회복은 이미 웨이브 런의 스택(`RunBoosts.leftovers`)이 쓰고 있었고, 거기에
  지닌물건(먹다남은음식)이 들어왔다. 둘이 겹치는 판이 실제로 있는데(웨이브 런에서 그 아이템을
  지니면) "합산" 과 "한쪽이 이긴다" 는 코드로는 한 줄 차이고 화면으로는 구별되지 않는다.
- **영구 캡처**: 합산으로 정하고 **한 줄로** 낸다(같은 회복을 두 줄로 내면 로그가 두 번 회복한
  것처럼 읽힌다). 테스트는 스택만 있는 판의 회복량을 먼저 재고, 둘 다 있는 판이 그 값 + 아이템
  몫인지 본다 — 한쪽이 다른 쪽을 대체하는 결함이 빨개진다(주입해 확인).
- **부류 일반화**: 같은 축의 두 번째 공급원이 생기면 합산 규칙을 **주석과 테스트 양쪽에** 적는다.
  더 흔한 실패는 둘 다 세는 것이 아니라 **한쪽이 다른 쪽을 조용히 대체하는** 것이다.
  (`BattleEngine.endOfTurnResidual`, 2026-09-07.)

## 세이브·와이어의 열거형은 **모르는 이름에 던지면** 그 파일 전체를 잃는다

- **증상**: 지닌물건을 `ItemKind` 로 타입된 채 디코딩하면, 모르는 rawValue 하나가
  `DecodingError` 를 던진다. 그 오류는 아이템 하나가 아니라 **개체 전체**(세이브는 기본값 = 알로
  리셋, 스냅샷은 대전 자체가 성립하지 않음)를 잃게 만든다. 앱을 되돌려 설치한 사용자와, 아이템을
  하나 늘린 신버전 피어와 붙는 옛 버전이 각각 밟는 경로다.
- **영구 캡처**: 두 자리 모두 `String` 으로 받아 `ItemKind(rawValue:)` 로 접는다(모르면 `nil`).
  특성 슬러그를 원문으로 싣는 정책과 같다. 접는지는 세이브·와이어 양쪽에서 따로 잠근다
  (`testAnUnknownHeldItemFoldsToNil`·`testAnUnknownHeldItemInTheSaveFoldsToNil`, 주입해 확인).
  공정성은 화이트리스트(`MultiplayerValidation.validHeldItem`)가 계속 본다 — 거절과 접기는 다른 일이다.
- **부류 일반화**: 값 하나를 못 읽는 대가가 **파일·세션 전체**면 던지지 않는다. 던지는 디코딩은
  그 값이 없으면 아무것도 못 하는 필드에만 쓴다.
  (`BattleSnapshot.init(from:)`·`MonState.init(from:)`, 2026-09-07.)

## 이벤트가 **나왔는지만** 보는 단언은 그 이벤트의 양이 틀린 결함을 통과시킨다

- **증상**: 대타출동의 인형을 때린 드레인·반동이 "인형에 실제로 들어간 만큼" 이 아니라 **자른 적
  없는 원래 데미지**를 기준으로 계산되게 결함을 주입했더니 테스트가 초록이었다. 단언이
  "회복 줄이 있는가"·"반동 줄이 있는가" 만 봤기 때문이다. 인형 HP 가 39 인데 250 위력 한 방을
  맞으면 그 결함은 인형 HP 의 몇 배를 빨아들이지만, 줄은 똑같이 한 줄 나온다.
- **근본원인**: 층·상한·클램프가 붙은 자리의 결함은 **줄의 유무가 아니라 숫자**로만 드러난다.
  존재만 보는 단언은 그 자리에서 아무것도 잠그지 않는다.
- **영구 캡처**: 값을 자르는 코드(상한·클램프·흡수) 옆의 테스트는 이벤트의 **양에 상한 단언**을
  건다(`testDrainAndRecoilCountOnlyTheDamageThatLandedOnTheSubstitute`). 첫 실행부터 초록인
  단언을 의심하라는 규칙과 짝이다 — 주입해서 빨개지는지 확인한 뒤에만 잠근 것으로 친다.
- **부류 일반화**: `events.contains { if case .heal = $0 ... }` 꼴은 그 이벤트가 **처음 생기는지**를
  볼 때만 쓴다. 이미 있는 이벤트의 계산을 바꾸는 변경이면 `compactMap` 으로 값을 꺼내 비교한다.
  (`BattleEngine.applyHit`, 2026-09-08.)

## 공격측·방어측을 **각각 사본으로 꺼내 쓰는 루프**는 둘이 같은 자리일 때 쓰는 순서가 곧 결과다

- **증상**: 방(`MultiplayerBattle`)의 라운드 해상은 공격측·방어측을 지역 사본으로 꺼내
  `applyAttack` 에 넘기고 둘 다 배열에 쓴다. 자기에게 거는 기술(따라와·대타출동·방어)은 공격측과
  방어측이 **같은 참가자**라, 공격측을 먼저 쓰고 방어측을 나중에 쓰면 시전 **전에** 뜬 방어측
  사본이 방금 붙은 상태를 통째로 덮어쓴다. 로그에는 "기술을 썼다 · 상태가 붙었다" 가 그대로
  남으므로(이벤트는 `applyAttack` 이 이미 만들었다) 화면만 보면 성공한 판과 구별되지 않는다.
- **근본원인**: "공격측과 방어측은 서로 다른 자리" 라는 전제가 코드 어디에도 적혀 있지 않았고,
  그 전제가 깨지는 입력이 **다른 가드에 막혀** 한 번도 들어오지 않았다(아래 항목).
- **영구 캡처**: 방어측을 **먼저**, 공격측을 **나중에** 쓴다(둘이 같은 자리면 공격측 사본이
  이긴다 — 그쪽이 시전 결과를 들고 있다). 회귀 테스트는 **이벤트가 아니라 상태**로 잰다
  (`testASelfAimedMoveSurvivesTheRoundInTheRoom`, 주입해 확인). 웨이브 런의 아군 대상 경로도
  같은 순서를 쓴다.
- **부류 일반화**: 같은 배열의 두 원소를 사본으로 꺼내 돌리는 자리는 **두 인덱스가 같을 수 있는지**
  를 먼저 답한다. 같을 수 있으면 쓰는 순서를 주석으로 고정한다.
  (`MultiplayerBattle.resolveRound`·`WaveBattle.execute`, 2026-09-08.)

## 입력을 막는 가드가 있으면 그 뒤의 결함은 **테스트로도 안 보인다**

- **증상**: 방의 사전 검증이 "자기 자신은 못 때린다" 로 **자기 지목 액션을 통째로 거절**했다.
  그래서 자기에게 거는 기술(방어·대타출동·회복)은 방에서 낼 방법이 아예 없었고, 그 사실을
  아무도 몰랐다 — 낼 수 없으니 실패하는 테스트도 없다. 그 문을 열자 곧바로 위의 덮어쓰기
  결함이 드러났다.
- **영구 캡처**: 자기 대상 기술은 검증에서 갈래를 따로 둔다(`targetsUser`·`targetsAlly` 를
  데이터가 답한다). 새 대상 부류를 열 때는 **그 부류로 한 번 끝까지 해상하는 테스트**를 같이 넣는다.
- **부류 일반화**: "이 경로는 아무도 안 밟는다" 는 안전의 근거가 아니라 **무측정의 근거**다.
  경계에서 거절되는 입력이 있으면, 그 입력이 정당해지는 날 뒤쪽 코드가 처음으로 실행된다.
  (`MultiplayerBattle.resolveRound` 사전 검증, 2026-09-08.)

## 스스로 행동을 만드는 자리(CPU)는 모드의 **사전 검증을 지나지 않는다**

- **증상**: 기술 선택을 막는 부류(도발·앙코르·구애)를 붙이자 사람이 고르는 경로는 전부 막혔는데
  CPU 는 그대로 냈다. 웨이브·모의전·방의 CPU 와 보스 폴백·토너먼트 자동 진행이 후보를
  **남은 PP** 로만 골랐기 때문이다. 다섯 자리 중 넷은 액션을 직접 만들어 모드의 사전 검증
  (`canUse` 를 보는 그 검증)을 지나지 않으므로, 화면에는 도발이 걸린 것으로 보이고 효과만
  새어 나간다.
- **근본원인**: 선택 규칙을 `BattleSide.canUse(moveAt:)` 한 자리에 뒀는데, **고르는 자리**가
  그 함수를 부르지 않아도 컴파일이 통과한다. 사람이 고르는 경로만 테스트했고(그쪽은 검증을
  지난다) CPU 경로는 "PP 만 보면 되는 자리" 로 남아 있었다.
- **영구 캡처**: 다섯 자리를 `canUse(moveAt:)` 로 바꾸고, 부류를 **소스 스캔 가드**로 닫았다
  (`testNoChooserPicksAMoveByRawPPAlone` — `pp[$0] > 0`·`pp.firstIndex(where: { $0 > 0 })` 를
  0건으로 잠근다). 엔진까지 실제로 닿는 웨이브 CPU 는 행동 테스트로도 잠근다
  (`testTheWaveCPUPicksStruggleWhenEveryMoveIsLocked`).
- **부류 일반화**: 사람의 입력을 검증하는 게이트를 새로 만들면 **같은 결정을 자동으로 내리는
  자리**를 함께 찾는다(CPU·폴백·자동 진행·재접속 복구). 그 자리들은 검증 앞이 아니라 **옆**에
  있어서, 게이트를 늘려도 조용히 예외로 남는다.
  (`WaveBattle.cpuAttacks`·`TeamPracticeBattle.cpuMoveChoice`·`MultiplayerBattle` 보스·자동
  액션·`PokemonTournament.automatic`, 2026-09-08.)

## 대체 행동(발버둥)의 조건은 **막는 규칙이 늘 때마다** 같이 넓혀야 한다

- **증상**: `mustStruggle` 이 남은 PP 만 셌다. 변화기만 든 개체가 도발당하면 고를 수 있는 칸이
  하나도 없는데 "낼 기술이 있다" 로 읽혀, 아무 기술도 못 내고 발버둥도 안 나오는 턴이 됐다.
- **근본원인**: "고를 수 있나" 를 두 곳이 서로 다른 기준으로 답했다 — 칸 하나는
  `canUse(moveAt:)`(잠금까지 본다), 전체는 `mustStruggle`(PP 만 본다). 기준이 갈리면 새 잠금이
  한쪽에만 반영된다.
- **영구 캡처**: `mustStruggle` 을 `canUse(moveAt:)` 의 전수 판정으로 바꿨다(기준이 하나가 된다).
  회귀 테스트는 변화기만 든 개체를 도발하는 트리거 그대로다
  (`testAMonWithNothingLeftToPickMustStruggle`).
- **부류 일반화**: "하나라도 가능한가" 를 묻는 값은 **개별 판정 함수에서 파생**한다. 같은 조건을
  두 번 적으면 둘 중 하나만 자라고, 자라지 않은 쪽이 대체 행동을 막는다.
  (`BattleSide.mustStruggle`, 2026-09-08.)

## 선택을 막는 규칙은 **선택이 끝난 뒤에 걸릴 수 있다** — 게이트가 한 자리면 그 턴이 새어 나간다

- **증상**: 도발을 건 쪽이 먼저 움직인 턴에서, 맞은 쪽이 **이미 골라 둔** 변화기가 그대로 나갔다.
  화면에는 도발이 걸린 것으로 보이고 효과만 한 턴 늦게 들었다.
- **근본원인**: 잠금을 **선택 경로 한 곳**(`BattleSide.selectionLock(for:)`)에만 뒀다. 선택과
  실행 사이에 상대 행동이 끼는 순서를 계산에 넣지 않은 것이다. 테스트는 전부 "걸린 다음 턴에 못
  고른다" 를 재현해 트리거가 되는 순서(같은 턴 안에서 걸림)를 한 번도 밟지 않았다.
- **부류 스윕**: 잠금 일곱을 쇼다운 `moves.ts` 와 대조했다 — 넷(씨앙코르·도발·봉인·비밀의힘)은
  `onBeforeMove` 로 실행도 막고, 트집·앙코르는 `onDisableMove` 만 들어 선택만 막는다(본가도
  그렇다). 구애는 자기 기술이 나가는 순간 걸려 선택과 실행 사이에서 값이 달라질 수 없다.
- **영구 캡처**: 잠금이 스스로 답하게 축을 하나 뒀다(`MoveSelectionLock.blocksExecution`) —
  `BattleEngine.beginAttack` 이 그 축만 보고 막는다. 회귀 테스트는 트리거 순서 그대로다
  (`testTauntBlocksAMoveThatWasAlreadyChosenThisTurn`), 그리고 반대 방향도 잠근다
  (`testTormentAndEncoreStayOutOfTheExecutionGate` — 넷을 다 막으면 본가보다 세진다).
...
## "지금의 가변 상태"를 고정 슬롯에 박아 저장하면, 그 상태가 바뀐 뒤 옛 값이 배열에 남는다

- **증상**: 모험 파티 1번(활성 파트너)을 다른 개체로 바꾸면(`switchCompanion`), 2~6번 예비가
  전원 한 칸씩 밀렸다 — 사용자 보고: "1번은 지금 선택한 포켓몬 고정인데 다른걸로 선택 바꾸면
  하나씩 밀리는 구조".
- **근본원인**: `setHomeParty` 가 저장할 때 `state.homePartyIDs = [active.id] + reserves` 로
  **그 순간의 활성 id 를 배열 맨 앞에 박아 저장**했다. 이후 `switchCompanion` 으로 활성이
  바뀌면 배열 속 값은 그대로인데 그건 이제 **예전** 활성 id 다. 읽는 쪽(`homeParty` getter)의
  `seen` 은 **지금** 활성만 걸러내므로, 예전 활성 id 가 "사용자가 고른 예비"로 오인돼 2번 자리에
  끼어들고 원래 2~6번이 하나씩 밀린다.
  - **테스트가 왜 못 걸렀나**: 기존 `homeParty` 테스트는 전부 `setHomeParty` 로만 파티를
    편성하고 검증했다 — **활성이 바뀐 뒤** `homeParty` 를 다시 읽는 테스트가 없었다. 원장류
    필드(레이드)는 "재로드"가 트리거였는데, 이건 "활성 교체"가 트리거라는 점만 다르고 부류는
    같다: **상태를 저장한 뒤, 그 상태의 전제(무엇이 활성인가)가 바뀌는 경로를 아무도 밟지 않았다.**
- **부류 스윕**: `[active.id] + ...` 형태로 "지금 값"을 배열에 박아 저장하는 다른 필드가
  있는지 grep 했다 — `homePartyIDs` 하나뿐이었다.
- **영구 캡처**: 저장 형식을 **예비만**(활성 미포함)으로 바꿨다 — `state.homePartyIDs =
  Array(reserves.prefix(5))`. 활성이 나중에 바뀌어도 배열 안에는 애초에 활성 id 가 없으니
  오염될 여지가 없다. 값 자체가 바뀌므로(같은 예비 조합도 canonical 문자열이 달라진다)
  `integrityVersion` 을 11 → 12 로 올렸다(레이드 원장 결함과 같은 턴에 났지만 서로 다른
  canonical 세그먼트라 별개로 올린다). 회귀 테스트
  (`testSwitchingActiveCompanionKeepsTheRestOfTheHomeParty`)는 수정 **전** CI 에 올려 실패를
  확인한 뒤(2026-09-08) 고쳤다.
- **부류 일반화**: "지금 이 값"(활성 개체·현재 선택·현재 국면 등 **가변** 상태)을 저장 형식
  안에 **고정 위치로** 끼워 넣지 마라. 그 상태가 나중에 바뀌면 저장된 옛 값이 배열/목록 안에
  남아 "여전히 유효한 항목"처럼 다시 읽힌다. 가변 상태는 별도 필드(`state.active`)로 두고,
  목록에는 그 가변 상태와 **겹치지 않는 항목만** 저장하는 것이 안전하다.
## 와이어 enum 에 case 를 더하면, 규칙 버전이 아니라 **프로토콜 버전**이 올라야 한다

- **증상**(출시 전 발견): 배틀 엔진에 볼라틸·지닌물건·기술 잠금을 더하면서 `BattleEvent` 에 case
  다섯이 늘었는데(#291) `rulesVersion` 만 24 로 올리고 `MultiplayerWireMessage.protocolVersion` 은
  18 그대로 뒀다. 구버전 게스트는 입장 검사(값이 같다)를 통과한 뒤 새 case 가 처음 뜨는 라운드에서
  `roundResolved` 디코딩에 실패하고, 수신부가 그 실패를 연결 종료로 처리해 방 밖으로 튕긴다.
- **근본원인**: 두 버전 상수가 뜻이 다른데 이름이 비슷하다. 방은 `rulesVersion` 을 **읽지 않는다** —
  그 사실이 주석으로만 있었다. 그리고 `BattleEvent` 는 associated value 를 든 자동합성 `Codable` 이라
  모르는 case 를 `nil` 로 접지 않고 throw 한다.
- **왜 안 걸렸나**: 엔진 테스트는 case 를 늘려도 전부 초록이고(엔진 안에서만 쓰니까), 버전 테스트는
  값을 그대로 두면 초록이다. 두 사실을 잇는 단언이 없었다. 나머지 버전 단언이 전부 `>= N` 형태라
  상향을 강제하지도 않는다.
- **영구 캡처**: `BattleEvent` 의 case 수를 소스에서 세어 동결한다
  (`LobbyRoleTests.testBattleEventCaseCountIsFrozenAgainstTheProtocolVersion` — Swift 가 associated
  value 를 든 enum 을 열거하지 못해 소스를 읽는다). 실패 문구가 "숫자만 고치지 말고 `protocolVersion`
  도 올려라" 를 말한다.
- **부류 일반화**: 와이어에 실리는 열거형·구조체의 **모양**이 바뀌면 규칙 버전이 아니라 프로토콜
  버전이다. 판정 기준은 "구버전 피어가 이 바이트를 디코딩할 수 있나" 하나다. (2026-09-08.)

## 상태 갱신 메시지가 **화면 단계까지** 되돌리면, 진행 중인 판이 통째로 멈춘다

- **증상**: 레이드가 도는 중에 누가 방에 들어오면 싸우던 게스트 전원이 배틀 화면에서 로비로 튕기고,
  그 뒤로 판이 멈춘 것처럼 보였다. 정산도 못 받았다.
- **근본원인**: 호스트는 입장·이탈·연결 끊김마다 `broadcastLobby()` 로 명단을 다시 뿌리는데, 게스트의
  `.lobby` 처리가 명단과 함께 `phase = .joined` 를 무조건 세웠다. `.battling` 을 세우는 것은
  `.start`·`.raidStart` 뿐이라 되돌아갈 길이 없고, 이후 라운드는 `applyGuestResolvedRound` 의
  `phase == .battling` 가드에 걸려 조용히 버려진다.
- **왜 안 걸렸나**: 준비·팀 변경에는 `where !self.isInPlay` 가드가 이미 있었다(같은 부류를 한 번
  겪은 자리다). 입장에는 없었고, 게스트 수신 로직이 수신 클로저 안에 있어 테스트가 밟을 입구가
  아예 없었다 — 레이드는 판이 도는 동안에도 입장을 받는 유일한 모드라 실사용에서만 드러났다.
- **영구 캡처**: 처리를 `applyGuestLobby(_:)` 로 떼어 테스트가 밟게 하고, 판이 도는 중이면 명단만
  바꾼다(`RaidRoomTests.testALobbyUpdateDuringTheRaidDoesNotThrowGuestsOutOfTheBattle` — 라운드가
  계속 들어오는 것까지 본다).
- **부류 일반화**: 브로드캐스트 메시지는 **무엇을 갱신하는지**로 범위를 잘라야 한다. 명단 갱신은
  명단만 바꾼다. 화면 단계·판 상태를 같이 세우는 것은 "판을 시작한다"는 메시지의 몫이다.
  (`MultiplayerRoomCenter.applyGuestLobby`, 2026-09-08.)

## 고정폭 컨트롤만 늘어선 줄에서, **접힐 수 있는 요소가 하나뿐이면** 모자란 폭이 전부 그리로 간다

- **증상**: 포켓몬 탭 머리줄의 "소유 포켓몬" 제목이 한 글자씩 세 줄로 접혀 나왔다.
- **근본원인**: 머리줄 `HStack` 에서 정렬·타입 메뉴는 `.fixedSize()`, 필터 버튼 셋과 마릿수 캡슐은
  고정 폭이라 **줄어들 수 있는 것이 제목 하나뿐**이었다. 팝오버 콘텐츠 폭은 332pt 로 고정인데
  정렬 이름("도감번호순")·마릿수("90/164")가 길어지며 예산을 넘었고, 모자란 몫이 전부 제목으로 갔다.
- **왜 안 걸렸나**: 같은 부류의 가드가 이미 둘 있었지만(트레이너 바 줄·집중 오늘 줄) 이 줄에는 없었다.
  머리줄이 뷰의 `private func` 이라 폭을 제안해 렌더해 볼 입구가 아예 없었던 것이 그 이유다. 앞선
  조사에서 화면 전체를 재 275pt 를 얻고 "안 넘친다"고 판단했는데, 그 값은 `Menu` 를 줄어들 수 있는
  것으로 본 이상적 폭이라 실제와 달랐다 — **한 화면의 이상적 폭은 그 안의 한 줄이 접히는지를 말해 주지
  않는다.**
- **영구 캡처**: 머리줄을 `RosterHeader` 로 떼어 실제 폭(332pt)으로 렌더하고, 넉넉한 폭에서의 높이와
  같은지를 정렬 네 가지 모두에서 본다(`PopoverLayoutTests.testTheRosterHeaderStaysOnOneLineAtThePopoverWidth`).
  제목을 되살려 넣으면 32~48pt 로 커지며 실패한다.
- **부류 일반화**: 팝오버는 폭이 고정이라 한 줄의 폭 예산도 고정이다. 그 줄에 **길이가 변하는 글자**를
  둘 때는 접히는지를 실제 폭으로 렌더해 확인한다. 화면 안에 이미 있는 이름(탭 이름 등)을 머리줄에서
  되풀이하는 제목은 예산만 먹으므로 아이콘으로 줄인다. (`RosterHeader`, 2026-09-08.)

## 기획을 자기 도메인 관용구로 접어 원본의 동사를 잃는 부류

- **남의 기획을 이식할 때, 이 저장소가 잘하는 관용구로 접으면 원본의 동사가 사라진다.**
  포코피아(《포켓몬 포코피아》)의 컨셉은 "메타몽이 변신해 그 포켓몬의 힘으로 마을을 만든다" 인데,
  첫 구현은 그것을 이 저장소가 익숙한 관용구 — **지갑·상한·멱등 가드·일당** — 로 옮겼다.
  결과는 지형별 티켓 8종을 모아 칠하는 자원 수집 게임이었고, **변신은 아바타 그림 + 티켓 종류
  modifier로 격하돼 한 번도 안 해도 게임이 돌아갔다.** 이주도 "메뉴에서 정착 버튼" 이 되어
  "내가 만든 곳이 좋아서 찾아온다" 는 보상의 감정이 남지 않았다.
- **신호는 설계 문서 자신이 냈다.** 계획서와 설계 문서가 두 곳에서 이렇게 적었다:
  ① "원하는 지형의 티켓이 나와야 칠할 수 있으므로 특정 그림을 완성하는 데는 더 걸린다 —
  **그것이 변신을 쓰는 이유다**"(변신을 *우회 수단*으로 설명하고 있다. 원본에서 변신은 우회가
  아니라 유일한 길이다) ② "이 수치는 코드에서 읽은 상한으로 계산한 것이고 실사용 관측치가
  아니다". 둘 다 "이 경제가 기능을 위해 존재하지 않는다" 는 신호였는데 결함으로 읽지 않았다.
- **되짚는 기준: 원본에 없는 명사가 설계 문서의 절 제목으로 등장하는가.** "티켓 경제", "지갑",
  "일당" 은 포코피아에 없는 말이다. 그런 절이 생기면 원본의 동사(변신한다·만든다·살러 온다)가
  아직 코드에 있는지 확인한다. 있으면 관용구가 그 위에 얹힌 것이고, 없으면 관용구가 동사를
  대체한 것이다 — 후자면 관용구를 지운다.
- **관용구 자체는 잘못이 아니다.** 지갑 입구를 하나로 두는 규칙(위 "한 지갑에 지급하는 경로가
  여럿일 때")도, 멱등 가드를 서명 안에 두는 규칙도 옳다. 문제는 **그 규칙을 적용할 자리가
  아닌데 적용한 것**이다. 실제로 재설계 뒤에는 지갑이 사라져 그 규칙을 적용할 대상이 없어졌고,
  이사 멱등은 "같은 종은 두 번 오지 않는다" 는 **도메인 사실**이 구조적으로 보장한다.
- **재설계가 지운 것**: `workTickets`·`grantTownTicket`·`claimTownResidentPayout`·
  `paintTownTile`·`settleTownResident`·`earnedTerrain`·티켓 팔레트 8버튼·주민 일당·
  `residentPayoutDayKey`·테스트 25개. 순증가는 Medium 이었고 삭제가 절반이었다.
  (사용자 리포트 "내가 알던 포코피아랑 다른데", 2026-09-08. 설계는
  [`pokopia-town-design.md`](pokopia-town-design.md).)

## 결정성 단정이 도달 불가 분기를 겨냥해 아무것도 안 지키는 부류

- **결함을 주입해 봤는데 테스트가 초록이면, 주입이 틀렸거나 테스트가 틀렸다 — 둘 다 확인한다.**
  마을 이사의 "굴림은 후보가 없어도 소비된다" 를 지키려고 테스트를 썼는데, 결함 주입
  (`let roll = rng.next()` 를 `guard` 뒤로 이동, 그리고 "부르는 타입이 없으면 조기 반환" 추가)에
  **두 번 다 초록이었다.** 원인 둘:
  ① 첫 주입은 provider 실패 가드 뒤로 옮긴 것인데 **스텁이 항상 성공**해 소비량이 안 갈렸다.
  ② 둘째 주입이 겨냥한 `welcomingTypes().isEmpty` 분기는 **실전에서 도달 불가**였다 —
  꽉 찬 격자는 비둘기집 원리로 언제나 한 지형이 문턱을 넘는다(192칸/8지형 = 최소 24칸).
- **게다가 테스트 자체가 동어반복이었다.** 스토어의 `rng` 가 `private` 이라, 소비를 관측하려고
  `SeededRNG` 를 테스트에서 직접 두 개 만들어 비교했다 — 그건 `SeededRNG` 를 테스트한 것이고
  **스토어를 지나지 않았다.** 통과는 당연했다.
- **고친 형태**: 실패를 켜고 끌 수 있는 **참조 타입 스텁**(구조체 스텁은 스토어에 복사돼 들어가
  나중에 바꿀 수 없다)으로 같은 스토어에서 "한 번 실패 → 성공" 을 만들고, 실패 없는 스토어와
  **도착 순서 전체**를 비교했다. 한 판만 비교하면 후보가 N개일 때 1/N 로 우연히 같아진다
  (위 "테스트가 시스템 RNG 를 밟고 있는 부류" 의 같은 규칙).
- **부산물**: 도달 불가로 판정된 분기를 근거와 함께 남기거나 지웠다. `?? []`·`default:` 처럼
  **전수 표에서 값을 읽는 폴백은 절대 실행되지 않아** 커버리지에 `^0` 으로 영원히 남는다 —
  뒤집은 표(`typesByTerrain`)를 아예 지우고 그 자리에서 파생하는 형태로 바꾸니 사라졌다.
  (그 표는 프로덕션에서 아무도 안 쓰고 테스트만 쓰던 값이기도 했다.)
- **`^0` 을 세는 grep 이 주석에 걸린다.** 이 절과 코드 주석에 적은 `^0` 텍스트가 커버리지 스캔에
  잡혀 "미실행 1건" 으로 보고됐다. region 마커는 `|` 가 없는 줄에만 오므로 `grep -v '|'` 로
  걸러야 한다 — 위 "소스 스캔 가드가 규칙을 설명한 주석에 걸리는 부류" 와 같은 함정이다.
  (2026-09-08.)

## 보간된 이름 바로 뒤에 조사를 붙이면 받침 없는 이름에서 틀린다

- **`"\(name)이에요"`·`"\(name)을"` 은 이름의 받침에 따라 갈린다.** 포코피아 지형 8종 중
  모래·나무·바위는 받침이 없어 "모래이에요"·"모래을" 이 됐다(`PokopiaTownView` 의 피드백 한 줄,
  `PokopiaTransformSheet` 의 VoiceOver 값 한 줄). 포켓몬 이름도 같다 — 피카츄·이상해씨 vs
  파이리·꼬부기. 렌더링 테스트로는 볼 수 없고, 문구 테스트는 리터럴을 박지 않는 규칙이라 잡지
  못한다. (2026-09-09 리뷰.)
- **설계 문서는 이 규칙을 이미 적어 두었다** — 타입 이름 뒤엔 `타입` 을 끼운다("…타입을").
  그 처방이 **타입 이름에만** 적용되고 같은 화면의 지형 이름엔 빠졌다. 규칙이 문서에 있어도
  적용 범위가 "그때 고친 명사" 하나로 좁게 읽힌다.
- **처방**: 이름과 조사 사이에 **받침이 정해진 명사**를 끼운다 — "…지형이에요", "…지형을",
  "…타입을". 조사 계산기를 만들지 않는다(값이 그럴 만큼 많지 않다). 소스 가드
  `PokopiaParticleGuardTests` 가 `\(…)` 바로 뒤의 조사를 정규식으로 세고, 정규식이 원래 위반
  두 줄을 실제로 잡는지도 같은 파일이 확인한다.
- **알려진 잔존 (범위 밖)**: 같은 패턴이 main 에 있다 — `Localization.swift:437,443,447`
  (`\(name)가/는`) · `MemoryHomeRoomLife.swift` (`\(companion)이`, 13곳) · `PokemonAuction.swift`
  · `PokemonTrade.swift` (`\(entry.name)은`). 가드의 접두어(`Pokopia`)를 지우면 저장소 전체를
  세지만 **오늘 main 이 빨개진다** — 그쪽을 먼저 고치고 접두어를 지운다.

## 테스트가 통제 못 하는 전역 입력(벽시계·기기 식별자)은 **그 값이 특정 구간에 들 때만** 빨개진다

- **`Date()` 를 기본 인자로 읽는 판정은 1년에 몇 시간만 테스트된다.** 한정 이벤트 창
  (`LiveEventWindow`, 9/11 08~20시)을 `MultiplayerRoomCenter`·`RaidView` 가 기본 인자로 불렀다.
  그래서 창 밖에서는 이벤트 분기가 **한 번도 실행되지 않는 줄**이었고, 창이 열린 동안에는 반대로
  평소 경로를 전제한 회귀 테스트 둘이 실패했다. CI 러너가 UTC 라 우리 오전에는 늘 창 밖이었다 —
  **초록이 "통과" 가 아니라 "그 분기를 안 밟았다" 였다.** 처방: 날짜로 콘텐츠가 갈리는 판정은
  세이브 스토어의 주입된 시계(`CompanionStore.now`·`isLiveEventActive`·`liveEventEndDate`)를
  지난다. 호출부가 각자 `Date()` 를 읽으면 같은 부류가 그 자리에서 다시 생긴다.
- **테스트 헬퍼도 같은 벽시계를 읽고 있으면 고친 뒤에 반대로 깨진다.** 센터가 스토어 시계를 쓰기
  시작하자 `todaysBoss`·`seedDrawing` 이 `Date()` 로 세운 보스가 "오늘의 보스" 와 갈려 게스트
  검증에 막혔다. 프로덕션 쪽 시계를 주입으로 바꿀 때 **헬퍼의 기본값도 같은 시각으로** 옮긴다.
- **기기 식별자를 씨앗으로 쓰면 같은 테스트가 맥마다 다른 답을 낸다.** 일간 미션 배정 씨앗이
  `DeviceID.stableIdentifier()` 라, 도감 등록 미션이 뽑힌 기기에서만 `hatch` 한 번으로 하나가
  끝나 `missions=0/3` 단언이 깨졌다(CI 초록·로컬 빨강). 처방: `SaveTransfer.deviceSeed` 처럼
  주입 가능한 값으로 두고(기본값은 그대로 기기 식별자) 테스트는 씨앗을 고정한다. 고정할 때
  **어떤 조합이 뽑히는지 확인하고** 고른다 — 아무 문자열이나 박으면 다른 미션이 뽑혀 같은 실패가
  다시 난다. (2026-09-11, #361.)

## 런루프로 사는 CLI 도구는 델리게이트 객체를 **전역에 붙잡지 않으면** 조용히 아무 일도 안 한다

- **`case` 블록 안의 지역 `let` 은 블록이 끝나면 ARC 가 바로 푼다.** BLE 프로브
  (`Tools/BLEProbe/main.swift`)가 `let scanner = Scanner(...)` + `_ = scanner` 로 잡은 척만 했다.
  `_ = x` 는 마지막 사용일 뿐 수명을 늘리지 않아, `RunLoop.main.run()` 에 들어갈 때 이미 스캐너가
  해제돼 CoreBluetooth 매니저도 함께 죽었다. 결과는 **에러도 로그도 없는 무동작** — 표본 0,
  `central state=` 한 줄도 안 찍히고, 상태 타이머가 `[weak self]` 라 첫 발화에서 스스로
  `invalidate()` 해 `--seconds` 가 지나도 프로세스가 안 끝났다.
- **증상이 권한 문제로 위장한다.** 표본 0 + 출력 0 이면 블루투스 권한부터 의심하게 된다(도구
  주석도 그렇게 안내한다). 실제로는 TCC 가 이미 허용(`auth_value=2`)이었다 — 권한을 뒤지느라
  한참 돌았다. 구분법: 권한 거부면 `central state=3` 이 **찍히기라도 한다**. 한 줄도 없으면
  객체가 살아 있지 않은 것이다.
- **왜 못 걸렀나**: 이 도구는 실측용이라 단위 테스트가 없고, PR 머지 전에 **맥 두 대로 실제
  왕복을 한 번도 안 돌렸다**. 컴파일과 리뷰만 통과하면 "동작한다" 로 보였다.
- **처방**: 런루프 도구는 소유자를 전역 변수(`var keepAlive: AnyObject?`)에 대입한다. 소스 가드
  `scripts/test-gate.sh` "런루프 CLI 도구의 소유권 스윕" 이 `RunLoop.main.run()` 을 쓰는
  `Tools/*/main.swift` 에서 `_ = <식별자>` 를 찾아 막고, 결함을 되돌려 넣으면 실제로 빨개지는
  것까지 확인했다. (2026-09-11, #342 0단계 측정 중 발견.)

## 계측 API 의 "못 쟀다" 센티넬을 측정값으로 세면 평균·표준편차가 통째로 뒤집힌다

- **CoreBluetooth 는 신호세기를 못 읽으면 `127` 을 준다.** BLE 프로브가 그 값을 검사 없이
  CSV 에 적었고(`Tools/BLEProbe/main.swift`), 판정 스크립트(`scripts/ble-probe-report.sh`)가
  다시 검사 없이 평균에 넣었다. 실제 60초 측정에서 **표본 70개 중 2개**가 섞였는데, 평균이
  -42.7 에서 -37.9 로, 표준편차가 **4 에서 29 로** 튀었다.
- **틀린 값이 그럴듯해서 더 나쁘다.** 결과가 숫자로 멀쩡히 나오므로 오류로 안 보인다. 저 표준편차는
  #342 0단계의 중단 기준(`낙폭 >= 3*sd`)에 그대로 들어가는 값이라, 눈치 못 챘으면 **기능 존폐
  판정이 센티넬 2개로 갈릴** 뻔했다. 관측이 아니라 "관측 실패" 를 관측으로 센 것이다.
- **왜 못 걸렀나**: 실측 도구라 단위 테스트가 없고, 사람이 보는 출력이 평균 한 줄이라 원시 분포를
  안 봤다. 값 하나하나는 유효 범위 안처럼 보여서(정수·부호 있는 dBm) 형식 검증으로도 안 걸린다.
  분포를 히스토그램으로 펼치고 나서야 `127` 이 두 개 튀어나왔다.
- **처방**: 센티넬은 **생산·소비 양쪽에서 같은 규칙으로** 버린다. 프로브가 기록 전에 버리고
  (`rssiUnavailable`), 판정 스크립트도 읽을 때 버린다 — 옛 프로브가 찍어 둔 CSV 가 남아 있어
  한쪽만 고치면 조용히 어긋난다. 버린 개수는 양쪽 다 출력에 드러낸다(`dropped=` ·
  `버린 센티넬 N`) — 소리 없이 버리면 장비 고장과 정상 측정을 구분할 수 없다.
- **같은 부류를 의심할 자리**: 계측 API 가 실패를 예외가 아니라 **범위 안의 특정 값**으로 알리는
  모든 곳(RSSI `127`, 배터리 `-1`, 좌표 정확도 음수 등). 저장소 전수 조사에서는 이 한 곳뿐이었다
  — 앱에는 아직 스캐너가 없다. 스캐너를 앱에 들일 때 같은 가드를 함께 옮긴다.
  (2026-09-11, #342 0단계 측정 중 발견.)

## 필터가 "보여야 채워지는 값"을 보면, 필터가 노출을 막는 순간 그 값은 영원히 안 채워진다

- **증상**: 상점 기술머신 탭에서 타입·"배울 수 있는 기술만" 필터를 켜면 목록이 거의 비었다.
  사용자 보고 — "상점 기술머신 필터가 안됨".
- **직접원인**: 타입·습득 여부(`machineTypes`/`machineLearnable`)는 각 카드(`TechnicalMachineShopCard`)
  의 `.task` 가 **자신이 실제로 그려질 때만** PokéAPI 를 조회해 채웠다. 그런데 어느 카드가
  그려지는지는 `ForEach(filteredMachines)` — 즉 그 필터 자신이 정한다. 필터를 켜는 순간 아직
  한 번도 그려진 적 없는 항목은 데이터가 없어 필터를 통과 못 하고, 통과 못 하니 카드도 안 그려져
  데이터가 영원히 안 채워지는 순환이 생겼다. `LazyVStack` 이라 스크롤로도 못 푼다 —
  `ForEach` 목록에 없는 항목은 애초에 스크롤할 대상 자체가 없다.
- **왜 못 걸렀나**: 이 기능을 처음 넣을 때 이미 "카드가 조회를 끝내야 채워진다"는 한계를
  주석으로 남겼지만, 그 한계를 "스크롤하면 나중에 채워진다"로 과소평가했다 — 실제로는 필터가
  걸린 항목은 스크롤할 방법 자체가 없어 **한계가 아니라 영구 봉쇄**였다. 뷰 상태·비동기 조회라
  단위 테스트 대상이 아니었고, 소스 리뷰에서도 "카드마다 조회"가 당연해 보여 넘어갔다.
- **처방**: 필터가 보는 값은 **필터링 이전의 전체 후보군**을 대상으로 미리 채운다
  (`prefetchMachineFilterData` — `TechnicalMachine.catalog` 전체를 `withTaskGroup` 으로 한 번에
  조회, 카드 노출과 무관). 이미 같은 모양의 필터를 쓰는 `TeamPicker.swift` 는 처음부터
  `.task(id: all.map(\.currentID))` 로 **필터링 전** `all` 목록을 돌아 정답 패턴이었다 — 상점만
  그 패턴을 안 따라 사고가 났다.
- **부류 규칙**: 목록에 필터를 걸 때, 그 필터가 보는 값이 **비동기로 채워지는 값**이면 항상
  자문한다 — "이 값을 채우는 자리가 **필터를 통과한 항목에만** 도는가, 아니면 **필터링 전
  전체 후보군**에 도는가?" 전자면 필터를 켜는 순간 못 채워진 항목이 영구히 제외되는 순환이다.
  같은 모양의 필터가 이미 있으면(`TeamPicker` 등) 그 구현을 베끼지 말고 **먼저 정답 패턴인지
  확인한 뒤** 베낀다.
...
## Swift 멤버와이즈 이니셜라이저는 라벨이 있어도 선언 순서를 지켜야 하는 부류

- **증상**: 구조체에 새 필드(`evolutionLevel`, `turnSignal` 등)를 추가하고 여러 호출부에
  키워드 인자로 채워 넣었는데, CI(cold build)에서만 `error: argument 'X' must precede
  argument 'Y'` 로 실패했다. 같은 세션에서 **두 번** 났다 — `EvoNode` 이니셜라이저
  (`EvolutionBranchTests.swift`, 야도뇽 회귀 테스트) 와 `BattleChatConfiguration`
  (`BattleView.swift` 등 6개 호출부, 레이드 채팅 포커스 결함 수정) 각각.
- **원인**: Swift 는 인자에 라벨이 붙어 있어도 **호출 시 순서를 구조체 선언 순서와 똑같이
  강제한다** — 키워드 인자라고 재배열이 자유로운 게 아니다. 새 필드를 넣을 자리를 "적당히
  중간"으로 잡고 호출부를 기억에 의존해 채우면, 실제 선언 순서와 다르게 쓰기 쉽다.
  로컬에서 `swift build` 가 안 되는 환경(Xcode 없음)이라 **CI 에 태우기 전까지 안 걸린다** —
  같은 실수를 두 번 다 CI 왕복(빌드 3~4분 + 로그 조회)으로 잡았다.
- **처방**: 새 필드를 추가한 구조체의 여러 호출부를 고칠 때는, 손으로 순서를 기억해 채우지 말고
  **구조체 선언을 먼저 한 번 더 읽어 필드 순서를 확정한 뒤** 모든 호출부를 그 순서로 맞춘다.
  더 안전한 대안은 새 필드를 **선언의 맨 끝**(마지막 클로저 인자가 있다면 그 바로 앞)에 두는
  것 — 그러면 기존 호출부는 그대로 두고 끝에만 추가하면 되어 순서 실수 자체가 안 난다.
- **부류 규칙**: 여러 호출부를 한 번에 고치는 작업에서, 편집 전 "이 구조체의 필드 선언 순서가
  정확히 무엇인지" 를 grep 으로 재확인하지 않고 진행하면 이 부류가 재발한다. 특히 로컬에서
  컴파일을 못 돌리는 환경일수록(이 레포처럼) CI 왕복 비용이 커서 미리 확인하는 쪽이 항상 싸다.
  (`EvoNode`/`BattleChatConfiguration`, 2026-09-22~23.)
