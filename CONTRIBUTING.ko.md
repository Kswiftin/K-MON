...
이 저장소는 협업 산출물의 작업 언어로 **한국어**를 씁니다:

- **풀 리퀘스트 제목과 본문은 한국어로 씁니다.**
- **커밋 메시지도 한국어로 씁니다.**
- 코드 식별자·API 이름·CLI 명령·에러 문자열은 원문 그대로 둡니다.

저장소가 squash-merge하므로 PR 제목이 `main`의 커밋 제목이 됩니다 — 공개 히스토리도
한국어가 되고, 그것이 의도한 결과입니다.

다른 언어가 편하다면 그 언어로 PR을 열고 그렇다고 적어 주세요. 머지 전에 메인테이너가
제목을 옮깁니다. 언어 때문에 기여를 돌려보내려는 규칙이 아닙니다.
