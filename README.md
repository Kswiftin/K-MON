...
4. 경험치로 파트너의 레벨을 올리고, 별의조각은 상점 구매와 랭크 배틀 판돈에 사용합니다. 홈에서 최대 5마리 예비 동행을 골라 모험 파티를 꾸리면, 수령 시 활성 파트너는 경험치 100%를, 고른 예비 동행은 각자 30%를 함께 받습니다. 홈에는 모험 수령·집중 시간·졸업을 세는 일간·주간 미션이, 도감에는 종·타입·이로치 수집 목표가 표시됩니다.
...
- **배틀 규칙** — 타입 상성, STAB, 물리·특수 능력치, 명중, 급소, PP, 기술 우선도, 상태이상 6종, 혼란을 반영합니다. 날씨·필드, 리플렉터·빛의장막·오로라베일 같은 진영 효과, 순풍의 우선도 보정, 방어 계열 가드 8종, 테라스탈까지 지원합니다(모의전과 LAN 1대1에서 사용 가능).
- **지닌 물건** — 배틀에서 쓰이는 지닌 물건 161종을 상점 '배틀' 탭에서 사서 포켓몬에게 들려 줍니다. 타입 강화 도구·열매·주얼·플레이트부터 생명의구슬·구애 계열·돌격조끼까지 있고, 물건마다 대가가 다릅니다. 무엇을 지녔는지는 포켓몬 상세 카드와 격자의 가방 표식으로 확인하고, 함께 다니는 포켓몬은 그 자리에서 벗길 수 있습니다. 배틀 화면에서는 내 포켓몬의 물건만 보이고 상대의 것은 발동할 때 기록으로 드러납니다.
...
- **협동 레이드** — 같은 네트워크에서 최대 8명의 트레이너가 함께 보스를 잡습니다. 오전·오후로 나뉘고, 티어마다 다른 보스가 등장합니다 — 1★는 고급종, 3★는 희귀종, 5★는 전설. 성공 보상과 포획 기회는 시간대·티어마다 각각 한 번씩이라 하루 최대 여섯 번(오전 셋·오후 셋) 도전할 가치가 있습니다. 보상을 받은 뒤에도 다시 참가할 수 있고 실패한 판은 횟수를 쓰지 않습니다. 성공하면 끝까지 방에 남은 참가자마다(쓰러졌어도 나가지만 않았으면 포함) 고급 25%·희귀 15%·전설 5% 확률로 보스를 포획하며, 결과는 채팅과 함께 한 명씩 차례로 공개됩니다.
...
- **웨이브 런** — 첫 포켓몬을 골라 30 웨이브에 도전합니다. 4웨이브마다 보스가 나오고(넘을 때마다 파티 HP 를 70% 까지 회복), 웨이브를 넘길 때마다 아이템 하나를 고릅니다. 야생 웨이브에서는 몬스터볼(판당 5개, 최대 9개)을 던져 상대를 잡아 파티를 여섯 마리까지 늘릴 수 있습니다. 야생 웨이브 여덟 번 중 한 번꼴로 상대가 둘씩 나오고, 이때는 내 포켓몬도 둘이 함께 서는 2대2 전투가 됩니다 — 칸마다 행동을 정하고, 지진처럼 범위가 넓은 기술은 필드의 여러 마리를 한 번에 때립니다. 판은 매번 새로 뽑히고 하루 횟수 제한이 없으며, 진행 중인 판은 앱을 껐다 켜도 이어집니다. 하루 첫 클리어에는 알 1개와 별의조각 5,000개를 받습니다.

## 사파리존과 포코피아 마을

- **사파리존** — 초원·습지·동굴 중 한 존에 입장해 방향키로 벌판을 걸어 다니며 야생 포켓몬을 찾습니다. 조우하면 미끼(포획 확률 상승, 도망 확률도 함께 오름)·진흙(도망 확률 하락, 포획 확률도 함께 내려감)·볼·도망 중 골라 대응하며, 볼을 던지면 포켓몬에게 실제로 날아가는 던지기 애니메이션이 재생됩니다. 조우 화면에서 상대의 성별·타입·특성을 확인할 수 있고, 벌판에는 나무·물웅덩이·바위 같은 장애물이 놓여 있습니다. 방문당 볼·걸음 수, 하루 방문·포획 횟수에 각각 상한이 있습니다.
- **포코피아 마을** — 도감에 등록된 종으로 변신하면 그 타입에 맞는 지형(풀·물·모래·바위 등)을 비용 없이 밀 수 있습니다. 집중 세션을 마칠 때마다 이사 판정이 돌아 마을 환경에 어울리는 포켓몬이 스스로 찾아와 정착하며, 지형을 다양하게 만들수록 마을이 받아들이는 인구가 늘어납니다. 지형 종수와 정착 비율에 따라 환경 레벨(Lv.1~10, 빈 터~포코피아 다섯 단계)이 오르내리는 별도 창입니다.
...
<td width="45%" align="center"><img src="assets/screenshot-pokemon-favorites-ko.png" width="180" alt="즐겨찾기"><br><br><img src="assets/screenshot-pokemon-held-item-ko.png" width="180" alt="포켓몬 상세 카드의 지닌 물건"><br><br><img src="assets/screenshot-collection-pokedex.png" width="180" alt="도감"><br><br><img src="assets/screenshot-collection-catchlog.png" width="180" alt="포켓몬 수집 기록"></td>
</tr>
<tr>
...
<td width="55%" valign="middle">
<h3>상점과 가방</h3>
별의조각으로 알, 이상한 사탕, 민트, 연결의끈, 진화의 돌, 기술머신, 의상을 구매합니다. 배틀에서 쓰는 지닌 물건 161종은 '배틀' 탭에 따로 모여 있어 소비 아이템과 섞이지 않습니다. 재고로 쌓이는 물건은 수량을 정해 한 번에 사고(잔액으로 살 수 있는 최대까지), 가방에서는 아이템을 사용하거나 필요 없는 것을 버립니다.
</td>
</tr>
<tr>
<td width="55%" valign="middle">
...
<td width="55%" valign="middle">
<h3>협동 레이드</h3>
같은 네트워크의 트레이너들과 보스를 함께 잡습니다. 오전과 오후는 물론 티어마다도 다른 보스가 등장하며(1★ 고급·3★ 희귀·5★ 전설), 성공 보상은 시간대·티어마다 한 번씩 받을 수 있습니다. 1★·3★는 아무 때나 열고, 5★는 하루 세 번 정해진 시각에 부화합니다. 화면 아래에서 들고 갈 포켓몬을 고르고, 근처에 열린 방이 있으면 목록에서 바로 참가합니다.
</td>
</tr>
<tr>
<td width="55%" valign="middle">
