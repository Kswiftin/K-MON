...
- [ ] **README.md** — 기능 목록, 화면 구성, 요구사항. 한국어 전용(2026-09-09부로 en/ja 버전 폐지).
- [ ] **`assets/` 스크린샷** — 언어 접미 없이 한 벌. UI(`Sources/PokeTokenBar/UI/`)가 바뀌었으면 갱신하고, **신규 기능이면
      새 에셋을 추가**한다(기존 이미지 재생성만으로는 새 화면이 문서에 없는 상태로 나간다).
      **"어느 스크린샷이 낡았다" 를 이 체크리스트에 손으로 적어 두지 않는다** — 그 메모는 갱신한
      뒤에도 남아, 다음 릴리스가 이미 최신인 에셋을 다시 그리게 한다(v2.26.0 이 옷장 스크린샷에서
      그럴 뻔했다: 메모는 살아 있었지만 에셋이 뷰보다 2시간 늦게 그려져 이미 최신이었다).
      대신 매번 두 시각을 대조한다 — 뒤가 더 최신이면 그때만 다시 그린다.
      ```bash
