...
final class BattleStatusTests: XCTestCase {
    ...
    func testRulesVersionMovesWithTheStatusConditions() {
        XCTAssertEqual(BattleEngine.rulesVersion, 24,
                       ...
                       개체에 붙는 volatile 여섯(조이기·저주·나이트메어의 잔뎀 + 아쿠아링·뿌리박기의 회복 + 씨뿌리기의 HP 이동 — 조이기가 지속 턴 rng 를 하나 더 뽑는다) + 편 방어기 넷(와이드가드·퀵가드·니가하지마·트릭가드 — 연속 실패 확률을 개인 방어와 공유한다) + 배율만 얹는 volatile 다섯(기합충전·레이저포커스의 급소 단계, 작아지기의 두 배 피격과 명중 판정 생략, 방어태세·충전의 위력 두 배 — 다섯 기술이 데미지 경로를 안 지나 rng 소비가 줄어든다) = 24
                       """)
    }
