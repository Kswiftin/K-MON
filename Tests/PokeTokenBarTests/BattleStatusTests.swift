...
final class BattleStatusTests: XCTestCase {
    ...
    func testRulesVersionMovesWithTheStatusConditions() {
        XCTAssertEqual(BattleEngine.rulesVersion, 38,
                       """
                       ...
                       개체에 붙는 volatile 여섯(조이기·저주·나이트메어의 잔뎀 + 아쿠아링·뿌리박기의 회복 + 씨뿌리기의 HP 이동 — 조이기가 지속 턴 rng 를 하나 더 뽑는다) + 편 방어기 넷(와이드가드·퀵가드·니가하지마·트릭가드 — 연속 실패 확률을 개인 방어와 공유한다) + 배율만 얹는 volatile 다섯(기합충전·레이저포커스의 급소 단계, 작아지기의 두 배 피격과 명중 판정 생략, 방어태세·충전의 위력 두 배 — 다섯 기술이 데미지 경로를 안 지나 rng 소비가 줄어든다) + 기절 순간에 답하는 volatile 셋(인내의 HP 1 남기기 — 방어와 연속 실패 카운터를 공유해 rng 를 한 번 더 뽑는다, 운명공동체의 동반 기절, 원한의 PP 0) + 입장 데미지 넷(압정뿌리기·독압정·스텔스록·끈적끈적네트 — 상대 편에 깔리고 교체로 나오는 개체가 밟는다. 난수는 안 쓰지만 같은 입력의 HP·상태·랭크가 갈리고, 층을 쌓는 두 번째 사용이 구버전에선 실패다) + 테라 타입을 스냅샷의 저장 값으로 싣기(테라피스 — 구버전 피어는 첫 번째 타입으로 테라스탈해 STAB·상성이 갈린다) + 지닌물건 3종을 스냅샷에 싣기(생명의구슬의 ×1.3 과 턴 끝 1/10 자해, 기합의띠의 만피 버티기, 먹다남은음식의 턴 끝 1/16 회복 — 구버전 피어는 그 필드를 안 보내 같은 판의 HP 가 갈린다. 난수 소비는 그대로다) + 다인전 타겟 유도 넷(따라와·성원·스포트라이트가 상대의 단일 타겟 공격을 한 자리로 끌어오고, 도우미가 아군의 이번 턴 위력을 1.5 배로 만든다 — 구버전 피어는 넷이 턴만 태워 누가 맞는지와 데미지가 갈리고, 방의 액션 검증이 자기 지목·같은 편 지목을 받게 넓어져 구버전은 그 액션을 거절한다) + 대타출동·쉐도우테일의 층(HP 대신 맞는 인형 — 구버전 피어는 그 기술이 턴만 태우므로 같은 판의 HP·상태·랭크가 갈리고, 층에 막힌 기술이 2차효과·랭크 확률을 안 굴려 난수 소비도 갈린다) + 기술 선택을 막는 여섯(씨앙코르·앙코르·도발·트집·봉인·비밀의힘 — 구버전 피어는 막혔어야 할 기술을 그대로 내고, 낼 기술이 없어 발버둥이 되는 자리를 정상 기술로 본다. CPU 가 후보를 `canUse` 로 걸러 뽑으므로 잠긴 칸이 있으면 난수 소비도 갈린다) + 구애 2종(구애머리띠·구애안경 — 한 계통 ×1.5 배율과 기술 고정이 스냅샷의 `heldItem` 으로 실린다) + 그 잠금 중 넷(씨앙코르·도발·봉인·비밀의힘)의 행동 직전 재판정(잠금을 건 쪽이 먼저 움직인 턴에서 구버전은 이미 고른 기술을 그대로 내고 이 버전은 못 낸다 — 막힌 턴은 명중·급소를 안 굴려 난수 소비도 갈린다) = 24, \
                       지닌물건 4종(구애스카프의 스피드 ×1.5, 화염구슬·독구슬의 턴 끝 자기 상태, 돌격조끼의 특수방어 ×1.5 와 변화기 금지 — 구버전 피어는 그 아이템 이름을 모르는 값으로 접어 같은 판의 HP·행동 순서가 갈린다. 난수 소비는 그대로다) = 25, \
                       타입 강화 도구 22종(해당 타입 기술 ×1.2 — 상성표를 보는 기술에만 붙는다. 구버전 피어는 그 이름을 모르는 값으로 접어 데미지가 갈린다) = 26, \
                       열매 29종(약점 반감 18 — 그 타입 기술의 데미지를 절반으로 깎고 소모된다. 위급 6 — HP 가 1/4 이하일 때 랭크 하나 또는 급소 단계를 올린다. 성격 회복 5 — 같은 조건에서 최대 HP 의 1/3 을 회복한다. 구버전 피어는 그 이름을 모르는 값으로 접어 데미지·랭크·HP 가 갈린다) = 27, \
                       주얼 18종(그 타입 기술 하나 ×1.3 뒤 소모) + 대가만 있는 셋(검은철구 — 스피드 절반에 땅에 닿아 부유·비행이 땅 기술을 못 흘린다, 느림보꼬리·만복향로 — 같은 우선도에서 뒤로 밀린다. 후공이 스피드 동점을 먼저 가르므로 무작위 tie-break 소비도 갈린다) = 28, \
                       플레이트 17종(타입 강화 도구와 같은 ×1.2 — 노말을 뺀 17타입에 하나씩이고, 강철은 금속코트가 진화 아이템이라 비어 있던 자리를 강철플레이트가 채운다) = 29, \
                       특정 종 전용 10종(전기구슬·굵은뼈·금속파우더·스피드파우더의 능력치 배율, 럭키펀치·대파의 급소 단계 +2, 마음의물방울의 특공·특방 ×1.5, 보옥 셋의 두 타입 ×1.2 — 종 조건은 `BattleSide.heldEffect` 한 자리가 보고, 구버전 피어는 그 이름을 모르는 값으로 접어 데미지·급소가 갈린다) = 30, \
                       일반 배틀 도구 12종(힘의머리띠·박식안경의 분류별 ×1.1, 달인의띠의 효과 굉장 ×1.2, 메트로놈의 연속 사용 배율, 초점렌즈의 급소 +1, 광각렌즈·포커스렌즈의 명중 상승, 반짝가루·무사태평향로의 상대 명중 하락, 조개껍질방울·큰뿌리의 회복, 검은오물의 턴 끝 회복 또는 데미지 — 명중이 갈리면 같은 seed 에서 맞고 빗나감이 갈려 난수 소비 횟수까지 달라지고, `DamageCause` 에 원인 하나(`heldItem`)가 늘어 구버전은 그 데미지 줄을 디코딩하지 못한다) = 31, \
                       면역·무시 물건 6종(풍선의 땅 기술 면역과 맞으면 터짐, 통굽부츠의 입장 데미지 무시, 방진고글의 날씨 잔뎀 무시, 만능우산의 볕·비 위력 보정 무시, 겨냥표적의 타입 면역 해제, 가벼운돌의 체중 절반 — 구버전 피어는 그 이름을 모르는 값으로 접어 통하지 않던 기술이 통하고 밟지 않던 함정을 밟는다. 데미지가 아니라 맞고 안 맞고가 갈린다) = 32, \
                       지속 시간을 늘리는 물건 6종(빛의점토의 장막 8턴, 날씨 돌 넷의 자기 날씨 8턴, 그라운드코트의 필드 8턴 — 늘어난 턴은 거는 자리에서 정해지고, 구버전 피어는 5턴에 걷힌 판으로 뒤 턴의 데미지·필드 효과를 계산한다) = 33, \
                       허브·무효화 물건 5종(하양허브의 랭크 원복, 멘탈허브의 선택 잠금 해제, 흉내허브의 랭크 상승 따라하기, 클리어참의 하락 차단, 은밀망토의 부가효과 차단 — 망토가 막은 부가효과는 확률을 굴리지 않아 rng 소비 횟수까지 갈린다) = 34, \
                       방아쇠 하나에 랭크를 올리고 사라지는 물건 10종(약점보험·구근·충전지·눈덩이·빛이끼가 맞은 히트에, 허탕보험이 빗나간 자기 기술에, 씨앗 넷이 발밑의 필드에 답한다 — 구버전 피어는 그 랭크를 안 올려 그 뒤 데미지·명중이 전부 갈린다) = 35, \
                       기술의 성질(접촉·펀치·소리)에 답하는 물건 8종(울퉁불퉁멧·끈적끈적바늘의 접촉 반응, 방호패드·펀치글러브의 접촉 해제, 펀치글러브의 펀치 ×1.1, 속임수주사위의 다단 하한 4, 조임밴드의 조이기 잔뎀 1/6, 끈기갈고리손톱의 조이기 7턴, 목스프레이의 특공 상승 — 끈기갈고리손톱은 4~5턴 난수를 굴리지 않아 rng 소비 횟수까지 갈린다) = 36, \
                       운에 걸린 물건 5종(선제공격손톱의 20% 선공, 기합의머리띠의 10% 버팀, 스타열매의 능력 상승, 애슈열매의 선공, 미클열매의 필중 — 손톱은 턴마다, 머리띠는 치명적인 히트마다 난수를 한 번씩 더 굴려 rng 소비 횟수가 갈린다) = 37, \
                       진화의휘석(아직 진화할 수 있는 개체의 방어·특방 ×1.5 — 스냅샷에 조건 필드가 하나 늘어 구버전 피어는 그 값을 안 보내고, 휘석이 한쪽에서만 일한다) = 38
                       """)
    }
