...
final class BattleStatusTests: XCTestCase {
    ...
    func testRulesVersionMovesWithTheStatusConditions() {
        XCTAssertEqual(BattleEngine.rulesVersion, 23,
                       ...
                       손가락흔들기 입력 = 17, 끊김 판정의 턴 합의(`resolvedThrough`) = 21, 쇼다운 기술 데이터 보정 = 22, 상황 배율·누적 카운터 가변 위력(분화·어시스트파워·악몽·리프블레이드·리벤지 부류) + 날씨·필드·진영 상태 레이어(순풍의 턴 순서 포함) + 방어 부류 + 테라스탈(모의전) = 23
                       """)
    }
