...
final class PeerDiscoveryEndToEndTests: XCTestCase {
    ...
    private func makeStore(_ clock: TestClock, trainerName: String) -> CompanionStore {
        ...
        // 서비스 이름은 `BattleCenter.init` 에서 트레이너 이름으로 굽는다 — 센터보다 먼저 정해야 한다.
        store.setTrainerName(trainerName)
        return store
    }

    /// 조건이 참이 될 때까지 런루프를 돌린다. Bonjour 는 초 단위로 느리고 콜백이 메인 큐로 오므로
    /// `Task.yield` 로는 부족하다.
    ...
    func testASecondTrainerSeesTheAdvertisedLevelAndTiers() async throws {
        ...
        // 광고할 진행도를 미리 만든다 — 레벨 2(25분 정산) + 업적 1단계(레이스 완주).
        ...
        // ① 발견 — 상대 목록에 잡히는가.
        //
        // 실패했을 때 "환경이 막았나 / 코드가 틀렸나" 를 구별할 수 있어야 한다. 리스너가 포트를
        // 얻지 못했다면(`listeningPort == nil`) 광고 자체가 시작되지 않은 것이므로 코드 문제가
        // 아니라 권한·mDNS 차단이다(`startListener` 의 `.waiting` 분기).
        ...
        // ② 광고한 세 값이 그대로 도착하는가 — 여기가 지금까지 무검증이던 구간이다.
        ...
        // ③ 값이 바뀌면 **재시작 없이** 상대 카드가 따라오는가(#85 부류의 수신 측).
