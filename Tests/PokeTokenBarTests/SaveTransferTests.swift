...
final class SaveTransferTests: XCTestCase {
    ...
    /// [설계 못박기] 불러오기는 **무결성 검사를 지나지 않는다** — 사용자가 고른 파일이 이 기기의 서명을
    /// 가질 수 없기 때문이다(다른 기기에서 왔으니 당연히 안 맞는다). 그래서 `isTampered` 의 구버전
    /// 면제 구멍(`integrityVersion` 을 낮게 써 넣으면 검사 면제)을 막아도 **조작 상한은 그대로다**:
    /// 같은 이득이 불러오기로 항상 열려 있다. 반대로 버전 하한을 세이브 밖에 두면 앱을 다운그레이드한
    /// 사용자의 정상 세이브가 초기화되는 오탐만 새로 생긴다. 그래서 그 구멍은 막지 않는다.
