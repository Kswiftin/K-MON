...
final class BattleLogTests: XCTestCase {
    ...
    func testOneAttackBecomesOneLine() {
        let stream: [BattleEvent] = [.turn(1), .move(.a, moveID: 57),
                                     .superEffective(.b), .damage(.b, amount: 122, cause: .move)]
        XCTAssertEqual(lines(stream),
                       ["턴 1", "거북왕의 파도타기! 122 데미지 · 효과가 굉장했다!"])
        // 반쪽 상성도 같은 자리에 붙는다 — 두 방향을 함께 봐야 한쪽만 배선된 경우를 걸러낸다.
        XCTAssertEqual(lines([.move(.b, moveID: 53), .resisted(.a), .damage(.a, amount: 40, cause: .move)]),
    ...
    func testEachTurnAndEachActionGetsItsOwnLine() {
        let stream: [BattleEvent] = [
            .turn(1), .move(.a, moveID: 57), .damage(.b, amount: 10, cause: .move),
            .move(.b, moveID: 53), .damage(.a, amount: 20, cause: .move),
            .turn(2), .move(.a, moveID: 57), .damage(.b, amount: 30, cause: .move),
        ]
    ...
    /// 붙어 있던 상태가 **일한** 줄은 접히지 않는다 — 때린 쪽 줄에 붙이면 누가 버텼는지가
    /// 뒤바뀌어 읽힌다(인내·운명공동체·원한은 맞은 쪽의 줄이다).
    func testATriggeredVolatileGetsItsOwnLine() {
    ...
    func testDamageWithoutAMoveIsNotRenderedAsAMove() {
        let out = lines([.damage(.a, amount: 12, cause: .move)])
    ...
    func testFaintGetsItsOwnLine() {
        XCTAssertEqual(lines([.move(.a, moveID: 57), .damage(.b, amount: 200, cause: .move), .faint(.b)]),
